...
export default function CuentaResidence() {
  ...
  const openNoSaleModal = m => {
    setNoSaleModalMessage(m || 'No hay una cuenta para esta mesa.');
    setNoSaleModalVisible(true);
  };
  ...
  const applyResolveJsonToState = useCallback(json => {
    try {
      ...
      setTotalComensales(
        (oc && oc.total_comensales) ?? json.total_comensales ?? null,
      );
      setFechaApertura(
        (oc && oc.fecha_apertura) ?? json.fecha_apertura ?? null,
      );
      ...
      const reportedTotalFromJson = safeNum(
        (oc && oc.total_consumo) ?? json.total_consumo ?? json.total ?? 0,
      );
      const sumPrecioFieldNoQty = itemsArray.reduce(
        (s, it) =>
          s +
          safeNum(
      ...
      try {
        const montoDesc = safeNum(
          (oc && oc.descuentos_venta && oc.descuentos_venta.monto_total) ??
      ...
      const aperturaStatusRaw = apertura?.status
        ? String(apertura.status).toUpperCase()
        : null;
  ...
  const fetchConsumo = useCallback(
    async (opts = {showLoading: true}) => {
      ...
      try {
        ...
        const resolveUrl = `${API_BASE_URL.replace(
          /\/$/,
          '',
        ...
        if (!res.ok) {
          ...
          try {
            ...
            if (errJson && (errJson.error || errJson.message))
              txt = String(errJson.error || errJson.message);
          } catch (e) {}
          openErrorModal(txt);
          if (isMountedRef.current) setLoading(false);
          return;
  ...
  const handleStartConsumption = async () => {
    ...
    try {
      ...
      const url = `${API_BASE_URL.replace(
        /\/$/,
        '',
      )}/api/mobileapp/residence/qr/open-account`;
      ...
      if (!res.ok) {
        ...
        try {
          ...
          if (ej && (ej.error || ej.message))
            txt = String(ej.error || ej.message);
        } catch (e) {}
  ...
  const handleApproveConsumption = async () => {
    ...
    if (!saleId) {
      Alert.alert(
        'Venta no encontrada',
        'No se pudo determinar el id de la venta.',
      );
      return;
    }
    ...
    try {
      ...
      const url = `${API_BASE_URL.replace(
        /\/$/,
        '',
      )}/api/mobileapp/residence/ventas/${encodeURIComponent(
        String(saleId),
      )}/approve`;
      const res = await fetch(url, {
        method: 'POST',
      ...
      try {
        ...
        const notifText = `Pago registrado: $${amountVal.toLocaleString(
          'es-MX',
          {minimumFractionDigits: 2, maximumFractionDigits: 2},
        )} — Venta ${json.sale_id ?? saleId}${
          json.mesa_id ?? mesaId ? ` (Mesa ${json.mesa_id ?? mesaId})` : ''
        }`;
        const notification = {
          id: notifId,
          text: notifText,
        ...
        const notifKeyBase = usuarioAppId
          ? `user_notifications_${usuarioAppId}`
          : 'user_notifications';
        try {
          ...
          if (json.sale_id) {
            arr = arr.filter(
              n =>
                String(n.payload?.sale_id || n.payload?.venta_id || '') !==
                String(json.sale_id),
            );
          }
  ...
  return (
    <SafeAreaView style={[styles.safe, {paddingTop: topSafe}]}>
      ...
      <Modal
        ...
        <View style={styles.modalBackdrop}>
          <LinearGradient
            ...
            style={[
              styles.modalBox,
              {width: Math.min(layoutWidth - 8, wp(94))},
            ]}>
      ...
      <Modal
        visible={noSaleModalVisible}
        transparent
        animationType="fade"
        onRequestClose={closeNoSaleModal}>
        <View style={styles.noSaleBackdrop}>
          <View
            style={[
              styles.noSaleBox,
              {width: Math.min(layoutWidth - 48, wp(86))},
            ]}>
      ...
      {loading ? (
        ...
        <ScrollView
          contentContainerStyle={[
            styles.container,
            {flexGrow: 1, paddingBottom: Math.max(20, hp(3)) + bottomSafe},
          ]}
          ...
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('QrResidence')}>
            <Text
              style={[
