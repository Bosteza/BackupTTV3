...
export default function ConfirmacionConsumo() {
  ...
  const edificioIdFromParams =
    (route &&
      route.params &&
      (route.params.edificioId ?? route.params.edificio_id)) ||
    null;
  ...
  useEffect(() => {
    ...
    async function fetchRestaurantName() {
      ...
      try {
        ...
        )}/restaurantes/${encodeURIComponent(String(restauranteId))}`;
  ...
  return (
    <View style={styles.root}>
      ...
      <LinearGradient
        ...
        {/* Safe area only for content (not for gradient background) */}
        <SafeAreaView style={styles.topSafeArea}>
          ...
          <View style={styles.topContent}>
            ...
            <View style={styles.miniInfoRow}>
              ...
              <View style={styles.miniInfoCol}>
                {/* Uncomment if you want to show sale label */}
...
function makeStyles({width, height, clamp, wp, hp, rf}) {
  ...
  const overlapCandidate = Math.min(
    28,
    topGradientHeightAdjusted * 0.12,
    headerH * 0.6,
  );
  const overlap = Math.round(
    clamp(overlapCandidate, 10, Math.max(28, Math.round(headerH * 0.6))),
  );
  ...
  return StyleSheet.create({
    ...
    headerTitle: {
      ...
      fontWeight: '800',
      color: '#fff',
      textAlign: 'center', // <-- centered in parent
    },
