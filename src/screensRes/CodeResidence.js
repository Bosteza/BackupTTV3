...
export default function CodeResidence(props) {
  ...
  const onContinue = async () => {
    ...
    if (!trimmed) {
      showToast(
        'Código requerido — ingresa el código enviado por tu residencia.',
        false,
      );
      return;
    }
    ...
    try {
      ...
      if (!mailFromStorage) {
        setLoading(false);
        showToast(
          'Correo no disponible. Asegúrate de haber iniciado sesión.',
          false,
        );
        return;
      }
      ...
      try {
        ...
        setLoading(false);
        showToast(
          'No se pudo conectar con el servidor. Revisa la URL y el token.',
          false,
        );
        return;
      }
      ...
      if (result.ok) {
        ...
        try {
          ...
          showToast(
            'Código verificado — pero no se pudo navegar automáticamente.',
            true,
          );
        }
        return;
      ...
      const serverMsg =
        ...
        `Error del servidor (${result.status}) en ${
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <KeyboardAvoidingView
        ...
        <ScrollView
          contentContainerStyle={[
            ...
            {paddingHorizontal: contentPad, paddingTop: Math.round(hp(4))},
          ]}
          ...
          <Text
            ...
            <Text style={{fontWeight: '700'}}>Términos de Servicio</Text> y
            nuestra{' '}
      ...
      <Animated.View
        ...
        <Text
          style={[styles.toastText, toastSuccess && styles.successToastText]}>
          {toastMsg}
        </Text>
      </Animated.View>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  iconButton: {marginLeft: 20, top: 25},
});
