...
export default function ProfileResidence({navigation}) {
  ...
  const normalizeNotification = (notif = {}) => {
    ...
    const baseText =
      notif.text ||
      raw.text ||
      `Consumo aprobado${approvedByName ? ` por ${approvedByName}` : ''}${
        amount > 0 ? ` por $${formatMoney(amount)}` : ''
      }`;
  ...
  const loadNotificationsFromApi = useCallback(async () => {
    ...
    try {
      const deptId = await AsyncStorage.getItem(
      ...
      const prevKeys = new Set(
        (notificationsRef.current || []).map(
          n => n._key || buildNotificationKey(n),
        ),
      );
      ...
      rawConsumptions.forEach((c, idx) => {
        ...
        const fechaA =
          (c.fechas && (c.fechas.fecha_apertura || c.fechas.fechaApertura)) ||
          c.fecha_apertura ||
          c.fechaApertura ||
          null;
        const fechaC =
        ...
        const notif = normalizeNotification({
          id: c.sale_id
            ...
            : `sale_${idx}_${normalizedDate}`,
          text: `Consumo aprobado${
            approvedByName ? ` por ${approvedByName}` : ''
          } por $${formatMoney(total)}`,
      ...
      if (newItems.length > 0) {
        newItems.forEach(item => {
          try {
            Toast.show(item.text || 'Consumo aprobado', {
    ...
    return (
      <TouchableOpacity
        ...
        <View style={styles.notLeft}>
          <Text style={styles.notBranch} numberOfLines={2}>
            {title}
          </Text>
          ...
          <View style={[styles.modalBox, {width: modalWidth}]}>
            ...
            <ScrollView
              ...
              {notifications && notifications.length > 0 ? (
                notifications.map(n => (
                  <NotificationRow key={n._key || n.id} n={n} />
                ))
              ) : (
