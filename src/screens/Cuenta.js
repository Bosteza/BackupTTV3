    ...
    const unsubscribe = navigation.addListener('focus', async () => {
      try {
        ...
        if (age <= DRAFT_TTL) {
          ...
          if (typeof parsed.firmaDeslinde === 'boolean')
            setFirmaDeslinde(parsed.firmaDeslinde);
          if (typeof parsed.privacyChecked === 'boolean')
            setPrivacyChecked(parsed.privacyChecked);
          if (typeof parsed.termsChecked === 'boolean')
            setTermsChecked(parsed.termsChecked);
        }
      ...
      <ScrollView
        ...
        <TouchableOpacity
          style={[
            ...
            (!(privacyChecked && termsChecked && firmaDeslinde) || loading) && {
              opacity: 0.5,
            },
          ]}
          disabled={
...
function makeStyles({wp, hp, rf, clamp, width, height, Platform, insets}) {
  ...
  return StyleSheet.create({
    ...
    logo: {
      ...
      height: Math.round(clamp(rf(15), 32, 80)),
      resizeMode: 'contain',
    ...
    stepTitle: {
      fontSize: Math.round(clamp(rf(6), 14, 25)),
      marginLeft: 0,
