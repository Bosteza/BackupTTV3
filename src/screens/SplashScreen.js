  ...
  useEffect(() => {
    ...
    const decideAndNavigate = async () => {
      try {
        ...
        const pendingVerification = await AsyncStorage.getItem(
          'pendingVerification',
        );
        ...
        if (email) {
          try {
            ...
            const res = await fetch(
              `https://api.tab-track.com/api/mobileapp/usuarios?mail=${encodeURIComponent(
                email.trim(),
              )}&presign_ttl=30`,
              {
                method: 'GET',
            ...
            console.warn(
              'SplashScreen: API check failed, falling back to local flags',
              apiErr,
            );
          }
        }
