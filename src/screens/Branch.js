...
export default function Branch() {
  ...
  const openMap = () => {
    ...
    Linking.openURL(url).catch(() =>
      showToast('No se pudo abrir la app de mapas'),
    );
  };
  ...
  const avatarUri =
    data.imagen_logo_url ?? data.logo ?? route.params?.logo ?? null;

  // Responsive derived sizes
  const SLIDER_HEIGHT = clamp(
    s(250),
    160,
    Math.round(Math.min(width * 0.7, 420)),
  );
    ...
    <SafeAreaView style={[styles.container, {paddingTop: topInset}]}>
      <View style={[styles.sliderContainer, {height: SLIDER_HEIGHT}]}>
          ...
          {images.length > 0 ? (
            images.map((uri, i) => (
              <Image
                key={i}
                source={{uri}}
                style={[styles.sliderImage, {width, height: SLIDER_HEIGHT}]}
              />
            ))
          ) : (
            <Image
        ...
        <View
          style={[
            styles.dots,
            {bottom: Math.max(8, s(10)), right: Math.max(12, s(16))},
          ]}>
            ...
            <View
              ...
              style={[
                styles.dot,
                {
                  ...
                  marginHorizontal: Math.max(3, s(4)),
                },
      ...
      <ScrollView
        contentContainerStyle={[
          styles.card,
          {paddingTop: AVATAR_SIZE / 2 + 16, paddingHorizontal: CARD_PADDING_H},
        ]}>
        <View style={styles.titleRow}>
          <Text style={[styles.title, {fontSize: clamp(s(24), 16, 28)}]}>
            {data.nombre || data.name || '—'}
          </Text>
          <View style={styles.rating}>
            <Text style={[styles.ratingStar, {fontSize: clamp(s(18), 12, 22)}]}>
              ★
            </Text>
            <Text style={[styles.ratingText, {fontSize: clamp(s(16), 12, 18)}]}>
              {data.avg_rating != null && !Number.isNaN(Number(data.avg_rating))
        ...
        <Text style={[styles.sectionTitle, {fontSize: clamp(s(14), 12, 16)}]}>
          Descripción breve
        </Text>
        <Text
          style={[
            styles.paragraph,
            {fontSize: clamp(s(13), 12, 16), lineHeight: clamp(s(18), 16, 22)},
          ]}>
        ...
        {Array.isArray(data.horarios) && data.horarios.length > 0 ? (
          <>
            <View style={styles.divider} />
            <Text
              style={[styles.sectionTitle, {fontSize: clamp(s(14), 12, 16)}]}>
              Horarios
            </Text>
        ...
        <Text style={[styles.paragraph, {fontSize: clamp(s(13), 12, 16)}]}>
          ...
          <>
            ...
            <TouchableOpacity
              ...
              <Text
                style={[
                  styles.paragraph,
                  {color: '#000', fontSize: clamp(s(13), 12, 15)},
                ]}>
                {data.telefono_sucursal}
              </Text>
            </TouchableOpacity>
        ...
        <View style={styles.buttonRow}>
          <TouchableOpacity
            style={[
              ...
              styles.btnReserve,
              {
                height: clamp(s(40), 36, 48),
                borderRadius: clamp(s(20), 16, 24),
                marginRight: s(8),
              },
            ]}
            onPress={handleReserve}>
            <Text style={[styles.buttonText, {fontSize: clamp(s(16), 14, 18)}]}>
              Reservar
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
        ...
        <View style={styles.socialIconsRow}>
          <TouchableOpacity
            style={[
              ...
              {
                width: clamp(s(44), 36, 56),
                height: clamp(s(44), 36, 56),
                borderRadius: clamp(s(22), 18, 28),
              },
            ]}
            onPress={() =>
              hasWhatsapp
                ? openSocial('whatsapp')
                : showToast('No disponible en esta sucursal')
            }
            activeOpacity={hasWhatsapp ? 0.8 : 1}>
            <Ionicons
              name="logo-whatsapp"
              size={clamp(s(22), 16, 26)}
          ...
          <TouchableOpacity
            ...
            <Image
              source={tiktokIcon}
              style={{
                width: clamp(s(22), 14, 28),
                height: clamp(s(22), 14, 28),
      ...
      {toastVisible && (
        <Animated.View
          ...
          style={[
            styles.toastWrap,
            {transform: [{translateY}], opacity, bottom: toastBottom},
          ]}>
