...
const LOGO_OPENPAY = require('../../assets/images/openpay.jpeg');
const LOGO_PAYNET = require('../../assets/images/paynet.jpeg');
...
export default function OpenPay() {
  ...
  const [awaitingNewTokenForPayment, setAwaitingNewTokenForPayment] =
    useState(false);
  ...
  const buildTransactionUrl = () => {
    const host = String(api_host || 'https://127.0.0.1')
      .trim()
      .replace(/\/$/, '');
  ...
  const handleWebMessage = async event => {
    try {
          ...
          try {
            webviewRef.current &&
              webviewRef.current.postMessage(
                JSON.stringify({type: 'init', payload: initPayload}),
              );
          } catch (e) {
      ...
      if (data.type === 'device_session_created') {
        console.warn(
          '[WebView] device_session_created =',
      ...
      if (data.type === 'openpay_token') {
        if (awaitingNewTokenForPayment) {
          console.warn(
            '[WebView] second openpay_token received (for payment) token=',
            data.token,
          );
        ...
        console.warn(
          '[DEBUG] openpay_token received (first) token=',
          openpay_source_id,
          'device=',
          device_session_id,
        );

        setPendingOpenpayToken({
          openpay_source_id,
          device_session_id,
          holder_name,
          customer_email,
      ...
      if (data.type === 'error') {
        ...
        showPaymentError(
          'Error',
          String(data.message || 'Ocurrió un error en la ventana de pago.'),
  ...
  // Save card on server
  // -----------------------
  const saveOpenpayCardOnServer = async ({
    token_id,
    device_session_id,
    set_preferred,
  }) => {
    ...
    const url = `${hostBase}/api/mobileapp/sucursales/${encodeURIComponent(
      sucursal_id,
    )}/payment-methods/openpay/cards?environment=${encodeURIComponent(
      environment,
    )}`;
    ...
    try {
      ...
      console.warn('read user_usuario_app_id failed', e);
    }
    ...
    console.warn(
      '[DEBUG] saveOpenpayCardOnServer - url:',
      url,
      'body:',
      body,
      'Idempotency-Key:',
    ...
    try {
      ...
      const json = await res.json().catch(() => null);
      console.warn(
        '[DEBUG] saveOpenpayCardOnServer - status:',
        res.status,
        'json:',
        json,
      );
  ...
  const processPaymentWithToken = async ({
    ...
    const monto_subtotal_to_send =
      monto_subtotal !== null
        ? Number(monto_subtotal)
        : safeNum(params.totalSinPropinaFinal ?? params.total ?? 0);
    const monto_propina_to_send =
      monto_propina !== null
        ? Number(monto_propina)
        : safeNum(params.tipAmount ?? 0);

    const items_pagados = Array.isArray(items)
      ? items.map(it => ({
          codigo_item: String(
            it.codigo_item ??
              it.codigo ??
    ...
    console.warn('[DEBUG] processPaymentWithToken - body (pay token):', {
      openpay_source_id,
      device_session_id,
    });
    ...
    try {
      ...
      console.warn(
        '[DEBUG] processPaymentWithToken - create transaction response:',
        json,
      );
      ...
      if (!transactionId) {
        ...
        showPaymentError(
          'Error',
          'El servidor no devolvió transaction_id. Revisa la respuesta en logs.',
          json ? JSON.stringify(json) : null,
        );
        return;
      }
      ...
      if (pollResult.ok) {
        try {
          ...
          console.warn('navigate ConfirmacionPago failed', e);
        }
        return;
      } else {
        showPaymentError(
          'Pendiente',
          'Transacción creada pero no se confirmó el pago inmediatamente.',
      ...
      showPaymentError(
        'Error',
        'No se pudo crear la transacción. Revisa la conexión y la URL.',
  ...
  const processPaymentWithSavedCard = async savedCard => {
    ...
    body.mobile_payment_method_id =
      savedCard?.id ?? savedCard?.mobile_payment_method_id ?? null;
    ...
    try {
      ...
      console.warn(
        '[DEBUG] processPaymentWithSavedCard - create transaction response:',
        json,
      );
      ...
      showPaymentError(
        'Error',
        'No se pudo crear la transacción con tarjeta guardada.',
        err ? JSON.stringify(err) : null,
      );
    }
  };
  ...
  const handleSaveAndPayWithOpenpayToken = async () => {
    ...
    setSavingCard(true);
    try {
      const {openpay_source_id: token_id, device_session_id} =
        pendingOpenpayToken;

      console.warn(
        '[SaveAndPay] token_save:',
        token_id,
        'device:',
        device_session_id,
        'set_preferred=',
      ...
      const saveResp = await saveOpenpayCardOnServer({
        token_id,
        device_session_id,
        set_preferred: savePreferred,
      });
      if (!saveResp.ok) {
        console.warn(
          '[SaveAndPay] saveOpenpayCardOnServer failed',
          saveResp.error,
          saveResp.raw,
        );
        Alert.alert(
          'Advertencia',
          'No se pudo guardar la tarjeta. Se intentará continuar con el pago sin guardar.',
        );
        await processPaymentWithToken({
          openpay_source_id: token_id,
          device_session_id,
          holder_name: pendingOpenpayToken.holder_name,
          customer_email: pendingOpenpayToken.customer_email,
        });
      ...
      console.warn(
        '[SaveAndPay] tarjeta guardada correctamente -> solicitar NUEVO token para pagar (token distinto requerido por ti). saved pm=',
        saveResp.payment_method,
      );
      ...
      console.warn(
        '[SaveAndPay] requesting new token_for_payment (masked): holder=',
        cardDataToRequest.holder_name,
        'last4=',
        (cardDataToRequest.card_number || '').slice(-4),
      );
      try {
        webviewRef.current &&
          webviewRef.current.postMessage(
            JSON.stringify({type: 'create_token', cardData: cardDataToRequest}),
          );
      } catch (e) {
        console.warn(
          '[SaveAndPay] failed to postMessage create_token for new token',
          e,
        );
        setAwaitingNewTokenForPayment(false);
        await processPaymentWithToken({
      ...
      Alert.alert(
        'Error',
        'Ocurrió un error guardando la tarjeta. Se intentará continuar con el pago sin guardar.',
      );
      if (pendingOpenpayToken) {
        const {openpay_source_id: token_id, device_session_id} =
          pendingOpenpayToken;
...
<script>
  ...
  useEffect(() => {
    ...
    (async function fetchCredsAndSendInit() {
      const subtotal =
        Number(monto_subtotal ?? params.monto_subtotal ?? 0) || 0;
      const prop = Number(monto_propina ?? params.monto_propina ?? 0) || 0;
      const computedDisplay = Number(
        (displayAmount ?? subtotal + prop).toFixed(2),
      );
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(
            `Error ${res.status} obteniendo credenciales: ${
              txt || res.statusText
            }`,
          );
        }
        ...
        const pub =
          json.openpay_public_api_key ||
          json.public_api_key ||
          json.publicKey ||
          '';
        const envResp = json.environment || environment || 'sandbox';
        if (!mid || !pub)
          throw new Error('Credenciales incompletas devueltas por servidor.');
        if (!mounted) return;
        const payload = {
          openpay_merchant_id: mid,
          openpay_public_api_key: pub,
          environment: envResp,
        };
        setInitPayload({...payload, displayAmount: computedDisplay});
        if (webReady) {
        ...
        showPaymentError(
          'Error',
          'No se pudieron obtener las credenciales de OpenPay. Revisa servidor/configuración.',
          err ? String(err) : null,
        );
        setLoading(false);
      }
  ...
  const requestTokenFromWebView = cardData => {
    try {
      ...
      console.log(
        '[requestTokenFromWebView] create_token request (masked): holder=',
      ...
      webviewRef.current &&
        webviewRef.current.postMessage(
          JSON.stringify({type: 'create_token', cardData}),
        );
    } catch (e) {
      console.warn('postMessage create_token failed', e);
      showPaymentError(
        'Error',
        'No se pudo iniciar la creación del token.',
        e ? String(e) : null,
      );
      setProcessing(false);
    }
  ...
  const fetchSavedCards = async (openModal = true) => {
    ...
    try {
      ...
      const url = `${hostBase}/api/mobileapp/sucursales/${encodeURIComponent(
        sucursal_id,
      )}/payment-methods?gateway=openpay&environment=${encodeURIComponent(
        environment,
      )}&usuario_app_id=${encodeURIComponent(usuarioAppUuid)}`;
      const idKey = genIdempotencyKey();
      ...
      const pmArr = Array.isArray(json.payment_methods)
      ...
      Alert.alert(
        'Error',
        `No se pudo obtener las tarjetas guardadas: ${String(err)}`,
      );
    } finally {
  ...
  const onPayPress = async () => {
    ...
    if (selectedSavedCard) {
      console.warn(
        '[onPayPress] pago con tarjeta guardada, mobilePaymentMethodId=',
      ...
      if (!deviceSessionId && initPayload && webReady) {
        try {
          ...
          console.warn('init postMessage failed', e);
        }
      }
  ...
  const onSelectSavedCard = cardObj => {
    ...
    setCardNum(
      `${(cardObj.brand || '').toUpperCase()} •••• ${cardObj.last4 ?? ''}`,
    );
  ...
  const computedModalHeight = Math.min(
    maxModalHeight,
    MODAL_HEADER_HEIGHT + contentNeeded + MODAL_FOOTER_HEIGHT,
  );
  ...
  const performDeleteSavedCard = async () => {
    ...
    const url = `${host}/api/mobileapp/sucursales/${encodeURIComponent(
      ...
      cardId,
    )}?gateway=openpay&environment=${encodeURIComponent(environment)}`;
