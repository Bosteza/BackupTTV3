//Working Version checkxing true
...
export default function SelectDefaultHome({navigation}) {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        ...
        const residenceNormalized = String(residenceRaw ?? '')
          .trim()
          .toLowerCase();
        const residenceActive =
          residenceNormalized === 'true' || residenceNormalized === '1';
        setShowResidence(residenceActive);

        const savedHome = String(defaultHomeRaw ?? '')
          .trim()
          .toLowerCase();
  ...
  const chooseHome = async value => {
    try {
      ...
      const finalValue =
        value === 'residence' && !showResidence ? 'home' : value;
...
const styles = StyleSheet.create({
  ...
  optionRowSelected: {
    backgroundColor: 'rgba(47,110,255,0.04)',
    borderRadius: 18,
  },
  circle: {
    alignItems: 'center',
