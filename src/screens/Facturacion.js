...
export default function Facturacion({navigation}) {
  ...
  const parseBackendFieldErrors = (json, fallbackText) => {
    ...
    if (Object.keys(errors).length === 0) {
      const msg = String(
        json?.message || json?.error || fallbackText || '',
      ).toLowerCase();
      if (msg) {
  ...
  const fetchRegimenes = async (force = false) => {
    ...
    try {
      await ensureToken();
      const res = await fetch(`${API_BASE_URL}/api/catalogos/regimenes`, {
      ...
      try {
        ...
        showToast(
          'Respuesta inválida del catálogo de régimen fiscal.',
          'error',
        );
        return [];
      }
  ...
  const fetchUsoCfdiOptions = async (force = false) => {
    ...
    try {
      await ensureToken();
      const res = await fetch(`${API_BASE_URL}/api/catalogos/uso_cfdi`, {
        headers: getAuthHeaders(),
      });
      if (!res || !res.ok) {
  ...
  const getRegimenLabel = () => {
    ...
    const found = regimenes.find(
      r => String(r.codigo) === String(regimenFiscal),
    );
  ...
  const getUsoCfdiLabel = () => {
    if (!usoCfdi) return '';
    const found = usoCfdiOptions.find(
      r => String(r.codigo) === String(usoCfdi),
    );
    ...
    try {
      ...
      if (!res || !res.ok) {
        ...
        if (Object.keys(backendErrors).length > 0) {
          ...
          showToast(
            `No se pudo ${fiscalExists ? 'actualizar' : 'crear'} los datos.`,
            'error',
          );
        }
      ...
      showToast(
        `Datos fiscales ${
          fiscalExists ? 'actualizados' : 'creados'
        } correctamente.`,
        'success',
      );
      await loadFiscal();
      ...
      try {
        ...
        xhr.onload = function () {
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            reject(
              new Error(
                `XHR upload failed status ${xhr.status} response ${xhr.responseText}`,
              ),
            );
          }
        };
  ...
  const presignAndUpload = async (type = 'csf') => {
    try {
      ...
      let presignEndpoint;
      if (type === 'ine') {
        presignEndpoint = `${API_BASE_URL}/api/mobileapp/usuarios/${encodeURIComponent(
          uid,
        )}/fiscal/presign-ine`;
      } else {
        presignEndpoint = `${API_BASE_URL}/api/mobileapp/usuarios/${encodeURIComponent(
      ...
      try {
        ...
        showToast(
          'No se pudo solicitar URL de presign. Revisa la conexión.',
          'error',
        );
        setUploading(false);
        return;
      ...
      if (presignRes.status === 409) {
        ...
        showToast(
          'Ya existen datos fiscales para este usuario. Se cargará la información.',
          'info',
        );
        await loadFiscal();
        setUploading(false);
      ...
      if (!presignRes.ok) {
        ...
        showToast(
          `No se pudo obtener URL para subir el archivo. ${
            txt || presignRes.status
          }`,
          'error',
        );
        setUploading(false);
      ...
      if (typeof uploadUrl !== 'string' || !/^https?:\/\//i.test(uploadUrl)) {
        ...
        showToast(
          'La URL de subida no es válida. Revisa la configuración del servidor.',
          'error',
        );
        setUploading(false);
        return;
      ...
      try {
        if (presignJson.fields && typeof presignJson.fields === 'object') {
          ...
          Object.keys(presignJson.fields).forEach(k =>
            form.append(k, presignJson.fields[k]),
          );
          ...
          if (!postRes || !postRes.ok) {
            ...
            throw new Error(
              `Upload failed (presigned POST): ${
                postRes?.status || 'no status'
              } ${txt || ''}`,
            );
          }
          ...
          if (!fileBlob)
            throw new Error(
              'No se pudo leer el archivo local en formato blob.',
            );

          // subir por XHR PUT (más robusto en RN)
          const headersForXhr =
            presignJson.headers && typeof presignJson.headers === 'object'
              ? presignJson.headers
              : {'Content-Type': 'application/pdf'};

          try {
            await uploadBlobViaXhr(
              uploadUrl,
              uploadMethod,
              headersForXhr,
              fileBlob,
        ...
        setUploading(false);
        showToast(
          'Ocurrió un error durante la subida. Revisa la consola.',
          'error',
        );
        return;
      }
      ...
      if (!uploadOk) {
        showToast(
          'La subida no se completó. Revisa la conectividad con la URL presign.',
          'error',
        );
        setUploading(false);
        return;
      ...
      let commitEndpoint;
      if (type === 'ine') {
        commitEndpoint = `${API_BASE_URL}/api/mobileapp/usuarios/${encodeURIComponent(
          uid,
        )}/fiscal/commit-ine`;
      } else {
        commitEndpoint = `${API_BASE_URL}/api/mobileapp/usuarios/${encodeURIComponent(
      ...
      try {
        ...
        showToast(
          'No se pudo confirmar el archivo (commit). Revisa la conexión.',
          'error',
        );
        setUploading(false);
        return;
      ...
      if (!commitRes || !commitRes.ok) {
        ...
        showToast(
          `No se pudo confirmar el archivo en el servidor. Detalle: ${
            txt || commitRes?.status
          }`,
          'error',
        );
        setUploading(false);
      ...
      showToast(
        `${type === 'ine' ? 'INE' : 'CSF'} subido correctamente.`,
        'success',
      );
      await loadFiscal();
    } catch (err) {
      ...
      if (
        String(err?.message || '').includes('Failed to construct Response') ||
        String(err?.message || '').includes('status 0')
      ) {
        showToast(
          'Error de red al leer el archivo. Revisa la URL/Certificados y la conectividad.',
          'error',
        );
      } else {
  ...
  return (
    <SafeAreaView style={[styles.container, {paddingTop: topSafe}]}>
      ...
      <View
        style={[
          styles.header,
          {paddingHorizontal: headerPaddingH, paddingVertical: headerPaddingV},
        ]}>
        <TouchableOpacity
          ...
          <Ionicons
            name="arrow-back"
            size={clamp(rf(3.4), 20, 26)}
            color={styles.headerTitle.color}
          />
        </TouchableOpacity>
        <Text style={[styles.headerTitle, {fontSize: clamp(rf(4.5), 20, 22)}]}>
          Perfil
        </Text>
        ...
        <View style={styles.headerRight}>
          ...
          <Text
            style={[
              styles.username,
              {
                fontSize: clamp(rf(3.4), 13, 18),
                marginRight: Math.max(8, wp(2)),
              },
            ]}
            numberOfLines={1}>
            {username}
      ...
      <ScrollView
        contentContainerStyle={[
          styles.scrollContent,
          {
            ...
            paddingBottom: Math.max(32, bottomSafe + 24),
          },
        ]}>
        <View style={styles.sectionHeader}>
          ...
          <Text
            style={[
              styles.sectionTitle,
              {fontSize: clamp(rf(3.6), 14, 20), marginLeft: 8},
            ]}>
            Facturación
          </Text>
        </View>
          ...
          <>
            ...
            <View style={{marginTop: 8}}>
              ...
              <TextInput
                ...
                style={[
                  styles.input,
                  fieldErrors.cp ? styles.inputError : null,
                ]}
                placeholder="03100"
                placeholderTextColor="#999"
              />
              {fieldErrors.cp ? (
                <Text style={styles.errorText}>{fieldErrors.cp}</Text>
              ) : null}
            </View>
            ...
            <View style={{marginTop: 12}}>
              ...
              <TextInput
                ...
                style={[
                  styles.input,
                  fieldErrors.razonSocial ? styles.inputError : null,
                ]}
              ...
              {fieldErrors.razonSocial ? (
                <Text style={styles.errorText}>{fieldErrors.razonSocial}</Text>
              ) : null}
            </View>
            ...
            <View style={{marginTop: 12}}>
              ...
              <TouchableOpacity
                style={[
                  styles.input,
                  styles.selectInput,
                  fieldErrors.regimenFiscal ? styles.inputError : null,
                ]}
            ...
            <View style={{marginTop: 12}}>
              ...
              <TextInput
                ...
                style={[
                  styles.input,
                  fieldErrors.rfc ? styles.inputError : null,
                ]}
              ...
              />
              {fieldErrors.rfc ? (
                <Text style={styles.errorText}>{fieldErrors.rfc}</Text>
              ) : null}
            </View>
            ...
            <View style={{marginTop: 12}}>
              ...
              <TouchableOpacity
                style={[
                  styles.input,
                  styles.selectInput,
                  fieldErrors.usoCfdi ? styles.inputError : null,
                ]}
              ...
              {fieldErrors.usoCfdi ? (
                <Text style={styles.errorText}>{fieldErrors.usoCfdi}</Text>
              ) : null}
            </View>
            ...
            <TouchableOpacity
              style={[
                styles.saveButton,
                {
                  marginTop: saveBtnMarginTop,
                  paddingVertical: saveBtnPaddingV,
                  paddingHorizontal: saveBtnPaddingH,
                },
              ]}
              onPress={saveFiscal}
              disabled={saving}
              ...
              {saving ? (
                ...
                <Text style={styles.saveButtonText}>
                  {fiscalExists ? 'Actualizar' : 'Crear y guardar'}
                </Text>
              )}
            </TouchableOpacity>
            ...
            <View style={{marginBottom: 12}}>
              ...
              <View
                ...
                <Text style={{color: csfKeyPresent ? '#0b8f56' : '#666'}}>
                  {csfKeyPresent ? 'CSF cargada' : 'CSF no cargada'}
                </Text>
                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                  {csfPresignedUrl ? (
                    <TouchableOpacity
                      onPress={() => openUrl(csfPresignedUrl)}
                  ...
                  <TouchableOpacity
                    ...
                    {uploading ? (
                      ...
                      <Text style={styles.smallBtnText}>Subir CSF (PDF)</Text>
                    )}
                  </TouchableOpacity>
                </View>
              </View>
            ...
            <View style={{marginBottom: 12}}>
              ...
              <View
                ...
                <Text style={{color: ineKeyPresent ? '#0b8f56' : '#666'}}>
                  {ineKeyPresent ? 'INE cargada' : 'INE no cargada'}
                </Text>
                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                  {inePresignedUrl ? (
                    <TouchableOpacity
                      onPress={() => openUrl(inePresignedUrl)}
                      style={styles.smallBtn}
                      hitSlop={{top: 8, bottom: 8, left: 8, right: 8}}>
                      <Text style={styles.smallBtnText}>Ver</Text>
                    </TouchableOpacity>
                  ...
                  <TouchableOpacity
                    ...
                    {uploading ? (
                      ...
                      <Text style={styles.smallBtnText}>Subir INE (PDF)</Text>
                    )}
                  </TouchableOpacity>
                </View>
              </View>
        ...
        <View style={styles.pickerOverlay}>
          <View style={styles.pickerContainer}>
            <View style={styles.pickerHeader}>
              ...
              <TouchableOpacity
                onPress={() => setRegimenModalVisible(false)}
                hitSlop={{top: 8, bottom: 8, left: 8, right: 8}}>
        ...
        <View style={styles.pickerOverlay}>
          <View style={styles.pickerContainer}>
            <View style={styles.pickerHeader}>
              ...
              <TouchableOpacity
                onPress={() => setUsoCfdiModalVisible(false)}
                hitSlop={{top: 8, bottom: 8, left: 8, right: 8}}>
                <Ionicons name="close" size={22} color="#333" />
              </TouchableOpacity>
            </View>
            {loadingUsoCfdi ? (
              ...
              <View style={{padding: 24, alignItems: 'center'}}>
                ...
                <TouchableOpacity
                  onPress={() => fetchUsoCfdiOptions(true)}
                  style={styles.smallBtn}>
                  <Text style={styles.smallBtnText}>Reintentar</Text>
                </TouchableOpacity>
              </View>
