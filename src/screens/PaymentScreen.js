...
const isIosVersionCompatibleWithApplePay = () => {
  ...
  return (
    ...
    majorVersion >= MIN_IOS_VERSION_FOR_APPLE_PAY
  );
};
...
export default function PaymentMarketplace() {
  ...
  const sucursal_id =
    params.sucursal_id ?? params.sucursalId ?? params.sucursal ?? null;

  const restaurante_id =
    params.restaurante_id ?? params.restauranteId ?? params.restaurante ?? null;
  ...
  const normalizeItems = itemsArr => {
    ...
    return itemsArr.map((it, idx) => {
      ...
      const line =
        Number(
          it.lineTotal ?? it.line_total ?? it.total ?? +(unit * qty).toFixed(2),
        ) || +(unit * qty).toFixed(2);

      const name =
        it.name ?? it.nombre ?? it.nombre_item ?? it.title ?? `Item ${idx + 1}`;
  ...
  const [userEmail, setUserEmail] = useState(
    params.user_email ?? params.userEmail ?? '',
  );

  const [userFullname, setUserFullname] = useState(
    params.user_fullname ?? params.userFullname ?? '',
  );

  const [usuarioAppId, setUsuarioAppId] = useState(
    params.usuario_app_id ??
      params.user_usuario_app_id ??
      params.userUsuarioAppId ??
      '',
  );
  ...
  const [restaurantPaymentEnvironment, setRestaurantPaymentEnvironment] =
    useState('');
  ...
  // Compatibilidad de Apple Pay a nivel dispositivo
  // (SO + versión + soporte real de Stripe/Wallet).
  ...
  const fallbackEqualSplitBase = splitAmountByIndex(
    splitBaseForCharge,
    groupPeopleCount,
    0,
  );

  const fallbackEqualSplitTip =
    tipPercent > 0
      ? round2(fallbackEqualSplitBase * (tipPercent / 100))
      : Number(tipAmount || 0);
  ...
  const buildPaymentMethodsUrl = useCallback(
    (userId, restId) => {
      const query = `usuario_app_id=${encodeURIComponent(userId)}${
        restId ? `&id_restaurante=${encodeURIComponent(restId)}` : ''
      }`;
  ...
  const buildRestaurantPaymentsUrl = useCallback(
    restId =>
      `${hostBase()}/api/restaurantes/${encodeURIComponent(restId)}/payments`,
    [hostBase],
  );
  ...
  const buildTransactionUrl = useCallback(() => {
    ...
    if (host.includes('/api/transacciones-pago')) {
      return host;
    }
  ...
  const buildSplitsUrl = useCallback(
    transactionId =>
      `${hostBase()}/api/transacciones-pago/${encodeURIComponent(
        transactionId,
      )}/splits`,
    [hostBase],
  );
  ...
  const buildSaleSplitsUrl = useCallback(
    (sucursalId, saleId) =>
      `${hostBase()}/api/transacciones-pago/sucursal/${encodeURIComponent(
        String(sucursalId),
      )}/ventas/${encodeURIComponent(String(saleId))}/splits`,
    [hostBase],
  );
  ...
  // El API distingue el wallet por "type" (apple_pay) y/o "category" (wallet), NO por
  // el campo "gateway" (que sigue siendo "stripe" porque es quien procesa el cobro).
  // Por eso normalizamos el gateway efectivo a "apple_pay" cuando detectamos ese wallet,
  // para que toda la UI (renderMethodRow, methodGatewaySet, placeholders, etc.) lo trate
  // de forma consistente como Apple Pay y no como una tarjeta guardada.
  const isWalletApplePayPayload = pm => {
    const rawType = String(pm?.type ?? pm?.wallet_type ?? '')
      .toLowerCase()
      .trim();

    const rawCategory = String(pm?.category ?? '')
      .toLowerCase()
      .trim();

    if (rawType === 'apple_pay' || rawType === 'applepay') {
      return true;
    }

    if (rawCategory === 'wallet' && rawType.includes('apple')) {
      return true;
    }
  ...
  // Nuevo formato simplificado de /payments:
  // solo importa configured + enabled.
    ...
    return source
      ...
      .map(payment => ({
        ...
        gateway: normalizeGateway(
          payment.gateway ?? payment.provider ?? payment.name,
        ),
      }))
      .filter(payment => payment.gateway)
      .filter(payment =>
        isApplePayGatewayName(payment.gateway) ? applePayDeviceSupported : true,
      );
  };
  ...
  const loadRestaurantPayments = useCallback(async () => {
    ...
    try {
      ...
      setRestaurantPaymentEnvironment(
        json?.environment ?? json?.data?.environment ?? '',
      );
    ...
    buildRestaurantPaymentsUrl,
    getAuthHeaders,
    restaurante_id,
    applePayDeviceSupported,
  ]);

  const resolveAvailableGateways = (restaurantGateways, normalizedMethods) => {
    const readyGateways = Array.from(
      new Set((restaurantGateways || []).map(normalizeGateway).filter(Boolean)),
    );

    if (readyGateways.length) {
      return readyGateways;
    }

    return Array.from(
      new Set(
        (normalizedMethods || [])
          .map(method => normalizeGateway(method.gateway))
          .filter(Boolean),
      ),
    );
  };
  ...
  const resolveCustomerForPayment = useCallback(async () => {
    ...
    if (!name) {
      ...
      name = String(
        storedFull || `${storedNombre ?? ''} ${storedApellido ?? ''}`.trim(),
      ).trim();
  ...
  // Detecta si el dispositivo (SO + versión)
  // y el SDK de Stripe soportan Apple Pay.
  useEffect(() => {
    let mounted = true;
    ...
    (async () => {
      if (!isIosVersionCompatibleWithApplePay()) {
        if (mounted) {
          setApplePayDeviceSupported(false);
        }
      ...
      try {
        ...
        if (mounted) {
          setApplePayDeviceSupported(Boolean(supported));
        }
      } catch (err) {
  ...
  // --- MAGNES: recolectar device ID para PayPal Fraud Protection
  ...
  const loadMarketplaceMethods = useCallback(async () => {
    ...
    try {
      ...
      const methodsUrl = buildPaymentMethodsUrl(userId, restaurante_id);

      const generalMethodsUrl = buildPaymentMethodsUrl(userId, null);

      console.log('[loadMarketplaceMethods] ‼️REQUEST URL:', methodsUrl);

      console.log(
        '[loadMarketplaceMethods] GENERAL METHODS URL:',
        generalMethodsUrl,
      );
      ...
      const [res, generalRes] = await Promise.all([
        fetch(methodsUrl, {
          method: 'GET',
          headers: getAuthHeaders({
            'Idempotency-Key': genIdempotencyKey('pm-restaurant'),
          }),
        }),
        fetch(generalMethodsUrl, {
          method: 'GET',
          headers: getAuthHeaders({
            'Idempotency-Key': genIdempotencyKey('pm-general'),
          }),
        }),
      ]);

      const [json, generalJson] = await Promise.all([
        res.json().catch(() => null),
        generalRes.json().catch(() => null),
      ]);

      console.log(
        '[loadMarketplaceMethods]🥹 COMPLETE RESPONSE:',
        JSON.stringify(json, null, 2),
      );

      console.log(
        '[loadMarketplaceMethods] GENERAL RESPONSE:',
      ...
      const restaurantMethods = Array.isArray(envBucket?.payment_methods)
      ...
      // The restaurant-specific endpoint may omit the virtual Apple Pay
      // method even when the user saved it as preferred. Read that preference
      // from the general endpoint and merge it only when this iPhone supports
      // Apple Pay and the restaurant has Stripe/Apple Pay enabled.
      const generalEnvBucket =
        generalJson?.environments?.[PAYMENT_METHODS_ENVIRONMENT];

      const generalMethods = Array.isArray(generalEnvBucket?.payment_methods)
        ? generalEnvBucket.payment_methods
        : Array.isArray(generalJson?.payment_methods)
        ? generalJson.payment_methods
        : Array.isArray(generalJson?.data?.payment_methods)
        ? generalJson.data.payment_methods
        : Array.isArray(generalJson?.data)
        ? generalJson.data
        : Array.isArray(generalJson)
        ? generalJson
        : [];

      if (!generalRes.ok) {
        console.warn(
          'loadMarketplaceMethods general methods error',
          generalRes.status,
          generalJson,
        );
      }

      const preferredApplePay = generalMethods.find(method => {
        const type = String(method?.type || '')
          .toLowerCase()
          .trim();

        return (
          (type === 'apple_pay' || type === 'applepay') &&
          Boolean(method?.is_preferred ?? method?.preferred)
        );
      });

      const normalizedRestaurantGateways = restaurantGateways
        .map(normalizeGateway)
        .filter(Boolean);

      const restaurantSupportsApplePay =
        applePayDeviceSupported &&
        (normalizedRestaurantGateways.includes('stripe') ||
          normalizedRestaurantGateways.some(isApplePayGatewayName));

      const restaurantAlreadyHasApplePay = restaurantMethods.some(
        isWalletApplePayPayload,
      );

      const mergedMethods = [...restaurantMethods];

      if (
        preferredApplePay &&
        restaurantSupportsApplePay &&
        !restaurantAlreadyHasApplePay
      ) {
        mergedMethods.push(preferredApplePay);
      }

      const normalized = mergedMethods
        .map(normalizePaymentMethod)
        .filter(method =>
          isApplePayGatewayName(method.gateway)
            ? applePayDeviceSupported
            : true,
        );
      ...
      const filtered = normalized.filter(method => {
        const gateway = normalizeGateway(method.gateway);

        // Apple Pay comes from Stripe, even though the UI identifies it
        // separately as "apple_pay".
        if (isApplePayGatewayName(gateway)) {
          return gateways.includes('apple_pay') || gateways.includes('stripe');
        }
      ...
      const preferred =
        filtered.find(method => Boolean(method.is_preferred)) || null;
  ...
  const pollSplitsUntilPaid = async (
    ...
    while (
      !pollingRef.current.stopRequested &&
      ...
      try {
        ...
        if (res.ok) {
          ...
          const paidSplits = splitsArr.filter(
            split => String(split.estado ?? '').toLowerCase() === 'paid',
          );

          if (paidSplits.length > 0) {
            const paidCodes = paidSplits
              .map(split =>
                String(
                  split.codigo_item ?? split.codigo ?? split.code ?? '',
                ).trim(),
              )
              .filter(Boolean);
    ...
    return {
      ok: false,
      reason: 'timeout',
      last: pollingRef.current.lastResult,
    };
  };
  ...
  const buildItemsForGateway = baseAmount => {
    ...
    return (Array.isArray(items) ? items : []).map(item => ({
      codigo_item:
        ...
        item.codigo ??
        item.code ??
        item.original_line_id ??
        String(item.id ?? ''),
      nombre_item: item.name ?? item.nombre ?? '',
      cantidad: Number(item.qty ?? item.cantidad ?? 1) || 1,
      precio_unitario:
        Number(
          item.unitPrice ?? item.price ?? item.precio_item ?? item.precio ?? 0,
        ) || 0,
    }));
  };
  ...
  const resolveEqualSplitCharge = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const computedTip =
          tipPercent > 0
            ? round2(baseAmount * (tipPercent / 100))
            : fallbackTip;
      ...
      const paidEqualSplits = splitsArr.filter(split => {
        const estado = String(split.estado ?? '').toLowerCase();
        ...
        const name = String(
          split.nombre_item ?? split.nombre ?? split.name ?? '',
        ).toLowerCase();
  ...
  // walletType: cuando se paga con Apple Pay,
  // gateway sigue siendo 'stripe'
  // y se agrega wallet_type: 'apple_pay'.
  // Para PayPal: flow cambia a 'checkout'
  // y se agrega paypal_client_metadata_id de Magnes.
  const createTransaction = async ({
    ...
    const body = {
      ...
      metadata: {
        ...
        venta_id: sale_id ?? '',
        equal_split_paid_count: chargeInfo.paidCount ?? 0,
      },
      mesa_id: mesa_id ?? null,
    ...
    if (savedMethod) {
      body.mobile_payment_method_id =
        savedMethod.id ?? savedMethod.mobile_payment_method_id ?? null;

      if (customer.usuarioAppId) {
        body.usuario_app_uuid = customer.usuarioAppId;
      }
    }
    ...
    if (!transactionId) {
      throw new Error('El servidor no devolvió transaction_id');
    }

    try {
      if (sale_id) {
        await AsyncStorage.setItem(
          `last_transaction_${sale_id}`,
  ...
  const validateBeforePayment = () => {
    if (!apiHost) {
      throw new Error('Falta api_host para crear el pago');
    }

    if (!sucursal_id) {
      throw new Error('Falta sucursal_id para crear el pago');
    }

    if (!sale_id) {
      throw new Error('Falta sale_id o venta_id para verificar el pago');
    }

    if (!restaurante_id) {
      throw new Error('Falta restaurante_id para validar métodos de pago');
    }
  ...
  const hasPaypalMethodSaved = () =>
    methods.some(
      method =>
        normalizeGateway(method.gateway) === 'paypal' && !method.isPlaceholder,
    );
  ...
  const payWithSavedMethod = async method => {
    ...
    if (gateway !== 'stripe') {
      setNotice({
        ...
        message:
          'Este método ya queda preparado en la pantalla, pero aún falta conectar su flujo de pago.',
      });
    ...
    try {
      ...
      if (poll.ok) {
        ...
        showPaymentError(
          'Pago pendiente',
          'El servidor aún no refleja la venta como pagada.',
          JSON.stringify(poll),
        );
      }
    } catch (err) {
      ...
      showPaymentError(
        'Pago no procesado',
        err?.message || 'No se pudo procesar el pago con el método guardado.',
      );
    } finally {
      setProcessing(false);
    }
  ...
  const createSetupIntentOnServer = async () => {
    ...
    if (!userId) {
      throw new Error('Falta usuario_app_id');
    }
    ...
    const res = await fetch(buildSetupIntentUrl(), {
      ...
      body: JSON.stringify({
        ...
        set_preferred: true,
        environment: PAYMENT_METHODS_ENVIRONMENT,
      }),
    });
    ...
    if (!clientSecret) {
      throw new Error('El servidor no devolvió client_secret');
    }
  ...
  const confirmAndSaveCard = async () => {
    ...
    if (accountIdToUse) {
      setStripeAccountId(accountIdToUse);
    }
    ...
    if (res.error) {
      throw new Error(res.error.message || 'No se pudo guardar la tarjeta');
    }
  ...
  const payWithManualStripeCard = async ({save}) => {
    ...
    try {
      ...
      if (error) {
        throw new Error(
          error.message || 'Error al confirmar el pago con Stripe',
        );
      }
      ...
      if (
        ...
        showPaymentError(
          'Pago pendiente',
          'Stripe confirmó el pago, pero el servidor aún no refleja la venta como pagada.',
          JSON.stringify(poll),
        );
      ...
      showPaymentError(
        'Pago no completado',
        `Estado del pago: ${String(paymentIntent?.status)}`,
  ...
  const payWithApplePay = async () => {
    ...
    try {
      ...
      if (error) {
        throw new Error(
          error.message || 'Error al confirmar el pago con Apple Pay',
        );
      }
      ...
      if (
        ...
        showPaymentError(
          'Pago pendiente',
          'Apple Pay confirmó el pago, pero el servidor aún no refleja la venta como pagada.',
          JSON.stringify(poll),
        );
      ...
      showPaymentError(
        'Pago no procesado',
        err?.message || 'No se pudo procesar el pago con Apple Pay.',
      );
    } finally {
      setProcessing(false);
    }
  ...
  // --- PayPal: abre checkout_url y hace polling
  // con intervalMs reducido a 1500ms ---
  // Se quitó el AppState listener porque en Android causaba
  // que el polling iniciara más tarde de lo esperado,
  // aumentando el tiempo total de espera.
  const payWithPaypal = async (savedMethod = null) => {
    ...
    try {
      ...
      try {
        ...
        const stcRes = await fetch(
          `${hostBase()}/api/mobileapp/paypal/risk/transaction-context`,
          {
            method: 'POST',
            headers: getAuthHeaders(),
            body: JSON.stringify({
        ...
        console.warn(
          '[PayPal STC] Error al enviar contexto de riesgo (no bloquea el pago):',
          stcErr,
        );
      }
      ...
      // Polling directo con intervalo de 1500ms
      // (la mitad del default de Stripe).
      // No usamos AppState porque en Android el evento
      // 'active' no llega de forma confiable cuando
      // se regresa de un browser externo.
      ...
      if (poll.ok) {
        ...
        showPaymentError(
          'Pago pendiente',
          'El servidor aún no refleja la venta como pagada con PayPal.',
          JSON.stringify(poll),
        );
      }
    } catch (err) {
      ...
      showPaymentError(
        'Pago no procesado',
        err?.message || 'No se pudo procesar el pago con PayPal.',
      );
    } finally {
      setProcessing(false);
    }
  ...
  const openManualGateway = gateway => {
    ...
    setNotice({
      ...
      title: 'Método preparado',
      message:
        'Este método se mostrará aquí cuando su integración esté lista para marketplace.',
    });
  };
  ...
  const renderCardNetworkLogos = () => (
    <View style={styles.cardNetworkRow}>
      <View style={styles.miniVisa}>
        <Text style={styles.miniVisaText}>VISA</Text>
      </View>

      <View style={styles.miniMastercard}>
        <View
          style={[styles.miniMastercardCircle, styles.miniMastercardLeft]}
        />

        <View
          style={[styles.miniMastercardCircle, styles.miniMastercardRight]}
        />
      </View>

      <View style={styles.miniAmex}>
        <Text style={styles.miniAmexText}>AMEX</Text>
      </View>
    </View>
  );
  ...
  const preferredMethod =
    methods.find(method => Boolean(method.is_preferred)) || null;

  const hasMethods = methods.length > 0;

  //------------------------------------------ REVISAR
  const gatewaysForEmptyState = useMemo(() => {
    const gateways = [...availableGateways];

    const applePayIndex = gateways.findIndex(gateway =>
      isApplePayGatewayName(gateway),
    );

    // No preferred/saved method + Apple Pay available:
    // ALWAYS show Apple Pay first.
    if (!preferredMethod && !hasMethods && applePayIndex > -1) {
      const [applePay] = gateways.splice(applePayIndex, 1);

      return [applePay, ...gateways];
    }

    return gateways;
  }, [availableGateways, preferredMethod, hasMethods]);

  // Pasarelas que el restaurante tiene habilitadas
  // pero que todavía no están representadas por un método guardado
  // (ej. PayPal, o Apple Pay cuando no viene como método "guardado").
  // Las tarjetas (stripe/card) se excluyen porque ya se ofrecen
  // vía "Usar una tarjeta nueva".
  const methodGatewaySet = useMemo(
    () => new Set(methods.map(method => normalizeGateway(method.gateway))),
    [methods],
  );
  ...
  const placeholderGatewayMethods = useMemo(() => {
    return availableGateways
      .filter(gateway => {
        const norm = normalizeGateway(gateway);

        if (!norm) return false;

        if (norm === 'stripe' || norm === 'card') {
          return false;
        }

        if (methodGatewaySet.has(norm)) {
          return false;
        }
  ...
  // Lista para la sección "Otros métodos":
  // todos los métodos guardados que no sean el preferido
  // (ya se muestra arriba) + las pasarelas disponibles
  // que aún no tienen método guardado
  // (PayPal, Apple Pay sin método guardado, etc).
  // Si el preferido ya es Apple Pay, este queda excluido
  // tanto de "nonPreferred" como de los placeholders.
  const preferredKey = preferredMethod
    ? String(preferredMethod.id ?? preferredMethod.external_payment_method_id)
    : null;

  const otherMethodsList = useMemo(() => {
    const nonPreferred = methods.filter(
      method =>
        String(method.id ?? method.external_payment_method_id) !== preferredKey,
    );
      ...
      <View style={[styles.brandMark, styles[`brandMark_${mark.style}`]]}>
        {mark.style === 'mastercard' ? (
          ...
          <Text
            style={[
              styles.brandText,
              mark.style === 'visa' && styles.brandTextVisa,
            ]}>
            {mark.text}
          </Text>
        )}
  ...
  const renderMethodRow = method => {
    ...
    const selected =
      selectedMethod &&
      String(selectedMethod.id ?? selectedMethod.external_payment_method_id) ===
        String(methodId);
    ...
    // --- CAMBIO: PayPal ocupa toda la fila
    // con el logo completo ---
    if (isPaypalGateway) {
      return (
        <TouchableOpacity
    ...
    if (!isCardGateway) {
      // Apple Pay (guardado o no)
      // y cualquier otra pasarela sin datos de tarjeta:
      // solo mostramos el logo, el nombre de la pasarela
      // y, si aplica, el chip "Predeterminado".
      return (
        <TouchableOpacity
          key={`${gateway}-${methodId}`}
    ...
    const exp =
      method.exp_month && method.exp_year
        ? `${method.exp_month}/${String(method.exp_year).slice(-2)}`
        : '--/--';
      ...
      <TouchableOpacity
        ...
        <View style={{flex: 1}}>
          ...
          <Text style={styles.methodSub}>
            {getBrandLabel(method.brand)} - {gatewayLabel(method.gateway)} -
            Expira {exp}
          </Text>
        </View>
        ...
        <LinearGradient
          ...
          <View style={styles.gradientRow}>
            <View style={styles.leftCol}>
              <Image
                source={TABTRACK_LOGO}
                style={[
                  styles.tabtrackLogo,
                  {
                    width: logoWidth,
                    height: Math.round(logoWidth * 0.32),
                    marginBottom: Math.max(8, hp(1)),
                  },
                ]}
                resizeMode="contain"
              />
              ...
              <View
                style={[
                  styles.logoWrap,
                  {
                    marginTop: Math.max(6, hp(0.5)),
                    padding: Math.max(6, wp(1.5)),
                    borderRadius: Math.max(8, wp(2)),
                  },
                ]}>
                <Image
                  source={
                  ...
                  style={[
                    ...
                    {
                      width: restaurantImgSize,
                      height: restaurantImgSize,
                      borderRadius: Math.round(restaurantImgSize * 0.16),
                    },
                  ]}
                />
              </View>
            ...
            <View
              style={[
                styles.rightCol,
                {
                  maxWidth: rightColMaxWidth,
                  marginRight: Math.max(12, wp(3)),
                },
              ]}>
              <Text
                style={[
              ...
              <View
                ...
                <Text
                  style={[
                    ...
                    {
                      fontSize: totalCurrencyFont,
                      marginLeft: Math.max(6, wp(1.6)),
                    },
                  ]}>
                  {moneda ?? 'MXN'}
                </Text>
              ...
              <View style={styles.rightThanks}>
                ...
                <Text
                  style={[
                    styles.itemsTipText,
                    {
                      fontSize: clamp(rf(2.2), 11, 13),
                    },
                  ]}>
                  {items.length} {items.length === 1 ? 'item' : 'items'} ·
                  Propina {formatMoney(displayCharge.tipAmount)} {moneda}
                </Text>
              </View>
            </View>
          </View>
        ...
        <View
          ...
          <View style={styles.securityLine}>
            ...
            <Text style={styles.securityText}>
              Pago seguro. Solo verás métodos disponibles para este restaurante.
            </Text>
          </View>
          ...
          {loadingMethods ? (
            ...
            <>
              ...
              {showOtherMethods || !preferredMethod ? (
                <View style={styles.featureBlock}>
                  <View style={styles.blockHeader}>
                    <View>
                      <Text style={styles.blockTitle}>
                        {preferredMethod ? 'Otros métodos' : 'Elige cómo pagar'}
                      </Text>

                      <Text style={styles.blockSub}>
                        Selecciona un método y confirma el pago.
                      </Text>
                    </View>
                  </View>
                  ...
                  <TouchableOpacity
                    style={styles.linkRow}
                    onPress={() => openManualGateway('stripe')}>
            ...
            <View style={styles.featureBlock}>
              ...
              {gatewaysForEmptyState.length ? (
                gatewaysForEmptyState.map(renderGatewayOption)
              ) : (
                <View style={styles.emptyPaymentsBox}>
                  ...
                  <Text style={styles.emptyPaymentsText}>
                    Puedes regresar e intentar más tarde o consultar con el
                    restaurante.
                  </Text>
                </View>
              )}
      ...
      <KeyboardAvoidingView
        ...
        <ScrollView
          contentContainerStyle={[
            ...
            {
              paddingHorizontal: pagePadding,
              paddingBottom: Math.max(insets.bottom, 16) + 210,
            },
          ]}
          ...
          <View style={styles.cardPreview}>
            <View style={styles.cardPreviewTop}>
              <Text style={styles.cardPreviewBrand}>
                {getBrandLabel(manualCardDetails?.brand)}
              </Text>
            ...
            <View style={styles.cardPreviewBottom}>
              ...
              <View>
                ...
                <Text style={styles.cardPreviewValue}>
                  {manualCardDetails?.expiryMonth &&
                  manualCardDetails?.expiryYear
                    ? `${String(manualCardDetails.expiryMonth).padStart(
                        2,
                        '0',
                      )}/${String(manualCardDetails.expiryYear).slice(-2)}`
                    : 'MM/AA'}
                </Text>
              </View>
            </View>
          ...
          <View style={styles.featureBlock}>
            ...
            <Text style={styles.blockSub}>
              Puedes pagar ahora y, si quieres, guardarla para futuros pagos.
            </Text>
            ...
            <View style={styles.saveRow}>
              <View style={{flex: 1}}>
                ...
                <Text style={styles.saveRowSub}>
                  Se guardará como método predeterminado si el pago es exitoso.
                </Text>
              </View>
        ...
        <View
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              payWithManualStripeCard({
                save: saveCard,
              })
            }
            disabled={processing || savingCard}>
            {processing || savingCard ? (
              <ActivityIndicator color="#fff" />
            ) : (
              <Text
                style={[
                  styles.primaryButtonTextWhite,
                  {
      ...
      <View style={styles.noticeBackdrop}>
        <View style={styles.noticeBox}>
          ...
          <Text style={styles.noticeMessage}>
            Para pagar con PayPal primero debes vincular tu cuenta desde la
            sección de métodos de pago en tu perfil.
          </Text>
...
const styles = StyleSheet.create({
  ...
  primaryButtonTextWhite: {color: '#ffffff', fontWeight: '800'},
  noticeBackdrop: {
    flex: 1,
