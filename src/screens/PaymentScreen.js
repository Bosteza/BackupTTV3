...
const promotePendingToLocal = async saleId => {
  ...
  try {
    ...
    const union = Array.from(
      new Set([...(localArr || []), ...pending.ids.map(String)]),
    );
    await AsyncStorage.setItem(localKey, JSON.stringify(union));
    await AsyncStorage.removeItem(pendKey);
    console.log(
      'promotePendingToLocal -> promoted pending to local for sale',
      saleId,
      union.length,
    );
  } catch (err) {
...
const mergePaidIdsLocal = async (saleId, ids = []) => {
  ...
  try {
    ...
    const union = Array.from(
      new Set([...(localArr || []), ...ids.map(String)]),
    );
    await AsyncStorage.setItem(localKey, JSON.stringify(union));
    ...
    try {
      ...
      if (rawPend) {
        ...
        if (pending && Array.isArray(pending.ids)) {
          const remaining = pending.ids
            .map(String)
            .filter(x => !new Set(ids.map(String)).has(String(x)));
...
export default function PaymentScreen() {
  ...
  const iva = Number(
    params.iva ?? params.monto_iva ?? params.ivaAmount ?? params.iva_local ?? 0,
  );
  ...
  const [userFullname, setUserFullname] = useState(
    params.user_fullname ?? params.userFullname ?? null,
  );
  const [userUsuarioAppId, setUserUsuarioAppId] = useState(
    params.user_usuario_app_id ?? params.userUsuarioAppId ?? null,
  );
  ...
  const checkGatewayAvailable = async gateway => {
    try {
      ...
      const list = Array.isArray(json.gateways)
        ? json.gateways
      ...
      return list
        ...
        .includes(String(gateway).toLowerCase());
    } catch (err) {
  ...
  const fetchStripeCredentials = async (restId, sucId) => {
    try {
      ...
      const url = `${hostBase}/api/restaurantes/${encodeURIComponent(
        restId,
      )}/sucursales/${encodeURIComponent(sucId)}/payment-gateways`;
      ...
      if (!arr || !Array.isArray(arr)) {
        console.warn(
          'fetchStripeCredentials -> unexpected response shape',
          json,
        );
        return null;
      }
      const found = arr.find(
        g =>
          String(g.gateway ?? '').toLowerCase() === 'stripe' &&
          (g.activo === undefined || !!g.activo),
      );
      if (!found) {
        console.log(
          'fetchStripeCredentials -> stripe gateway not found in list',
        );
        return null;
      }
  ...
  const openUrlRobust = async rawUrl => {
    ...
    const cleanRaw = String(rawUrl)
      ...
      .replace(/[\n\r\t]/g, '');
    try {
      await Linking.openURL(cleanRaw);
      return true;
    } catch (e) {
      console.warn('openUrlRobust1', e);
    }
    try {
      const encoded = encodeURI(cleanRaw);
      await Linking.openURL(encoded);
      return true;
    } catch (e) {
      console.warn('openUrlRobust2', e);
    }
    if (/\s/.test(cleanRaw)) {
      try {
        const noSpaces = cleanRaw.replace(/\s+/g, '');
        await Linking.openURL(noSpaces);
        return true;
      } catch (e) {
        console.warn('openUrlRobust3', e);
      }
      try {
        const encNo = encodeURI(cleanRaw.replace(/\s+/g, ''));
        await Linking.openURL(encNo);
        return true;
      } catch (e) {
        console.warn('openUrlRobust4', e);
      }
    }
    return false;
  };
  ...
  const showGatewayUnavailableModal = (gateway, message) => {
    setGatewayModalMessage(
      message ??
        `Lo sentimos — el método de pago ${gateway.toUpperCase()} no está disponible para esta sucursal.`,
    );
  ...
  const pollSplitsUntilPaid = async (
        ...
        if (res.ok) {
          ...
          if (paidSplits.length > 0) {
            ...
            try {
              ...
              console.warn('mergePaidIdsLocal error', e);
            }
    ...
    return {
      ok: false,
      reason: 'timeout',
      last: pollingRef.current.lastResult ?? null,
    };
  };

  const startCheckoutAndPoll = async ({
    gateway,
    payment_method_id: passedPaymentMethodId = undefined,
  }) => {
    if (loadingKey) return;
    if (loadingInit) {
      Alert.alert(
        'Espere',
        'Cargando datos de sesión, intente de nuevo en un momento.',
      );
      return;
    }
    if (!apiHost) {
      Alert.alert(
        'Falta API host',
        'No hay api_host configurado. Pasa api_host en route.params o ajusta API_HOST_CONST.',
      );
      return;
    }
    if (!userEmail || !userFullname) {
      Alert.alert(
        ...
        'No se encontraron datos del usuario (correo/nombre). Inicia sesión o pásalos en params.',
      );
      return;
    }
    if (!sucursal_id || !sale_id || !restaurante_id) {
      Alert.alert(
        'Faltan datos',
        'No hay sucursal_id / sale_id / restaurante_id. No es posible verificar el pago automáticamente.',
      );
      return;
    }
      ...
      if (restaurante_id) {
        ...
        if (restRes.ok) {
          ...
          const plataforma = String(
            restJson.plataforma_gestion ?? restJson.plataforma ?? '',
          ).toLowerCase();
          if (
            plataforma.includes('eposnow') ||
            plataforma.includes('epos now')
          ) {
          ...
          console.warn(
            'startCheckoutAndPoll: no se pudo consultar restaurante (http ' +
              String(restRes.status) +
              '). Usando default payment_method_id=3',
          );
          resolvedPaymentMethodId = 3;
        }
      } else {
        console.warn(
          'startCheckoutAndPoll: restaurante_id no disponible para resolver plataforma_gestion. Usando default payment_method_id=3',
        );
        resolvedPaymentMethodId = 3;
      }
    } catch (err) {
      console.warn(
        'Error consultando plataforma_gestion del restaurante:',
        err,
        '-> usando default payment_method_id=3',
      );
      resolvedPaymentMethodId = 3;
    ...
    const payment_method_id =
      typeof passedPaymentMethodId === 'number' && passedPaymentMethodId > 0
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (
          ...
          lowerMsg.includes('no configur') ||
          lowerMsg.includes('no disponible')
        ) {
          showGatewayUnavailableModal(gateway);
          setLoadingKey(null);
          return;
      ...
      const checkoutUrl =
        json?.checkout_url ?? json?.data?.checkout_url ?? null;
      const transactionId =
        json?.transaction_id ??
      ...
      console.log(
        'createTransaction -> checkout_url:',
        checkoutUrl,
        ' transaction_id:',
        transactionId,
      );
      ...
      if (!checkoutUrl) {
        Alert.alert(
          'Respuesta inválida',
          'El servidor no devolvió checkout_url.',
        );
      ...
      if (transactionId && sale_id) {
        try {
          await AsyncStorage.setItem(
            lastTransactionKeyForSale(sale_id),
      ...
      (async () => {
        pollingRef.current.stopRequested = false;
        if (transactionId) {
          const result = await pollSplitsUntilPaid(
            transactionId,
            120 * 1000,
            3000,
          );
          if (result.ok) {
            ...
            try {
              await promotePendingToLocal(sale_id);
            } catch (e) {
          ...
          console.warn(
            'No transactionId returned; no splits to poll and /pagos polling removed by request.',
          );
          return;
        }
      })();
      ...
      Alert.alert(
        'Error',
        'No se pudo conectar con el servidor de pagos. Revisa la URL y el token.',
      );
      setLoadingKey(null);
    }
  };
  ...
  useEffect(() => {
    const handler = ({url}) => {
      ...
      (async () => {
        if (!sale_id) return;
        try {
          const rawTx = await AsyncStorage.getItem(
            lastTransactionKeyForSale(sale_id),
          );
        ...
        console.warn(
          'Deep-link returned but no confirmation from splits and fallback to /pagos was removed.',
        );
      })();
    };
    ...
    try {
      if (typeof Linking.addListener === 'function') {
        ...
        console.warn(
          'Linking does not expose addListener/addEventListener on this platform.',
        );
      }
    } catch (e) {
      console.warn('Could not attach Linking listener', e);
      try {
        Linking.addEventListener && Linking.addEventListener('url', handler);
      } catch (er) {
        console.warn('Could not attach Linking listener fallback', er);
      }
    }
  ...
  const validateBeforeStripe = () => {
    ...
    if (!sucursal_id || !sale_id || !restaurante_id) {
      Alert.alert(
        'Faltan datos',
        'No se encontró sucursal_id / sale_id / restaurante_id en los datos. No se puede iniciar verificación automática.',
      );
      return false;
    }
    return true;
  ...
  const fetchCardPaymentMethods = async (restId, sucId) => {
    try {
      ...
      const url = `${hostBase}/api/restaurantes/${encodeURIComponent(
        restId,
      )}/sucursales/${encodeURIComponent(sucId)}/metodos-pago`;
      const res = await fetch(url, {
        method: 'GET',
        headers: {
          Accept: 'application/json',
          ...
          if (
            ...
            if (
              ...
              (nameRaw.includes('cred') ||
                nameRaw.includes('credito') ||
                nameRaw.includes('crédito') ||
                nameRaw.includes('visa') ||
                nameRaw.includes('mastercard') ||
                nameRaw.includes('amex'))
            ) {
              creditId = candidate;
            }
            ...
            if (
              ...
              (nameRaw.includes('deb') ||
                nameRaw.includes('debito') ||
                nameRaw.includes('débito') ||
                nameRaw.includes('debit'))
            ) {
              debitId = candidate;
            }
          }
  ...
  const onOptionPress = async opt => {
    if (opt.key === 'stripe') {
      ...
      try {
        ...
        const methods = await fetchCardPaymentMethods(
          restaurante_id,
          sucursal_id,
        );
        setLoadingKey(null);
        ...
        if (methods.creditId || methods.debitId) {
          ...
          setSelectedCardType(
            methods.creditId ? 'credit' : methods.debitId ? 'debit' : null,
          );
  ...
  const dateText = fecha_apertura
    ? new Date(fecha_apertura).toLocaleString('es-MX', {
        dateStyle: 'long',
        timeStyle: 'short',
      })
  ...
  const contentWidth = Math.min(
    Math.round(width - 32),
    Math.max(420, Math.round(width * 0.92)),
  );
  const contentPadding = clamp(Math.round(width * 0.04), 20, 8);
  ...
  async function fetchOpenpayCredentials(restId, sucId) {
    try {
      ...
      if (!arr || !Array.isArray(arr)) {
        console.warn(
          'fetchOpenpayCredentials -> unexpected response shape',
          json,
        );
        return null;
      }
      const found = arr.find(
        g =>
          String(g.gateway ?? '').toLowerCase() === 'openpay' &&
          (g.activo === undefined || !!g.activo),
      );
      if (!found) {
        console.log(
          'fetchOpenpayCredentials -> openpay gateway not found in list',
        );
        return null;
      }
      const credentials = found.credentials ?? {};
      ...
      if (!merchant || !publicKey) {
        console.warn(
          'fetchOpenpayCredentials -> missing merchant or publicKey',
          {merchant, publicKey, found},
        );
        return null;
      }
      return {
  ...
  const confirmCardSelection = async () => {
    ...
    if (!chosenId) {
      Alert.alert(
        'Selección inválida',
        'No se encontró el id del método seleccionado. Intenta nuevamente.',
      );
      return;
    }
  ...
  return (
    <SafeAreaView style={[styles.safe, {paddingTop: topSafe}]}>
      ...
      <View style={[styles.topBar, {height: headerHeight}]}>
        ...
        <Text
          style={[styles.topSmall, {fontSize: clamp(rf(1.6), 10, 14)}]}
          numberOfLines={1}
          ellipsizeMode="tail">
          {dateText}
        </Text>
      ...
      <ScrollView
            ...
            <View style={[styles.leftCol, {flex: 1}]}>
              <Image
                ...
                style={[
                  styles.tabtrackLogo,
                  {
                    width: Math.min(logoSize, 160),
                    height: Math.round(Math.min(logoSize, 160) * 0.32),
                  },
                ]}
                resizeMode="contain"
              />
              <View
                style={[
                  styles.logoWrap,
                  {marginTop: Math.max(6, hp(1)), padding: Math.max(6, wp(1))},
                ]}>
                <Image
                  source={
                    restaurantImage
                  ...
                  style={[
                    styles.restaurantImage,
                    {
                      ...
                      borderRadius: clamp(
                        Math.round(restaurantImageSize / 6),
                        8,
                        18,
                      ),
                    },
        ...
        <View
          style={[
            styles.content,
            {width: contentWidth, padding: contentPadding},
          ]}>
          <TouchableOpacity
            style={[
              styles.payButton,
            ...
            onPress={() => {
              Alert.alert(
                'Pagar',
                'Seleccione un método de pago en la lista inferior.',
              );
            }}
            ...
            <Text
              style={[
                ...
                {fontSize: clamp(rf(1.8), 14, 18)},
              ]}>
              {loadingKey === 'main' ? 'Creando pago…' : 'Pagar'}
            </Text>
          </TouchableOpacity>
              ...
              <TouchableOpacity
                ...
                style={[
                  styles.optionRow,
                  {
                    ...
                    paddingHorizontal: Math.max(6, wp(2)),
                  },
                ]}
                  ...
                  <View
                    style={[
                      styles.iconBox,
                      {
                        width: iconBoxSize,
                        height: iconBoxSize,
                        borderRadius: Math.round(iconBoxSize / 6),
                      },
                    ]}>
                    {loadingKey === opt.key ? (
                      ...
                      <Ionicons
                        name={opt.icon}
                        size={clamp(Math.round(iconBoxSize * 0.42), 16, 24)}
                        color="#0046ff"
                      />
                    )}
                  </View>
                ...
                <Ionicons
                  ...
                  size={clamp(Math.round(rf(1.8)), 14, 20)}
                  color="#999"
                />
              </TouchableOpacity>
            ))}
      ...
      {gatewayModalVisible && (
        <View style={styles.modalBackdrop}>
          <LinearGradient
            ...
            <Text
              style={[
                styles.gatewayModalTitle,
                {fontSize: clamp(rf(2.2), 16, 20)},
              ]}>
              Método no disponible
            </Text>
            <Text
              style={[
                styles.gatewayModalMessage,
                {fontSize: clamp(rf(1.6), 13, 16)},
              ]}>
              {gatewayModalMessage}
            </Text>
