...
function buildVisitFromFav(item) {
  try {
    ...
    const restaurante_id =
      item.restaurante_id ?? item.restaurante ?? item.restaurant_id ?? null;
    ...
    const visit = {
      ...
      sucursal_id:
        sucursal_id !== undefined && sucursal_id !== null
          ? String(sucursal_id)
          : null,
      restaurante_id:
        restaurante_id !== undefined && restaurante_id !== null
          ? String(restaurante_id)
          : null,

      restaurantName:
        restaurantName || (item.name ?? 'Restaurante') || 'Restaurante',
      ...
      avg_rating:
        avg_rating != null
          ? Number.isFinite(Number(avg_rating))
            ? Number(avg_rating)
            : null
          : null,
      ...
      horarios: Array.isArray(item.horarios)
        ...
        : null,
      url_whatsapp:
        item.url_whatsapp ?? item.whatsapp ?? item.whatsapp_url ?? null,
      ...
      price_range_raw:
        item.price_range_raw ?? item.price_range ?? item.price_symbol ?? null,
...
export default function FavoritesScreen({route, navigation}) {
  ...
  const flatlistPaddingBottom = Math.round(
    (insets.bottom || 0) + listPaddingBottom,
  );
  ...
  useEffect(() => {
    ...
    setDisplayed(
      favorites.filter(item =>
        ((item.name ?? item.restaurantName ?? item.restaurant ?? '') + '')
          .toLowerCase()
          .includes(q),
      ),
    );
  ...
  const removeFavorite = async id => {
    try {
      ...
      const newIds = Array.isArray(ids)
        ? ids.filter(i => String(i) !== String(id))
        : [];
      ...
      const newObjs = Array.isArray(objs)
        ? objs.filter(o => String(o.id) !== String(id))
        : [];
  ...
  const onPressNavigateToRestaurant = item => {
    try {
      ...
      navigation.navigate('Restaurant', {
        visit: {restaurantName: item?.name ?? 'Restaurante'},
        id: item?.id ?? null,
      });
    }
  };
  ...
  const renderCard = ({item}) => {
    ...
    return (
      <View
        style={[
          styles.card,
          {borderRadius: cardRadius, marginHorizontal: horizPadding / 2},
        ]}>
        <TouchableOpacity
          activeOpacity={0.85}
        ...
        <View style={styles.cardInfo}>
          <Text
            style={[styles.cardName, {fontSize: cardNameSize}]}
            numberOfLines={2}>
          ...
          <View style={styles.cardMeta}>
            <View style={styles.ratingRow}>
              ...
              <Text style={[styles.ratingStar, {fontSize: metaFontSize}]}>
                ★
              </Text>
            </View>
  ...
  return (
    <SafeAreaView style={[styles.container, {paddingTop: topSafe}]}>
      <View
        style={[
          styles.header,
          {paddingHorizontal: horizPadding, paddingVertical: headerPaddingV},
        ]}>
        <TouchableOpacity
      ...
      <View
        style={[
          styles.searchBar,
          {marginHorizontal: horizPadding, height: searchHeight},
        ]}>
        ...
        <FlatList
          ...
          contentContainerStyle={[
            styles.list,
            {
              ...
              paddingHorizontal: horizPadding / 2,
            },
          ]}
        />
      )}
