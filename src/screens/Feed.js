...
const parsePriceRange = raw => {
  ...
  try {
    ...
    if (numbers && numbers.length > 0) {
      ...
      if (nnums.length === 1)
        return {min: nnums[0], max: nnums[0], symbol: null};
      return {
        min: Math.min(nnums[0], nnums[1]),
        max: Math.max(nnums[0], nnums[1]),
        symbol: null,
      };
    }
...
const fetchSurveyAvgForSucursal = async sucursalId => {
  ...
  try {
    ...
    if (!json) return 0.0;
    const resumen = Array.isArray(json.resumen_por_sucursal)
      ? json.resumen_por_sucursal
      : [];
    const block =
      resumen.find(r => String(r.sucursal_id) === String(sucursalId)) ||
      resumen[0] ||
      null;
    if (!block || !Array.isArray(block.preguntas)) return 0.0;
    const starQuestions = block.preguntas.filter(
      p =>
        (p.tipo || '').toString().toUpperCase() === 'ESTRELLAS' &&
        p.promedio != null,
    );
    if (!starQuestions.length) return 0.0;
    const sum = starQuestions.reduce(
      (acc, p) => acc + (Number(p.promedio) || 0),
      0,
    );
    ...
    while (page <= maxPages) {
      ...
      if (!res.ok) {
        if (page === 1) {
          ...
          else if (Array.isArray(json2.restaurantes))
            itemsFallback = json2.restaurantes;
...
export default function RestaurantsScreen() {
    ...
    (async () => {
      try {
        ...
        const mergedCities = Array.from(
          new Set(['Todos', ...sampleCities, ...uniqueCitiesFromApi]),
        );
  ...
  const applyFilters = () => {
    ...
    const filtered = restaurants.filter(item => {
      ...
      if (cuisine && cuisine !== 'todos') {
        ...
        const cuisineField = cuisineFieldParts
          .join(',')
          .toString()
          .toLowerCase();
        const needle = cuisine.toString().toLowerCase();
        matchCuisine =
          cuisineField.includes(needle) ||
      ...
      if (
        item.price_min != null &&
        item.price_max != null &&
        Number.isFinite(Number(item.price_min)) &&
        Number.isFinite(Number(item.price_max))
      ) {
        pMin = Number(item.price_min);
        pMax = Number(item.price_max);
      } else if (
        Number.isFinite(Number(item.avg_price)) &&
  ...
  const openFavoritesFromHeaderGuarded = async () => {
    if (await isGuestMode()) {
      ...
      navigation.navigate('Perfil'); // or 'Experiences' or 'QR'
      return;
    }
  ...
  return (
    <SafeAreaView style={[styles.container, {paddingTop: topSafe}]}>
      <View
        style={[
          styles.header,
          {paddingHorizontal: horizPad, height: headerHeight},
        ]}>
        ...
        <TextInput
          style={[
            styles.searchInput,
            {height: searchHeight, marginHorizontal: 8},
          ]}
        ...
        <TouchableOpacity
          onPress={openFavoritesFromHeaderGuarded}
      ...
      <FlatList
        ...
        contentContainerStyle={[
          styles.list,
          {paddingBottom: Math.max(16, hp(3)) + bottomSafe},
        ]}
        renderItem={({item}) => (
          <RestaurantCard
            restaurant={item}
            ...
            <View
              ...
              <ScrollView
                ...
                <Text style={[styles.modalLabel, {marginTop: 14}]}>
                  Radio de búsqueda ({searchRadiusKm} km)
                </Text>
                    ...
                    {sampleTypes.map(t => (
                      <TouchableOpacity
                        ...
                        <Text
                          style={{
                            color: '#333',
                            fontWeight: cuisine === t.id ? '800' : '600',
                          }}>
                          {t.label}
                        </Text>
                      </TouchableOpacity>
                ...
                <View style={{marginTop: 8}}>
                  ...
                  <Slider
                    ...
                    maximumTrackTintColor="#ddd"
                  />
                  <Text style={{color: '#444', marginTop: 12, marginBottom: 6}}>
                    Máx: {maxPrice} MXN
                  </Text>
                  <Slider
                    style={styles.slider}
...
function RestaurantCard({
  ...
  const shouldShowSurveyRating = !!(
    ...
    (restaurant.raw.mostrar_rating === true ||
      (restaurant.raw.mostrar_rating &&
        String(restaurant.raw.mostrar_rating).toLowerCase() === 'true') ||
      restaurant.mostrar_rating === true ||
      (restaurant.mostrar_rating &&
        String(restaurant.mostrar_rating).toLowerCase() === 'true'))
  );
    ...
    <View style={[styles.card, {borderRadius: cardRadius}]}>
      ...
      <TouchableOpacity
        ...
        <View style={{justifyContent: 'center', alignItems: 'flex-end'}}>
          {shouldShowSurveyRating &&
          restaurant.avg_rating != null &&
          !Number.isNaN(Number(restaurant.avg_rating)) ? (
...
const styles = StyleSheet.create({
  ...
  picker: {height: 180, width: '100%', color: '#000'},
