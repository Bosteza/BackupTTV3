...
export default function OneExhibicion() {
  ...
  const normalizeItems = rawItems => {
    ...
    return rawItems.map((it, idx) => {
      ...
      const paidPartial = !!(
        it.paidPartial === true || it.paid_partial === true
      );
      const paidAmount =
        Number(
          it.paidAmount ??
  ...
  const items = useMemo(
    () => normalizeItems(params.items ?? incomingItems ?? []),
    [params.items, incomingItems],
  );
  ...
  const people =
    typeof total_comensales === 'number' && total_comensales > 0
      ? total_comensales
      : 1;
  ...
  const renderItem = ({item}) => {
    ...
    const remainingForItem = item.paid
      ? 0
      : item.paidPartial
      ? +(line - Number(item.paidAmount || 0)).toFixed(2)
      : line;
    ...
    return (
      <View style={styles.itemRow}>
        <View style={{flex: 1}}>
          ...
          {item.paid ? (
            <Text style={styles.metaPaid}>
              Pagado • {formatMoney(item.paidAmount)} {moneda}
            </Text>
          ) : item.paidPartial ? (
            <Text style={styles.metaPaid}>
              Parcial: {formatMoney(item.paidAmount)} pagado — Falta{' '}
              {formatMoney(remainingForItem)} {moneda}
            </Text>
          ) : null}
        </View>
        ...
        <View style={{alignItems: 'flex-end'}}>
          ...
          {!item.paid && item.paidPartial ? (
            <Text style={styles.metaPending}>
              Pendiente: {formatMoney(remainingForItem)} {moneda}
            </Text>
          ) : null}
        </View>
      </View>
  ...
  const handlePay = () => {
    ...
    const tipAmount = tipObj
      ? Number(tipObj.tipAmount || tipObj.tip_amount || 0)
      : 0;
    const tipPercent = tipObj
      ? Number(tipObj.percent || tipObj.tipPercent || 0)
      : 0;
    const totalWithTip = tipObj
      ? Number(
          tipObj.totalWithTip ||
            ...
            <>
              <View style={styles.row}>
                <Text style={styles.label}>
                  Propina ({tipApplied.percent ?? tipApplied.tipPercent ?? 0}%)
                </Text>
                <Text style={styles.value}>
                  {formatMoney(
                    tipApplied.tipAmount ?? tipApplied.tip_amount ?? 0,
                  )}{' '}
                  MXN
                </Text>
              </View>
              <View style={[styles.row, {marginTop: 6}]}>
                ...
                <Text
                  style={[
                    styles.value,
                    styles.boldValue,
                    {fontSize: styles.totalNumber.fontSize * 0.65},
                  ]}>
                  {formatMoney(
                    tipApplied.totalWithTip ??
                      tipApplied.total_with_tip ??
                      shownTotal,
                  )}{' '}
                  MXN
                </Text>
...
function makeStyles({width, height, wp, hp, rf, clamp, insets}) {
  const safeTop = Math.max(
    insets?.top ?? 0,
    Platform.OS === 'android' ? StatusBar.currentHeight || 8 : insets?.top ?? 8,
  );
  ...
  const restaurantImageSize = Math.round(
    clamp(rf(6.5), 48, Math.max(64, Math.min(96, width * 0.18))),
  );
  const contentMaxWidth = Math.min(width - 32, 720);
  const totalNumberSize = Math.round(
    clamp(rf(8.5), 20, Math.max(28, Math.min(42, width * 0.09))),
  );
  ...
  return StyleSheet.create({
    ...
    backArrow: {
      fontSize: Math.round(clamp(rf(8), 24, 38)),
      color: '#0b58ff',
      marginLeft: 2,
    },
    title: {
      fontSize: Math.round(clamp(rf(4.5), 14, 18)),
      fontWeight: '800',
      color: '#0b58ff',
    },
    ...
    restaurantImage: {
      ...
      borderRadius: Math.round(Math.min(16, restaurantImageSize * 0.18)),
      backgroundColor: '#fff',
    },
    ...
    rightCol: {
      ...
      paddingTop: 2,
      maxWidth: Math.round(Math.min(width * 0.45, 320)),
    ...
    totalNumber: {
      ...
      fontWeight: '900',
      letterSpacing: 0.6,
      lineHeight: Math.round(totalNumberSize * 1.05),
    },
    totalCurrency: {
      color: '#fff',
    ...
    thanksText: {
      color: '#fff',
      fontWeight: '700',
      fontSize: Math.round(clamp(rf(3.4), 12, 14)),
    },
    thanksSub: {
      color: 'rgba(255,255,255,0.95)',
      fontSize: Math.round(clamp(rf(3), 10, 13)),
      marginTop: 6,
    },
    ...
    itemPrice: {
      width: Math.round(Math.min(140, contentMaxWidth * 0.34)),
      textAlign: 'right',
      color: '#111827',
      fontSize: Math.round(clamp(rf(3.6), 12, 16)),
    },
    ...
    metaPaid: {
      color: '#0b8f56',
      fontWeight: '700',
      fontSize: Math.round(clamp(rf(2.8), 11, 13)),
      marginTop: 6,
    },
    metaPending: {
      fontSize: Math.round(clamp(rf(2.8), 11, 13)),
    ...
    separator: {
      ...
      marginVertical: Math.round(hp(1.2)),
    },
    ...
    boldValue: {
      fontWeight: '900',
      fontSize: Math.round(clamp(rf(4.5), 14, 20)),
    },
    ...
    ghostButtonText: {
      color: '#444',
      fontWeight: '700',
      fontSize: Math.round(clamp(rf(3.6), 12, 15)),
    },
