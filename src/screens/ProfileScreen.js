...
export default function ProfileScreen({navigation}) {
  ...
  const titleFont = clamp(rf(25), 18, 26);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        Toast.show('Error al cargar usuario', {
    ...
    try {
      ...
      for (const venta of ventas) {
        const ventaSaleId =
          ...
          venta?.saleId ??
          null;
        const ventaRestaurantId =
          venta?.restaurante_id ??
          venta?.restaurant_id ??
          venta?.restauranteId ??
          venta?.restaurantId ??
          null;
        const ventaBranchId =
          ...
          venta?.branchId ??
          null;
        const ventaRestaurantName =
          venta?.nombre_restaurante ??
          venta?.restaurantName ??
          venta?.restaurante ??
          '';
        const ventaBranchName =
          venta?.nombre_sucursal ?? venta?.branchName ?? venta?.sucursal ?? '';
  ...
  async function enrichVisitWithBranchLogo(visit, fallbackMeta = {}) {
    try {
      ...
      const restauranteId =
        ...
        visit?.restauranteId ??
        visit?.restaurantId ??
        fallbackMeta?.restauranteId ??
        fallbackMeta?.restaurantId ??
        null;
      const sucursalId =
        visit?.sucursal_id ??
        visit?.branch_id ??
        visit?.sucursalId ??
        visit?.branchId ??
        fallbackMeta?.sucursalId ??
        fallbackMeta?.branchId ??
        null;

      let restaurantName =
        visit?.restaurantName ??
        visit?.nombre_restaurante ??
        fallbackMeta?.restaurantName ??
        '';
      let branchName =
        visit?.branchName ??
        visit?.nombre_sucursal ??
        fallbackMeta?.branchName ??
        '';
      let restaurantImage =
        visit?.restaurantImage ??
        visit?.logo_url ??
        visit?.imagen_logo_url ??
        null;
      ...
      if (restauranteId) {
        ...
        try {
          ...
          console.warn(
            'enrichVisitWithBranchLogo restaurant info cache read error',
            e,
          );
        }
        ...
        try {
          ...
          console.warn(
            'enrichVisitWithBranchLogo branches cache read error',
            e,
          );
        }
        ...
        if (!infoData) {
          try {
            const endpoint = `${API_URL}/api/restaurantes/${encodeURIComponent(
              restauranteId,
            )}`;
            ...
            if (res.ok) {
              ...
              try {
                await AsyncStorage.setItem(
                  infoCacheKey,
                  JSON.stringify({savedAt: Date.now(), data: infoData}),
                );
              } catch (e) {
                console.warn(
                  'enrichVisitWithBranchLogo restaurant info cache save error',
                  e,
                );
              }
            }
            ...
            console.warn(
              'enrichVisitWithBranchLogo restaurant info fetch error',
              e,
            );
          }
        }
          ...
          try {
            ...
            if (res.ok) {
              ...
              try {
                await AsyncStorage.setItem(
                  branchesCacheKey,
                  JSON.stringify({savedAt: Date.now(), data: branchesData}),
                );
              } catch (e) {
                console.warn(
                  'enrichVisitWithBranchLogo branches cache save error',
                  e,
                );
              }
            }
        ...
        if (infoData && typeof infoData === 'object') {
          restaurantName =
            cleanName(infoData?.nombre) ||
            cleanName(infoData?.name) ||
            restaurantName;
        }

        const sucursales = Array.isArray(branchesData?.sucursales)
          ? branchesData.sucursales
          : [];
        const matched =
          sucursales.find(s => String(s.id) === String(sucursalId)) ||
          sucursales[0] ||
          null;
      ...
      const finalRestaurantImage = restaurantImage
        ? getCacheBustedUrl(String(restaurantImage))
        : visit.restaurantImage || null;
      const finalBannerImage = bannerImage
        ? getCacheBustedUrl(String(bannerImage))
        : visit.bannerImage || null;
      ...
      return {
        ...
        nombre_restaurante:
          cleanName(restaurantName) ||
          cleanName(visit?.nombre_restaurante) ||
          cleanName(visit?.restaurantName) ||
          '',
        restaurantName:
          cleanName(restaurantName) ||
          cleanName(visit?.restaurantName) ||
        ...
        if (found) {
          const toNav = await enrichVisitWithBranchLogo(found, {
            ...
            restaurantName:
              ...
              data?.restaurantName ??
              found?.nombre_restaurante ??
              found?.restaurantName ??
              '',
            branchName:
              data?.nombre_sucursal ??
              data?.branchName ??
              found?.nombre_sucursal ??
              found?.branchName ??
              '',
          });
  ...
  const loadProfileFromApi = async () => {
    try {
      ...
      const endpoint = `${API_URL}/api/mobileapp/usuarios?mail=${encodeURIComponent(
        email,
      )}&presign_ttl=30`;
      const headers = getAuthHeaders();
  ...
  const onSelectImage = async () => {
    try {
      ...
      if (!asset) {
        Toast.show('No se seleccionó imagen', {
          duration: Toast.durations.SHORT,
        });
        setShowAvatarOptions(false);
        return;
      ...
      Toast.show('Error al seleccionar imagen', {
        duration: Toast.durations.SHORT,
      });
      setShowAvatarOptions(false);
    }
  ...
  const uploadProfilePhoto = async asset => {
    try {
      ...
      if (!presignRes.ok) {
        ...
        Toast.show('No se pudo iniciar la subida', {
          duration: Toast.durations.SHORT,
        });
        setUploading(false);
        return;
      ...
      if (!uploadUrl || !uploadKey) {
        Toast.show('Respuesta inválida para presign', {
          duration: Toast.durations.SHORT,
        });
        setUploading(false);
        return;
      ...
      const commitUrl = `${API_URL}/api/mobileapp/usuarios/${encodeURIComponent(
        uid,
      )}/foto/commit`;
      ...
      if (!commitRes.ok) {
        ...
        Toast.show('No se pudo confirmar la imagen', {
          duration: Toast.durations.SHORT,
        });
        setUploading(false);
        return;
  ...
  const removeProfilePhoto = async () => {
    try {
      ...
      const deleteUrl = `${API_URL}/api/mobileapp/usuarios/${encodeURIComponent(
        uid,
      )}/foto`;
      const headers = getAuthHeaders();
      ...
      try {
        ...
        if (!res.ok) {
          ...
          Toast.show('No se pudo eliminar la foto en el servidor', {
            duration: Toast.durations.SHORT,
          });
        } else {
          Toast.show('Foto de perfil eliminada', {
            duration: Toast.durations.SHORT,
          });
        }
      } catch (err) {
        console.warn('Error calling DELETE foto endpoint', err);
        Toast.show('Error al eliminar la foto en el servidor', {
          duration: Toast.durations.SHORT,
        });
      }
  ...
  const handleLogout = async () => {
    try {
      ...
      try {
        if (email) {
          ...
          arr.unshift({
            ...
            savedAt: Date.now(),
            //  email,
            //fullname: fullnameCached || '', // NO BORRAR JAMÁS TAMPOCO
          });
      ...
      try {
        navigation.reset({
          index: 0,
          routes: [{name: 'Recent'}], //NO CAMBIAR JAMÁS (Login)
        });
      } catch (e) {
        console.warn(
    ...
    <SafeAreaView style={[styles.container, {paddingTop: topSafe}]}>
        ...
        <View style={styles.modalOverlay}>
          <View
            style={[
              ...
              {width: logoutModalWidth, padding: basePadding},
            ]}>
            <Text
              style={[styles.logoutTitle, {fontSize: clamp(rf(4.4), 18, 22)}]}>
              Cerrar sesión
            </Text>
            <Text
              style={[
            ...
            <View
              style={[
                ...
                {marginTop: Math.round(basePadding / 2)},
              ]}>
              <TouchableOpacity
                style={[
                  styles.cancelButton,
                  {paddingVertical: clamp(Math.round(hp(1.6)), 8, 14)},
                ]}
                onPress={() => setShowLogoutModal(false)}>
                <Text
                  style={[
                    styles.cancelText,
                    {fontSize: clamp(rf(3.4), 13, 16)},
                  ]}>
                  Cancelar
                </Text>
              </TouchableOpacity>
      ...
      <Modal
        ...
        <View style={styles.modalOverlay}>
          <View
            style={[
              styles.avatarModal,
              {width: Math.min(modalWidth * 0.86, 520)},
            ]}>
            <Text
              style={[
                styles.avatarModalTitle,
                {fontSize: clamp(rf(3.8), 15, 18)},
              ]}>
            ...
            <TouchableOpacity
              ...
              <Text
                style={[
                  styles.avatarModalBtnText,
                  {color: '#444', fontSize: clamp(rf(3.6), 13, 16)},
                ]}>
                Eliminar foto
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              <Text
                style={[
                  styles.avatarModalBtnText,
                  {color: '#333', fontSize: clamp(rf(3.6), 13, 16)},
                ]}>
                Cancelar
              </Text>
            </TouchableOpacity>
      ...
      <ScrollView
        contentContainerStyle={[
          styles.scroll,
          {paddingBottom: Math.max(24, hp(4), bottomSafe + 8)},
        ]}>
        <View
          style={[
            styles.header,
        ...
        <View
          style={[styles.sectionDivider, {marginHorizontal: basePadding}]}
        />
        ...
        <View style={[styles.profileSection, {paddingHorizontal: basePadding}]}>
          <View
            style={{
              width: avatarSize,
              height: avatarSize,
              marginRight: Math.round(basePadding * 0.6),
              position: 'relative',
            }}>
            <View
              style={{
            ...
            <TouchableOpacity
              ...
              {uploading ? (
                ...
                <Ionicons
                  name="pencil"
                  size={Math.max(12, Math.round(avatarSize * 0.18))}
                  color="#fff"
                />
              )}
            </TouchableOpacity>
          ...
          <View style={{flex: 1, justifyContent: 'center'}}>
            <Text style={[styles.greeting, {fontSize: smallText}]}>
              Hola :)
            </Text>
            <Text
              style={[styles.username, {fontSize: clamp(rf(4.2), 16, 22)}]}
              numberOfLines={2}>
              {username || 'Usuario'}
            </Text>
          </View>
        ...
        <View
          style={[
            ...
            {marginTop: 8, marginHorizontal: basePadding},
          ]}
        />
        ...
        <Text
          style={[
            styles.sectionTitle,
            {fontSize: sectionTitleFont, paddingHorizontal: basePadding},
          ]}>
          Configuración
        </Text>

        <View
          style={[styles.optionsContainer, {paddingHorizontal: basePadding}]}>
          <Option
            icon="person-outline"
