...
const VALIDATE_BASE =
  'https://api.tab-track.com/api/mobileapp/usuarios/verification-codes/validate';
...
export default function VerificationScreen({navigation, route}) {
  ...
  const logoSize = clamp(
    Math.round(Math.min(width * 0.56, height * 0.32)),
    rf(72),
    360,
  );
  const otpSize = clamp(
    Math.round(
      Math.min(
        80,
        Math.max(44, (width - horizontalPadding * 2 - NUM * 8) / NUM),
      ),
    ),
    40,
    88,
  ...
  const verifyBtnWidth = Math.round(
    Math.min(520, Math.max(width * 0.62, width * 0.82)),
  );
  const bottomSafe = insets.bottom || 16;

  // key for storing resend count per email
  const resendKeyFor = e =>
    `verification_resend_count_${String(e || '').toLowerCase()}`;
    ...
    (async () => {
      ...
      (async () => {
        try {
          ...
          console.warn(
            'VerificationScreen: error reading email from storage',
            e,
          );
        }
      })();
    ...
    try {
      ...
      if (!e) {
        Alert.alert(
          ...
          'No se pudo determinar el correo. Vuelve al registro o intenta ingresar el correo manualmente.',
        );
        return;
      }
  ...
  const onVerify = async () => {
    ...
    try {
      ...
      if (!e) {
        Alert.alert(
          'Email no disponible',
          'No pude localizar el correo asociado. Vuelve al registro o intenta nuevamente.',
        );
      ...
      // llamar validate
      const url = `${VALIDATE_BASE}?email=${encodeURIComponent(
        e,
  ...
  const renderResendInfo = () => {
    ...
    if (resendLimitReached) {
      return (
        <Text style={[styles.resendNote, {fontSize: helperFont}]}>
          Límite de reenvíos alcanzado (5).
        </Text>
      );
    }
    return (
      <Text style={[styles.resendNote, {fontSize: helperFont}]}>
        Reenvíos disponibles: {left}
      </Text>
    );
  };
        ...
        <KeyboardAvoidingView
          ...
          <View style={[styles.logoWrap, {marginTop: Math.round(rf(6))}]}>
            <Image
              ...
              style={{
                width: logoSize,
                height: Math.round(logoSize * 0.48),
                resizeMode: 'contain',
              }}
            />
          </View>
          ...
          <View style={{marginTop: Math.round(rf(8))}}>
            <Text style={[styles.emailText, {fontSize: helperFont}]}>
              {email ? `Código enviado a: ${email}` : 'No se detectó correo'}
            </Text>
          </View>
          ...
          <Text
            style={[
              ...
              {
                fontSize: helperFont,
                marginTop: Math.round(rf(14)),
                paddingHorizontal: Math.round(rf(6)),
              },
            ]}>
          ...
          {verificationError ? (
            <View
              style={{marginTop: Math.round(rf(12)), paddingHorizontal: 12}}>
              <Text
                style={{
                  color: '#ef4444',
                  textAlign: 'center',
          ...
          <View
            ...
            <TouchableOpacity
              ...
              style={[
                ...
                {
                  minWidth: Math.max(120, Math.round(width * 0.34)),
              ...
              {loadingResend ? (
                ...
                <Text
                  style={[
                    ...
                    {fontSize: helperFont, fontWeight: '800'},
                  ]}>
                  Reenviar código
                </Text>
              )}
            ...
            <TouchableOpacity
              onPress={async () => {
                Alert.alert(
                  'Ayuda',
                  'Si no llega el correo, revisa tu carpeta de spam o intenta reenviar el código.',
                );
              }}
              style={[
                styles.spamInfo,
                {
                  paddingHorizontal: Math.max(10, Math.round(width * 0.03)),
                  paddingVertical: Math.max(8, Math.round(rf(8))),
                  marginTop: 0,
                },
              ]}>
              <Text
                style={[
                  styles.spamText,
                  {fontSize: Math.max(12, helperFont * 0.95)},
                ]}>
          ...
          <TouchableOpacity
            style={[
              styles.verifyBtn,
              {
                width: verifyBtnWidth,
                paddingVertical: Math.max(12, Math.round(rf(12))),
                marginBottom: Math.max(24, bottomSafe),
              },
