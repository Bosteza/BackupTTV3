...
export default function GPSScreen() {
  ...
  const pushLocation = (
    ...
    const itemEnvironment = normalizeEnvironment(
      obj.environment ?? obj.raw?.environment ?? fallbackEnvironment ?? '',
    );
    ...
    (async () => {
      ...
      Geolocation.getCurrentPosition(
        async position => {
          ...
          const rawEnvironment = await AsyncStorage.getItem(
            USER_ENVIRONMENT_KEY,
          );
          const normalizedEnvironment = normalizeEnvironment(rawEnvironment);

          warn('GPSScreen environment loaded from AsyncStorage:', {
            rawEnvironment,
          ...
          try {
            ...
            const restaurantsFilteredByEnvironment = restaurants.filter(
              item => {
                ...
                if (!okEnv) {
                  warn('restaurant discarded by environment:', {
                    id: item?.id,
            ...
            warn(
              'restaurants after environment filter:',
              restaurantsFilteredByEnvironment.length,
            );
            ...
            const restaurantsNeedingFetch =
              restaurantsFilteredByEnvironment.filter(r => {
                const hasSuc =
            ...
            if (restaurantsNeedingFetch.length > 0) {
              try {
                ...
                results.forEach(resItem => {
                  ...
                  const parentEnv =
                    resItem.parentEnv ||
                    normalizeEnvironment(resItem.restaurant?.environment ?? '');
                  if (Array.isArray(resItem.branches)) {
                    resItem.branches.forEach(b =>
                      pushLocation(locations, b, parentName, parentEnv),
                    );
                  }
                });
              } catch (e) {
            ...
            const filteredLocations = locations.filter(loc => {
              if (!normalizedEnvironment) return true;
              const locEnv = normalizeEnvironment(
                loc?.environment ?? loc?.raw?.environment ?? '',
              );
              const okEnv = locEnv === normalizedEnvironment;
              if (!okEnv) {
                warn('location discarded by environment:', {
                  id: loc?.id,
                  name: loc?.name,
                  locEnv,
                  expected: normalizedEnvironment,
            ...
            if (filteredLocations.length === 0) {
              if (normalizedEnvironment) {
                Alert.alert(
                  'Sin resultados',
                  `No se encontraron sucursales para el environment "${normalizedEnvironment}". Revisa la API o el valor guardado en AsyncStorage.`,
                );
              } else {
                Alert.alert(
                  'Sin pines',
                  'No se encontraron sucursales con coordenadas. Revisa la API y que las coordenadas no sean 0,0.',
                );
              }
            }
    ...
    try {
      ...
      if (payload.action === 'viewProfile') {
        ...
        const restaurantName =
          branchObj?.restaurantName ?? branchObj?.raw?.nombre ?? '';
