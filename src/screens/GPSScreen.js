import React, {useEffect, useState, useRef} from 'react';

import {
  View,
  ActivityIndicator,
  StyleSheet,
  Alert,
  Platform,
  Linking,
  Text,
  Dimensions,
} from 'react-native';
import {WebView} from 'react-native-webview';
import Geolocation from 'react-native-geolocation-service';
import {check, request, PERMISSIONS, RESULTS} from 'react-native-permissions';
import {useNavigation, useRoute} from '@react-navigation/native';
import AsyncStorage from '@react-native-async-storage/async-storage';

const {width} = Dimensions.get('window');

let API_URL = 'https://api.tab-track.com/api/restaurantes';
const API_TOKEN =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJmcmVzaCI6ZmFsc2UsImlhdCI6MTc2NzM4MjQyNiwianRpIjoiODQyODVmZmUtZDVjYi00OGUxLTk1MDItMmY3NWY2NDI2NmE1IiwidHlwZSI6ImFjY2VzcyIsInN1YiI6IjMiLCJuYmYiOjE3NjczODI0MjYsImV4cCI6MTc2OTk3NDQyNiwicm9sIjoiRWRpdG9yIn0.tx84js9-CPGmjLKVPtPeVhVMsQiRtCeNcfw4J4Q2hyc';
const MARKER_BASE64 =
  'data:image/png;base64,';
const INCLUDE_ZERO_COORDINATES = false;

const warn = (..._args) => {};

async function hasLocationPermission() {
  if (Platform.OS === 'android') {
    const status = await request(PERMISSIONS.ANDROID.ACCESS_FINE_LOCATION);
    return status === RESULTS.GRANTED;
  } else {
    const status = await check(PERMISSIONS.IOS.LOCATION_WHEN_IN_USE);
    if (status === RESULTS.GRANTED) return true;
    const res = await request(PERMISSIONS.IOS.LOCATION_WHEN_IN_USE);
    return res === RESULTS.GRANTED;
  }
}

function makeAbsoluteUrl(url) {
  if (!url) return null;
  try {
    new URL(url);
    return url;
  } catch (e) {
    try {
      if (typeof API_URL === 'string' && API_URL.trim()) {
        const u = new URL(API_URL);
        return u.origin + '/' + String(url).replace(/^\/+/, '');
      }
    } catch (e2) {
      return url;
    }
    return url;
  }
}

function generateMapHtml(userLat, userLng, locations, markerSrc, markerCount) {
  const safe = JSON.stringify(locations).replace(/</g, '\\u003c'); // escapar <
  const fallbackImg = markerSrc || '';

  return `<!doctype html>
<html>
<head>
<meta charset="utf-8" />
<meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no"/>
<link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css"/>
<style>
  html,body,#map{height:100%;margin:0;padding:0}
    :root{
  --safe-top: env(safe-area-inset-top, 0px);
  --ui-top: calc(50px + var(--safe-top)); /* <-- this is the one number you tune */
}

/* Keep Leaflet control in the same corner, just push it down */
.leaflet-top .leaflet-control-zoom{
  margin-top: var(--ui-top) !important;
}

  .marker-badge{
    position:absolute;  top: var(--ui-top);right:8px;z-index:9999;background:rgba(0,0,0,0.6);color:#fff;padding:6px 10px;border-radius:12px;font-weight:700;font-size:13px;
  }
  .custom-popup{width:240px;max-width:60vw;background:#fff;border-radius:10px;padding:8px;box-shadow:0 6px 18px rgba(0,0,0,0.12);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Arial;font-size:13px;color:#222;overflow:hidden;}
  .custom-row{display:flex;align-items:flex-start;margin-bottom:6px}
  .custom-thumb{width:60px;height:60px;border-radius:8px;margin-right:8px;object-fit:cover;flex-shrink:0;background:#f3f4f6;}
  .custom-info{flex:1;min-width:0}
  .custom-name{font-weight:700;font-size:14px;margin-bottom:4px;color:#111}
  .custom-desc{font-size:12px;color:#555;line-height:14px;overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical}
  .star-row{margin:6px 0}
  .star{font-size:13px;margin-right:2px}
  .btn-row{display:flex;gap:8px;margin-top:8px}
  .btn{display:inline-block;padding:8px;border-radius:8px;text-align:center;text-decoration:none;font-weight:700;cursor:pointer;border:none;font-size:13px;}
  .btn-primary{background:#0066FF;color:#fff;flex:1}
  .btn-ghost{background:transparent;color:#0066FF;border:1px solid rgba(0,102,255,0.14);flex:1}
</style>
</head>
<body>
  <div id="map"></div>
  <div class="marker-badge">Pines: ${markerCount}</div>
  <div id="loading" style="position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);color:#666">Cargando mapa…</div>

    <script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
  <script>
    (function(){
      try {
        var locations = ${safe};
        console.log('WEBVIEW locations length:', locations.length);

        // ✅ Crear el mapa UNA sola vez
        var map = L.map('map').setView([${userLat}, ${userLng}], 13);
        L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png', {
          attribution: '&copy; OpenStreetMap &copy; CartoDB',
          maxZoom: 19
        }).addTo(map);

        var loading = document.getElementById('loading');
        if (loading && loading.parentNode) loading.parentNode.removeChild(loading);

        L.circleMarker([${userLat}, ${userLng}], {
          radius: 7,
          color: '#007AFF',
          fillColor: '#007AFF',
          fillOpacity: 0.9
        }).addTo(map).bindPopup('Tú estás aquí');

        // ---- Bridge helpers ----
        window._openBranch = function(i){
          try {
            var loc = locations[i];
            var payload = { action: 'viewProfile', branch: loc };
            if (window.ReactNativeWebView && window.ReactNativeWebView.postMessage) {
              window.ReactNativeWebView.postMessage(JSON.stringify(payload));
            } else {
              console.log('RN bridge not available (viewProfile)', payload);
            }
          } catch(e){ console.error('openBranch error', e); }
        };

        window._navigateTo = function(i){
          try {
            var loc = locations[i];
            var lat = Number(loc.latitud || loc.latitude || loc.lat || 0);
            var lng = Number(loc.longitud || loc.longitude || loc.lng || 0);
            var payload = { action: 'navigate', lat: lat, lng: lng };
            if (window.ReactNativeWebView && window.ReactNativeWebView.postMessage) {
              window.ReactNativeWebView.postMessage(JSON.stringify(payload));
            } else {
              console.log('RN bridge not available (navigate)', payload);
            }
          } catch(e){ console.error('navigateTo error', e); }
        };

        function makePopupHtml(idx, loc) {
          var title = loc.nombre || loc.name || 'Sin nombre';
          var desc = loc.descripcion || loc.short_description || loc.direccion || loc.description || '';
          var img = loc.imagen_logo_url || loc.image || loc.logo || '${fallbackImg}';
          var r = Number(loc.rating || loc.avg_rating || 0);
          var stars = '';
          for (var i=0;i<5;i++){
            stars += '<span class="star" style="color:' + (i < r ? '#FFD700' : '#DDD') + '">★</span>';
          }

          var html =
            '<div class="custom-popup">' +
              '<div class="custom-row">' +
                (img ? '<img class="custom-thumb" src="'+ img +'" onerror="this.style.display=\\'none\\'"/>' : '') +
                '<div class="custom-info">' +
                  '<div class="custom-name">' + (title) + '</div>' +
                  '<div class="custom-desc">' + (desc||'') + '</div>' +
                '</div>' +
              '</div>' +
              '<div class="star-row">' + stars + '</div>' +
              '<div class="btn-row">' +
                '<button class="btn btn-ghost" onclick="_openBranch(' + idx + ');">Ver perfil</button>' +
                '<button class="btn btn-primary" onclick="_navigateTo(' + idx + ');">Ir</button>' +
              '</div>' +
            '</div>';
          return html;
        }

        var allMarkerLayers = [];
        locations.forEach(function(loc, idx){
          try {
            var lat = Number(loc.latitud || loc.latitude || loc.lat || 0);
            var lng = Number(loc.longitud || loc.longitude || loc.lng || 0);
            if (!isFinite(lat) || !isFinite(lng)) return;
            if (lat === 0 && lng === 0) return;

            var iconOptions;
            if ('${fallbackImg}' && '${fallbackImg}'.length > 10) {
              iconOptions = { iconUrl: '${fallbackImg}', iconSize: [32,32], iconAnchor: [16,32] };
            } else {
              iconOptions = { iconUrl: 'https://unpkg.com/leaflet@1.9.4/dist/images/marker-icon.png', iconSize: [25,41], iconAnchor: [12,41] };
            }
            var icon = L.icon(iconOptions);
            var marker = L.marker([lat, lng], { icon: icon }).addTo(map);
            marker.bindPopup(makePopupHtml(idx, loc), { closeButton: false, offset: [0,-36] });
            allMarkerLayers.push(marker);
          } catch(e){
            console.error('marker error', e);
          }
        });

        if (allMarkerLayers.length > 0) {
          try {
            var group = L.featureGroup(allMarkerLayers);
            map.fitBounds(group.getBounds(), { padding: [40,40] });
          } catch(e) { console.warn('fitBounds failed', e); }
        }

        //Recentrar en el usuario
        map.setView([${userLat}, ${userLng}], 13);

      } catch(err) {
        console.error('map error', err);
      }
    })();
  </script>

</body>
</html>`;
}

/* COMPONENTE GPSScreen */
export default function GPSScreen() {
  const navigation = useNavigation();
  const route = useRoute();
  const webviewRef = useRef(null);

  const [mapHtml, setMapHtml] = useState(null);
  const [loading, setLoading] = useState(true);
  const mountedRef = useRef(true);

  useEffect(() => {
    mountedRef.current = true;
    return () => {
      mountedRef.current = false;
    };
  }, []);

  const getAuthToken = async () => {
    if (
      typeof API_TOKEN === 'string' &&
      API_TOKEN &&
      API_TOKEN.trim().length > 0
    )
      return API_TOKEN.trim();
    if (route && route.params && route.params.token) return route.params.token;
    try {
      const t = await AsyncStorage.getItem('API_TOKEN');
      if (t && t.trim().length > 0) return t.trim();
    } catch (e) {}
    return null;
  };

  const pushLocation = (locationsArr, obj, fallbackName) => {
    if (!obj) return;
    const lat = Number(obj.latitude ?? obj.latitud ?? obj.lat ?? 0);
    const lng = Number(obj.longitude ?? obj.longitud ?? obj.lng ?? 0);
    if (!isFinite(lat) || !isFinite(lng)) return;
    const INCLUDE_ZERO =
      typeof INCLUDE_ZERO_COORDINATES !== 'undefined'
        ? !!INCLUDE_ZERO_COORDINATES
        : false;
    if (!INCLUDE_ZERO && lat === 0 && lng === 0) return;

    const imagen_logo_raw =
      obj.imagen_logo_url ?? obj.image ?? obj.logo ?? null;
    const imagen_banner_raw = obj.imagen_banner_url ?? obj.banner ?? null;

    const imagen_logo = makeAbsoluteUrl(imagen_logo_raw);
    const imagen_banner = makeAbsoluteUrl(imagen_banner_raw);

    locationsArr.push({
      id: obj.id ?? obj.restaurante_id ?? null,
      name: obj.name ?? obj.nombre ?? fallbackName ?? '',
      short_description:
        obj.short_description ??
        obj.direccion ??
        obj.descripcion ??
        obj.description ??
        '',
      latitude: lat,
      longitude: lng,
      rating: obj.rating ?? obj.avg_rating ?? obj.score ?? null,
      image: imagen_logo ?? null, // para compatibilidad
      logo: imagen_logo ?? null, // nombre común para logo (útil en Branch)
      banner: imagen_banner ?? null, // banner si existe
      raw: obj,
      // campos duplicados útiles para WebView/Branch (mantenemos compatibilidad)
      latitud: obj.latitud ?? obj.latitude ?? obj.lat ?? null,
      longitud: obj.longitud ?? obj.longitude ?? obj.lng ?? null,
      nombre: obj.nombre ?? obj.name ?? null,
      direccion: obj.direccion ?? obj.address ?? null,
      imagen_logo_url: imagen_logo ?? null,
      imagen_banner_url: imagen_banner ?? null,
      telefono_sucursal: obj.telefono_sucursal ?? null,
      tipo_comida: obj.tipo_comida ?? null,
      horarios: obj.horarios ?? null,
      url_whatsapp: obj.url_whatsapp ?? null,
      url_opentable: obj.url_opentable ?? null,
      url_instagram: obj.url_instagram ?? null,
      url_facebook: obj.url_facebook ?? null,
      url_tik_tok: obj.url_tiktok ?? null,
      rango_precios: obj.rango_precios ?? null,
      codigo: obj.codigo ?? null,
      descripcion: obj.descripcion ?? null,
    });
  };

  const fetchSucursalesForRestaurant = async (
    baseUrl,
    token,
    restaurantId,
    headers,
  ) => {
    if (!restaurantId) return [];
    try {
      const base = (baseUrl || '').replace(/\/$/, '');
      const url = base + '/' + encodeURIComponent(restaurantId) + '/sucursales';
      const r = await fetch(url, {method: 'GET', headers});
      if (!r.ok) return [];
      const j = await r.json();
      if (Array.isArray(j)) return j;
      if (j && Array.isArray(j.sucursales)) return j.sucursales;
      for (const k in j) {
        if (Array.isArray(j[k])) return j[k];
      }
      return [];
    } catch (e) {
      warn('fetchSucursalesForRestaurant error', e);
      return [];
    }
  };

  useEffect(() => {
    (async () => {
      setLoading(true);
      const ok = await hasLocationPermission();
      if (!mountedRef.current) return;
      if (!ok) {
        Alert.alert(
          'Permiso requerido',
          'Sin permiso no podemos mostrar el mapa.',
        );
        setLoading(false);
        return;
      }

      Geolocation.getCurrentPosition(
        async position => {
          if (!mountedRef.current) return;
          const userLat = position.coords.latitude;
          const userLng = position.coords.longitude;
          const token = await getAuthToken();
          if (!mountedRef.current) return;

          if (!token) {
            warn('No token found - mostrando solo ubicación del usuario');
            const solo = generateMapHtml(
              userLat,
              userLng,
              [],
              MARKER_BASE64 || null,
              0,
            );
            if (!mountedRef.current) return;
            setMapHtml(solo);
            setLoading(false);
            return;
          }

          try {
            if (!API_URL || !API_URL.trim()) {
              const solo = generateMapHtml(
                userLat,
                userLng,
                [],
                MARKER_BASE64 || null,
                0,
              );
              if (!mountedRef.current) return;
              setMapHtml(solo);
              setLoading(false);
              return;
            }

            warn('Fetching API_URL (restaurantes list):', API_URL);
            const headers = {
              Accept: 'application/json',
              'Content-Type': 'application/json',
              Authorization: `Bearer ${token}`,
            };
            const res = await fetch(API_URL, {method: 'GET', headers});

            if (!mountedRef.current) return;

            if (res.status === 401 || res.status === 403) {
              Alert.alert(
                'Autenticación',
                'Token inválido o expirado (HTTP ' + res.status + ').',
              );
              const solo = generateMapHtml(
                userLat,
                userLng,
                [],
                MARKER_BASE64 || null,
                0,
              );
              if (!mountedRef.current) return;
              setMapHtml(solo);
              setLoading(false);
              return;
            }

            let json = null;
            try {
              json = await res.json();
            } catch (e) {
              warn('no json body');
            }

            warn(
              'server response preview:',
              (json && JSON.stringify(json).slice(0, 400)) || 'empty',
            );

            // Normalizar respuesta a array de restaurantes
            let restaurants = [];
            if (Array.isArray(json)) restaurants = json;
            else if (json && Array.isArray(json.restaurantes))
              restaurants = json.restaurantes;
            else if (json && Array.isArray(json.restaurants))
              restaurants = json.restaurants;
            else if (json && json.data && Array.isArray(json.data))
              restaurants = json.data;
            else if (
              json &&
              typeof json === 'object' &&
              (json.id || json.nombre)
            )
              restaurants = [json];
            else {
              for (const k in json || {}) {
                if (Array.isArray(json[k])) {
                  restaurants = json[k];
                  break;
                }
              }
            }

            const locations = [];

            // Procesar sucursales embebidas o restaurante con coordenadas
            for (const item of restaurants) {
              if (
                item &&
                Array.isArray(item.sucursales) &&
                item.sucursales.length > 0
              ) {
                item.sucursales.forEach(s =>
                  pushLocation(locations, s, item.nombre || item.name),
                );
              } else if (
                item &&
                Array.isArray(item.branches) &&
                item.branches.length > 0
              ) {
                item.branches.forEach(s =>
                  pushLocation(locations, s, item.nombre || item.name),
                );
              } else {
                const maybeLat = item
                  ? item.latitude ?? item.latitud ?? item.lat ?? null
                  : null;
                const maybeLng = item
                  ? item.longitude ?? item.longitud ?? item.lng ?? null
                  : null;
                if (maybeLat != null || maybeLng != null)
                  pushLocation(locations, item, item?.nombre || item?.name);
              }
            }

            // Para restaurantes que no traen sucursales embebidas, pedir /{id}/sucursales
            const restaurantsNeedingFetch = restaurants.filter(r => {
              const hasSuc =
                r && Array.isArray(r.sucursales) && r.sucursales.length > 0;
              const hasBranches =
                r && Array.isArray(r.branches) && r.branches.length > 0;
              return !(hasSuc || hasBranches);
            });

            if (restaurantsNeedingFetch.length > 0) {
              try {
                const fetchPromises = restaurantsNeedingFetch.map(r => {
                  const id =
                    r.id ?? r.restaurante_id ?? r._id ?? r.id_restaurante;
                  return fetchSucursalesForRestaurant(
                    API_URL,
                    token,
                    id,
                    headers,
                  )
                    .then(arr => ({restaurant: r, branches: arr}))
                    .catch(() => ({restaurant: r, branches: []}));
                });
                const results = await Promise.all(fetchPromises);
                results.forEach(resItem => {
                  const parentName =
                    (resItem.restaurant &&
                      (resItem.restaurant.nombre || resItem.restaurant.name)) ||
                    '';
                  if (Array.isArray(resItem.branches)) {
                    resItem.branches.forEach(b =>
                      pushLocation(locations, b, parentName),
                    );
                  }
                });
              } catch (e) {
                warn('Error fetching branches in parallel', e);
              }
            }

            // Fallback a origin+'/api/restaurantes' si no hay pines
            if (locations.length === 0) {
              try {
                let fallback = null;
                try {
                  const u = new URL(API_URL);
                  fallback = u.origin + '/api/restaurantes';
                } catch (err) {
                  const parts = API_URL.split('/');
                  if (parts.length >= 3)
                    fallback = parts[0] + '//' + parts[2] + '/api/restaurantes';
                }
                if (fallback) {
                  warn('Trying fallback:', fallback);
                  const r2 = await fetch(fallback, {method: 'GET', headers});
                  if (r2.ok) {
                    const j2 = await r2.json();
                    const maybeArr = Array.isArray(j2)
                      ? j2
                      : Array.isArray(j2.restaurantes)
                      ? j2.restaurantes
                      : Array.isArray(j2.data)
                      ? j2.data
                      : [];
                    if (Array.isArray(maybeArr)) {
                      maybeArr.forEach(item => {
                        if (item.sucursales && Array.isArray(item.sucursales))
                          item.sucursales.forEach(s =>
                            pushLocation(
                              locations,
                              s,
                              item.nombre || item.name,
                            ),
                          );
                        else
                          pushLocation(
                            locations,
                            item,
                            item.nombre || item.name,
                          );
                      });
                    }
                  } else {
                    warn('fallback failed status', r2.status);
                  }
                }
              } catch (e) {
                warn('fallback error', e);
              }
            }

            warn('locations processed:', locations.length);
            if (locations.length === 0) {
              Alert.alert(
                'Sin pines',
                'No se encontraron sucursales con coordenadas. Revisa la API, token y que las coordenadas no sean 0,0.',
              );
            }

            const markerToUse =
              typeof MARKER_BASE64 === 'string' && MARKER_BASE64.length > 50
                ? MARKER_BASE64
                : null;
            const html = generateMapHtml(
              userLat,
              userLng,
              locations,
              markerToUse,
              locations.length,
            );
            if (!mountedRef.current) return;
            setMapHtml(html);
          } catch (err) {
            console.error('Error cargando restaurantes:', err);
            if (!mountedRef.current) return;
            Alert.alert('Error', 'No se pudo cargar los restaurantes.');
            const solo = generateMapHtml(
              userLat,
              userLng,
              [],
              MARKER_BASE64 || null,
              0,
            );
            setMapHtml(solo);
          } finally {
            if (!mountedRef.current) return;
            setLoading(false);
          }
        },
        err => {
          console.error('Error al obtener ubicación:', err);
          if (!mountedRef.current) return;
          Alert.alert('Ubicación', 'No se pudo obtener la ubicación actual.');
          setLoading(false);
        },
        {enableHighAccuracy: true, timeout: 15000, maximumAge: 10000},
      );
    })();
  }, []);

  const toFiniteNumber = v => {
    const n = typeof v === 'number' ? v : Number(v);
    return Number.isFinite(n) ? n : null;
  };

  const promptFallback = (title, message, webUrl, appStoreUrl) => {
    Alert.alert(
      title,
      message,
      [
        {text: 'Abrir en Safari', onPress: () => Linking.openURL(webUrl)},
        ...(appStoreUrl
          ? [
              {
                text: 'Obtener la app',
                onPress: () => Linking.openURL(appStoreUrl),
              },
            ]
          : []),
        {text: 'Cancelar', style: 'cancel'},
      ],
      {cancelable: true},
    );
  };

  const openAppleMaps = async (lat, lng) => {
    // Try native Apple Maps first (may fail if Maps app is deleted)
    const nativeUrl = `maps://?daddr=${lat},${lng}`;
    const webUrl = `https://maps.apple.com/?daddr=${lat},${lng}`;
    const appStoreUrl = `itms-apps://apps.apple.com/app/id915056765`; // Apple Maps :contentReference[oaicite:0]{index=0}

    try {
      await Linking.openURL(nativeUrl);
    } catch (e) {
      // Fallback + optional "Get the app"
      promptFallback(
        'Apple Maps',
        'No se pudo abrir Apple Maps. ¿Quieres abrir la ruta en Safari o instalar la app?',
        webUrl,
        appStoreUrl,
      );
    }
  };

  const openGoogleMaps = async (lat, lng) => {
    // Check installation via URL scheme, per Google + Apple guidance
    const scheme = 'comgooglemaps://';
    const nativeUrl = `comgooglemaps://?daddr=${lat},${lng}&directionsmode=driving`;
    const webUrl = `https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}&travelmode=driving`;
    const appStoreUrl = `itms-apps://apps.apple.com/app/id585027354`; // Google Maps :contentReference[oaicite:1]{index=1}

    try {
      const installed = await Linking.canOpenURL(scheme); // requires Info.plist LSApplicationQueriesSchemes :contentReference[oaicite:2]{index=2}
      if (installed) {
        await Linking.openURL(nativeUrl);
      } else {
        promptFallback(
          'Google Maps',
          'Google Maps no está instalado. ¿Quieres abrir la ruta en Safari o instalar la app?',
          webUrl,
          appStoreUrl,
        );
      }
    } catch (e) {
      // If canOpenURL/openURL throws, still offer fallback
      promptFallback(
        'Google Maps',
        'No se pudo abrir Google Maps. ¿Quieres abrir la ruta en Safari o instalar la app?',
        webUrl,
        appStoreUrl,
      );
    }
  };

  const openNavigationChooserIOS = (lat, lng) => {
    Alert.alert(
      'Abrir navegación',
      'Elige una app',
      [
        {text: 'Apple Maps', onPress: () => void openAppleMaps(lat, lng)},
        {text: 'Google Maps', onPress: () => void openGoogleMaps(lat, lng)},
        {text: 'Cancelar', style: 'cancel'},
      ],
      {cancelable: true},
    );
  };

  const onMessage = event => {
    try {
      console.log('WEBVIEW onMessage raw:', event.nativeEvent.data);
      const payload = JSON.parse(event.nativeEvent.data);
      if (!payload || !payload.action) return;

      if (payload.action === 'viewProfile') {
        const branchObj = payload.branch ? payload.branch : null;

        let rid = null;
        if (branchObj) {
          rid =
            branchObj.id ?? branchObj.restaurante_id ?? branchObj._id ?? null;
        }
        if (rid == null && payload.id != null) {
          rid = payload.id;
        }

        navigation.navigate('Branch', {
          id: rid,
          branch: branchObj,
          logo: branchObj?.logo ?? branchObj?.imagen_logo_url ?? null,
          banner: branchObj?.banner ?? branchObj?.imagen_banner_url ?? null,
          raw: branchObj?.raw ?? null,
        });
      } else if (payload.action === 'navigate') {
        const lat = toFiniteNumber(payload.lat);
        const lng = toFiniteNumber(payload.lng);

        if (lat == null || lng == null) {
          Alert.alert(
            'Navegar',
            'Coordenadas no disponibles para esta sucursal.',
          );
          return;
        }

        if (Platform.OS === 'ios') {
          openNavigationChooserIOS(lat, lng);
          return;
        }

        // Android: keep your current behavior
        const url = `geo:${lat},${lng}?q=${lat},${lng}`;
        Linking.openURL(url).catch(() =>
          Alert.alert('Abrir mapas', 'No se pudo abrir la app de mapas'),
        );
      }
    } catch (e) {
      console.error('onMessage parse error', e);
    }
  };

  if (loading || !mapHtml) {
    return (
      <View style={styles.loading}>
        <ActivityIndicator size="large" />
        <Text style={{marginTop: 8, color: '#666'}}>Cargando mapa…</Text>
      </View>
    );
  }

  return (
    <View style={{flex: 1}}>
      <WebView
        ref={webviewRef}
        originWhitelist={['*']}
        source={{html: mapHtml}}
        style={styles.map}
        javaScriptEnabled
        domStorageEnabled
        onMessage={onMessage}
      />
    </View>
  );
}

const styles = StyleSheet.create({
  map: {flex: 1, width: Dimensions.get('window').width},
  loading: {flex: 1, justifyContent: 'center', alignItems: 'center'},
});
