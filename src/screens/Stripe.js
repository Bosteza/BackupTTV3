...
export default function StripePay() {
  ...
  const PADDING = Math.max(12, Math.round(winW * 0.01));
  ...
  const displayAmountFinal = Number(
    (displayAmount ?? subtotalNum + propinaNum).toFixed(2),
  );
  ...
  const buildTransactionUrl = () =>
    `${String(api_host || 'https://127.0.0.1').replace(
      /\/$/,
      '',
  ...
  const buildSetupIntentUrl = sucursalId =>
    `${String(api_host || 'https://127.0.0.1').replace(
      /\/$/,
      '',
    )}/api/mobileapp/sucursales/${encodeURIComponent(
      sucursalId,
    )}/payment-methods/stripe/setup-intent`;
  const buildListPaymentMethodsUrl = (sucursalId, usuarioAppId) =>
    `${String(api_host || 'https://127.0.0.1').replace(
      ...
      sucursalId,
    )}/payment-methods?gateway=stripe&environment=${encodeURIComponent(
      environment,
    )}&usuario_app_id=${encodeURIComponent(usuarioAppId)}`;
  ...
  const validateForm = () => {
    ...
    if (!usingSavedCard && (!cardDetails || !cardDetails.complete))
  ...
  const fetchSavedPaymentMethods = async () => {
    ...
    if (!usuarioAppIdToSend) {
      console.warn(
        'fetchSavedPaymentMethods - no user_usuario_app_id disponible',
        {usuario_app_id},
      );
    ...
    console.warn(
      '[DEBUG] fetchSavedPaymentMethods - Authorization present:',
      !!AUTH_TOKEN,
    );
    ...
    try {
      ...
      console.warn(
        '[DEBUG] fetchSavedPaymentMethods - httpStatus:',
        res.status,
      );
      const json = await res.json().catch(() => null);
      ...
      if (!res.ok) {
        ...
        Alert.alert(
          'Error',
          `No se pudo obtener tarjetas guardadas (${res.status})`,
        );
        setSavedCards([]);
      ...
      if (!arr.length) {
        console.warn(
          'fetchSavedPaymentMethods: payment_methods vacío o no existe',
          {url, responseJson: json},
        );
      } else {
        console.warn(
          'fetchSavedPaymentMethods: tarjetas encontradas',
          arr.length,
        );
      }
      ...
      Alert.alert(
        'Error',
        'No se pudo conectar al servidor de tarjetas guardadas.',
      );
      setSavedCards([]);
  ...
  const createSetupIntentOnServer = async () => {
    await ensureToken();
    if (!sucursal_id)
      throw new Error('Falta sucursal_id para crear setup intent');
    ...
    const body = {
      ...
      usuario_app_id: usuarioAppIdToSend,
      set_preferred: Boolean(savePreferred),
    };
    const idKey = genIdempotencyKey();
    console.warn(
      '[DEBUG] createSetupIntentOnServer - url:',
      url,
      'idKey:',
      idKey,
      'body:',
    ...
    try {
      ...
      const json = await res.json().catch(() => null);
      console.warn(
        '[DEBUG] createSetupIntentOnServer - response status:',
        res.status,
        'json:',
        json,
      );
      ...
      if (!clientSecret)
        throw new Error(
          'El servidor no devolvió client_secret para SetupIntent',
        );
  ...
  const confirmAndSaveCard = async setupClientSecret => {
    if (!setupClientSecret)
      return {ok: false, error: new Error('No client_secret')};
    try {
  ...
  const processPaymentFlow = async () => {
    ...
    const usuario_app_id_to_send =
      ...
      (usuario_app_id && String(usuario_app_id).trim()) ||
      '';
    ...
    if (usingSavedCard && selectedSavedCard) {
      ...
      body.mobile_payment_method_id =
        ...
        selectedSavedCard.mobile_payment_method_id ??
        null;
    }
    ...
    try {
      ...
      console.warn(
        '[DEBUG] processPaymentFlow - create transaction response:',
        json,
      );
      ...
      const clientSecret =
        clientSecretFromResponse || presetClientSecret || null;

      if (!transactionId) {
        setProcessing(false);
        showPaymentError(
          'Error',
          'El servidor no devolvió transaction_id. Revisa la respuesta en consola.',
          json ? JSON.stringify(json) : null,
        );
      ...
      if (usingSavedCard && selectedSavedCard) {
        console.warn(
          '[DEBUG] usando tarjeta guardada -> saltando confirmPayment en cliente; iniciando pollSplitsUntilPaid',
        );
        const pollResult = await pollSplitsUntilPaid(
          transactionId,
          pollingTimeoutMs,
          pollingIntervalMs,
        );
        ...
        if (pollResult.ok) {
          ...
          showPaymentError(
            'Pendiente',
            'Pago con tarjeta guardada: el servidor aún no refleja la venta como pagada.',
            pollResult ? JSON.stringify(pollResult) : null,
          );
          return;
        }
      }
      ...
      if (clientSecret) {
        try {
          ...
          if (error) {
            ...
            showPaymentError(
              ...
              error.code ? `code: ${error.code}` : JSON.stringify(error),
            );
            return;
          }
          ...
          if (
            ...
            if (pollResult.ok) {
              ...
              showPaymentError(
                'Pendiente',
                'Pago confirmado por Stripe pero el servidor aún no refleja la venta como pagada.',
                pollResult ? JSON.stringify(pollResult) : null,
              );
              return;
            }
            ...
            showPaymentError(
              ...
              paymentIntent ? JSON.stringify(paymentIntent) : null,
            );
            return;
          }
        } catch (err) {
          ...
          showPaymentError(
            'Error',
            'Ocurrió un error confirmando el pago con Stripe.',
        ...
        showPaymentError(
          'Falta client_secret',
          'El servidor no devolvió client_secret. Revisa la respuesta en consola.',
          json ? JSON.stringify(json) : null,
        );
  ...
  const handleSaveAndPay = async () => {
    ...
    try {
      ...
      if (!setupResp.ok) {
        ...
        Alert.alert(
          'Advertencia',
          'No se pudo guardar la tarjeta. Se continuará con el pago sin guardar la tarjeta.',
        );
      ...
      if (!setupConfirmResp.ok) {
        console.warn(
          'No se pudo confirmar SetupIntent:',
          setupConfirmResp.error,
        );
        Alert.alert(
          'Advertencia',
          'No se pudo confirmar el guardado de la tarjeta. Se continuará con el pago sin guardar la tarjeta.',
        );
        await processPaymentFlow();
        return;
      }
      ...
      Alert.alert(
        'Error',
        'Ocurrió un error al intentar guardar la tarjeta. Se intentará continuar con el pago.',
      );
      await processPaymentFlow();
    } finally {
      setSavingCard(false);
  ...
  const onPayPress = async () => {
    ...
    if (
      ...
      Alert.alert(
        'Falta Stripe key',
        'Pasa stripe_public_key (creds.public_key) a esta pantalla en params o reemplaza pk_test_REPLACE_ME en el código.',
      );
      return;
    }
  ...
  useEffect(() => {
    ...
    const unsub = navigation.addListener('focus', () => {
      ...
      fetchSavedPaymentMethods().catch(e =>
        console.warn('refresh saved cards on focus error', e),
      );
    });
    return () => {
      try {
  ...
  const restaurantSrc = restaurantImage
    ? {uri: restaurantImage}
    : DEFAULT_RESTAURANT;
  const currentDateText = new Date().toLocaleString('es-MX', {
    dateStyle: 'long',
    timeStyle: 'short',
  });
  ...
  const SavedCardView = ({card}) => {
    ...
    const exp = `${card.exp_month ?? ''}/${String(card.exp_year ?? '').slice(
      -2,
    )}`;
    return (
      <View style={styles.savedCardRow}>
        <Ionicons size={20} color={'#0b1220'} style={{marginRight: 8}} />
        <View style={{flex: 1}}>
          <Text style={{fontWeight: '800', color: '#0b1220'}}>
            {brand} • {mask}
          </Text>
          <Text style={{fontSize: 12, color: '#6b7280'}}>
            Exp: {exp} {card.is_preferred ? ' • Preferida' : ''}
          </Text>
        </View>
        <TouchableOpacity
          onPress={() => {
  ...
  const performDeleteSavedCard = async () => {
    ...
    const cardId =
      cardToDelete.id ??
      cardToDelete.mobile_payment_method_id ??
      cardToDelete.external_payment_method_id;
    const url = `${host}/api/mobileapp/sucursales/${encodeURIComponent(
      sucursal_id,
    )}/payment-methods/${encodeURIComponent(
      cardId,
    )}?gateway=stripe&environment=${encodeURIComponent(environment)}`;
    ...
    try {
      ...
      console.warn(
        'performDeleteSavedCard - read user_usuario_app_id failed',
        e,
      );
    }
    ...
    try {
      ...
      setSavedCards(prev =>
        Array.isArray(prev)
          ? prev.filter(c => String(c.id) !== String(cardToDelete.id))
          : [],
      );
  ...
  const computedListHeight = Math.min(
    (savedCards?.length || 0) * approxItemHeight,
    maxVisibleItems * approxItemHeight,
  );
  const modalHeight = Math.min(
    Math.max(headerHeight + computedListHeight + footerHeight, minModalHeight),
    winH - 80,
  );
  ...
  const hasTypedCard =
    ...
    (cardDetails?.brand && cardDetails.brand !== 'Unknown');

  const showSavedCardBlock =
    !usingSavedCard && !cardFieldFocused && !hasTypedCard;
            ...
            <View style={styles.gradientInner}>
              ...
              <View style={styles.gradientRight}>
                ...
                <View style={{flexDirection: 'row', alignItems: 'flex-end'}}>
                  ...
                  <Text style={styles.gradientCurrency}>
                    {' '}
                    {moneda ?? 'MXN'}
                  </Text>
                </View>
            ...
            <View
              ...
              <View style={[styles.inputWrap, {paddingVertical: 8}]}>
                <View style={{flex: 1}}>
                  {usingSavedCard && selectedSavedCard ? (
                    ...
                    <View style={styles.cardFieldShell}>
                      <CardField
                  ...
                  {showSavedCardBlock && (
                    <TouchableOpacity
                      onPress={openSavedCardsModal}
                      style={styles.savedCardBlock}
                      activeOpacity={0.85}>
                      <View
                        ...
                        <Ionicons
                          ...
                          style={{marginRight: 8}}
                        />
                        <Text style={styles.savedCardBlockText}>
              ...
              <View style={{marginTop: PAY_BTN_MARGIN, alignItems: 'center'}}>
                <TouchableOpacity
                  style={[
                    styles.payBtn,
                    {width: Math.min(560, winW - PADDING * 2)},
                  ]}
                  onPress={onPayPress}
                  activeOpacity={0.9}
          ...
          <Modal
            ...
            <View style={styles.autoModalBackdrop}>
              <View
                ...
                <Text
                  style={{fontSize: 13, color: '#334155', marginBottom: 14}}>
                  Puedes guardar la tarjeta en Stripe para futuros pagos. Elige
                  una opción:
                </Text>
              ...
              <View
                ...
                <View
                  ...
                  <Text style={{fontSize: 13, color: '#334155', marginTop: 6}}>
                    Toca una tarjeta para seleccionarla o pulsa "Pagar con
                    otra".
                  </Text>
                </View>
                ...
                <View style={{flex: 1, padding: 8}}>
                  {savedCardsLoading ? (
                    ...
                    <FlatList
                      ...
                      keyExtractor={i =>
                        String(
                          i.id ?? i.external_payment_method_id ?? Math.random(),
                        )
                      }
                      style={{flex: 1}}
                      ...
                      renderItem={({item}) => (
                        <View style={styles.savedCardTouchable}>
                          ...
                          <View
                            ...
                            <Text style={styles.savedCardStatus}>
                              {item.status ?? ''}
                            </Text>
                          </View>
                        </View>
                      ...
                      ListEmptyComponent={() => (
                        <View style={{padding: 12}}>
                          ...
                          <TouchableOpacity
                            onPress={() => {
                              console.warn(
                                '[DEBUG] lastSavedCardsResponse:',
                                lastSavedCardsResponse,
                              );
                              Alert.alert(
                                'Respuesta (debug)',
                                JSON.stringify(
            ...
            <View style={styles.autoModalBackdrop}>
              <View style={styles.autoModalBox}>
                ...
                <View style={{marginLeft: 12, flex: 1}}>
                  ...
                  <Text
                    style={styles.autoModalMsg}
                    numberOfLines={3}
                    ellipsizeMode="tail">
                    Gracias — el pago se procesó correctamente. Puedes regresar
                    al menú.
                  </Text>
                </View>
              </View>
