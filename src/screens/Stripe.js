...
export default function StripePay() {
  ...
  const prevValidCvcRef = useRef('Unknown');
  ...
  const {
    ...
    api_token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJmcmVzaCI6ZmFsc2UsImlhdCI6MTc3MDEzNjkxMCwianRpIjoiMzM3YjlkY2YtYjlkMi00NjFjLTkxMDItYzlkZjFkNDFlYmFjIiwidHlwZSI6ImFjY2VzcyIsInN1YiI6IjMiLCJuYmYiOjE3NzAxMzY5MTAsImV4cCI6MTc3MjcyODkxMCwicm9sIjoiRWRpdG9yIn0.GVPx2mKxkE7qZQ9AozQnldLlkogOOLksbetncQ8BgmY',
    sucursal_id = null,
    sale_id = null,
    restaurante_id = null,
    usuario_app_id = null,
            ...
            if (pollResult.ok) {
              try {
                await notifyPayment({
                  ...
                  // optional extras if your provider supports them:
                  // transactionId,
                  // paidCodes: pollResult.paidCodes,
                  // gateway: 'stripe',
                });
              } catch (e) {
                console.warn('notifyPayment failed', e);
              }
              ...
              setTimeout(async () => {
                ...
                try {
                  ...
                  console.warn('navigate QRMain failed', e);
                }
              }, 1200);
              return; */
            } else {
  ...
  if (!stripePublishableKey) {
    Alert.alert(
      ...
      'No se recibió la Stripe publishable key',
    );
    return null;
  }
              ...
              <View style={{flex: 1}}>
                <CardField
                  ...
                  onCardChange={details => {
                    ...
                    const prev = prevValidCvcRef.current;
                    const curr = details?.validCVC ?? 'Unknown';
                    prevValidCvcRef.current = curr;

                    // When CVV reaches "Valid", dismiss keyboard
                    if (prev !== 'Valid' && curr === 'Valid') {
                      cardFieldRef.current?.blur?.(); // most reliable for CardField
