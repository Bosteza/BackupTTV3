  ...
  try {
    ...
    const normalized = {
      ...
      restaurantName:
        ...
        'Restaurante',
      restaurantImage:
        visit.restaurantImage ?? visit.restaurantImageUri ?? null,
    ...
    try {
      if (normalized.sale_id) {
        ...
        pend = pend.filter(
          p => String(p.sale_id) !== String(normalized.sale_id),
        );
        await AsyncStorage.setItem(PENDING_VISITS_KEY, JSON.stringify(pend));
        try {
          await AsyncStorage.removeItem(
            `pending_payment_${String(normalized.sale_id)}`,
          );
        } catch (e) {}
      }
    } catch (e) {}
...
async function savePendingVisit(visit) {
  try {
    ...
    const arr = raw ? JSON.parse(raw) || [] : [];
    const idKey = visit.sale_id
      ? String(visit.sale_id)
      : visit.id
      ? String(visit.id)
      : null;
    let filtered = arr;
    if (idKey)
      filtered = arr.filter(a => String(a.sale_id || a.id || '') !== idKey);
...
export default function Escanear() {
  ...
  const checkPendingPromotions = useCallback(async (log = false) => {
    try {
      ...
      if (!Array.isArray(pend) || pend.length === 0) {
        if (log) console.log('checkPendingPromotions: no pendings');
        return;
      }
      ...
      for (const p of pend) {
        try {
          ...
          const splitsBySaleUrl = `${API_BASE_URL.replace(
            /\/$/,
            '',
          ...
          try {
            ...
            if (log)
              console.warn('checkPendingPromotions: error fetching splits', e);
            keep.push(p);
          }
        } catch (inner) {
      ...
      try {
        await AsyncStorage.setItem(PENDING_VISITS_KEY, JSON.stringify(keep));
      } catch (e) {
        console.warn('error saving pendings', e);
      }
    } catch (err) {
      console.warn('checkPendingPromotions error', err);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!mounted) return;
      try {
        await checkPendingPromotions(true);
      } catch (e) {}
    })();
    const id = setInterval(() => {
      if (!mounted) return;
      checkPendingPromotions(false).catch(() => {});
  ...
  const fetchConsumo = useCallback(
    async (opts = {showLoading: true}) => {
      if (!token) {
        openErrorModal('Token no encontrado. Vuelve a escanear.');
      ...
      try {
        ...
        if (!res.ok) {
          openErrorModal(`No se pudo obtener el consumo (HTTP ${res.status}).`);
          if (isMountedRef.current) setLoading(false);
          return;
        }
        ...
        const computedTotal =
          reportedTotal > 0
            ? reportedTotal
            : expandedItems.reduce((s, x) => s + safeNum(x.lineTotal), 0);
        if (isMountedRef.current)
          setOriginalTotalConsumo(+computedTotal.toFixed(2));

        const neutralItems = expandedItems.map(it => ({
          ...it,
          paid: false,
          paidPartial: false,
          paidAmount: 0,
        ...
        try {
          ...
          console.warn('Error reading equal_split_paid flag', e);
        }

        let localPaidSet = new Set();
        try {
          localPaidSet = await readLocalPaidIds(sale);
        } catch (e) {
          localPaidSet = new Set();
        }

        if (!eqPreviously) {
          try {
            const allocatedFromLocal = expandedItems.map(it => ({
              ...it,
              paid: false,
              paidPartial: false,
              paidAmount: 0,
            ...
            const paidSumLocal = allocatedFromLocal.reduce(
              (s, it) => s + safeNum(it.paidAmount || 0),
              0,
            );
            const outstandingLocal = +(computedTotal - paidSumLocal).toFixed(2);
            if (isMountedRef.current) {
              setItems(allocatedFromLocal);
              setTotalConsumo(outstandingLocal >= 0 ? outstandingLocal : 0);
            }
          } catch (e) {
            console.warn('Error applying local paid set', e);
          }
        }
        ...
        try {
          ...
          if (rawTx) {
            ...
            if (tx) {
              const splitsUrl = `${API_BASE_URL.replace(
                /\/$/,
                '',
              ...
              try {
                ...
                if (splitsRes && splitsRes.ok) {
                  ...
                  const splitsArr = Array.isArray(splitsJson.splits)
                    ? splitsJson.splits
                    : [];
                  const paidSplits = splitsArr.filter(
                    s => String(s.estado ?? '').toLowerCase() === 'paid',
                  ...
                  if (paidSplits.length > 0) {
                    ...
                    const hasEqualSplitPaid = paidSplits.some(s => {
                      ...
                      const name = String(
                        s.nombre_item ?? s.nombre ?? '',
                      ).toLowerCase();
                      return (
                        code === '1' ||
                        /partes iguales|pago por partes iguales|pago por partes/i.test(
                          name,
                        )
                      );
                    });
                    ...
                    if (hasEqualSplitPaid) {
                      ...
                      const paidCodesRaw = paidSplits
                        .map(s =>
                          String(
                            s.codigo_item ?? s.codigo ?? s.code ?? '',
                          ).trim(),
                        )
                        .filter(Boolean);
                      const paidCodesFiltered = paidCodesRaw.filter(
                        c => c !== '1',
                      );
                      ...
                      const allocated = expandedItems.map(it => ({
                        ...it,
                        paid: false,
                        paidPartial: false,
                        paidAmount: 0,
                      ...
                      const paidSum = allocated.reduce(
                        (s, it) => s + safeNum(it.paidAmount || 0),
                        0,
                      );
                      ...
                      if (isMountedRef.current) {
                        setItems(allocated);
                        setTotalConsumo(outstanding >= 0 ? outstanding : 0);
                      }
                      const unionArr = Array.from(
                        new Set([
                          ...Array.from(localSet2 || []).map(String),
                          ...paidCodes.map(String),
                        ]),
                      ).filter(Boolean);
                      if (unionArr.length > 0)
                        await mergePersistLocalPaidIds(sale, unionArr);
                    }
                  }
                }
              } catch (e) {
              ...
              if (sr && sr.ok) {
                ...
                if (paidSplits.length > 0) {
                  ...
                  if (hasEqualSplitPaid) {
                    ...
                    const unionArr = Array.from(
                      new Set([
                        ...Array.from(localSet3 || []).map(String),
                        ...paidCodes.map(String),
                      ]),
                    ).filter(Boolean);
                    if (unionArr.length > 0)
                ...
                console.log(
                  'splitsBySale http error',
                  sr ? sr.status : 'no res',
                );
              }
            } catch (e) {
              console.warn('Error fetching splitsBySale', e);
            }
          }
        } catch (e) {
  ...
  const fechaTexto = fechaApertura
    ? new Date(fechaApertura).toLocaleString('es-MX')
    : '';
  const fechaCierreTexto = fechaCierre
    ...
    <SafeAreaView style={[styles.safe, {paddingTop: topSafe}]}>
      ...
      <View
        ...
        <TouchableOpacity
          ...
          <Text style={[styles.backArrow, {fontSize: clamp(rf(4.2), 22, 36)}]}>
            {'‹'}
          </Text>
        </TouchableOpacity>

        <View style={{flex: 1}}>
          <Text style={[styles.title, {fontSize: clamp(rf(3.6), 14, 18)}]}>
            Tu cuenta
          </Text>
          <Text style={[styles.topSmall, {fontSize: clamp(rf(1.6), 10, 12)}]}>
            {fechaTexto}
          </Text>
          {fechaCierreTexto ? (
            <Text
              style={[
                styles.topSmall,
                {marginTop: 4, fontSize: clamp(rf(1.6), 10, 12)},
              ]}>
              Cierre: {fechaCierreTexto}
            </Text>
          ) : null}
        </View>
      ...
      <Modal
        ...
        <View style={styles.modalBackdropModal}>
          <LinearGradient
            colors={['#9F4CFF', '#6A43FF', '#2C7DFF']}
            style={styles.modalBox}>
            <Text style={styles.modalTitle}>Aviso</Text>

            {/* If messages can get long, this avoids cutting */}
            <ScrollView
            ...
            <View style={styles.modalButtonsRow}>
              <TouchableOpacity
                ...
                <Text style={styles.modalBtnPrimaryText} numberOfLines={2}>
                  Volver a escanear
                </Text>
              </TouchableOpacity>

              {/* Spacer instead of margins that can overflow */}
              <View style={{width: 10}} />
      ...
      {loading ? (
        ...
        <ScrollView
          ...
          automaticallyAdjustContentInsets={false} // older RN safety
          contentInset={{top: 0, bottom: 0}} // extra safety
          scrollIndicatorInsets={{top: 0, bottom: 0}}
          contentContainerStyle={[
            styles.container,
            {
              flexGrow: 1,
              paddingBottom: Math.max(20, hp(3)) + bottomSafe,
              paddingTop: 0,
            },
          ]}
          showsVerticalScrollIndicator={false}>
          ...
          <View
            style={[
              ...
              {
                width: layoutWidth,
                paddingVertical: Math.max(12, hp(1.6)),
                paddingHorizontal: Math.max(12, wp(3)),
                marginTop: Math.max(8, hp(1)),
              },
            ]}>
            <View style={styles.cardHeaderRow}>
              <Text style={[styles.mesaText, {fontSize: mesaFont}]}>
                Mesa: {mesaId ?? '—'}
              </Text>
              <View style={{width: wp(10)}} />
            </View>

            <Text style={[styles.desgloseTitle, {fontSize: desgloseTitleFont}]}>
              Desglose
            </Text>
            ...
            <View style={styles.items}>
              ...
              <View style={[styles.itemRow, {paddingTop: 6}]}>
                ...
                <Text
                  ...
                  {formatMoney(subtotal)} {moneda ?? 'MXN'}
                </Text>
              </View>
              ...
              <View style={[styles.itemRow, styles.ivaRow]}>
                ...
                <Text
                  style={[
                    styles.itemPrice,
                    styles.ivaText,
                    {fontSize: clamp(rf(3), 12, 16), width: itemPriceWidth},
                  ]}>
                  {formatMoney(iva)} {moneda ?? 'MXN'}
                </Text>
              </View>
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              ...
              try {
                ...
                if (keySale) {
                  const pendingPaymentObj = buildPendingPaymentObj(
                    keySale,
                    items,
                    originalTotalConsumo,
                  );
                  try {
                    await AsyncStorage.setItem(
                      `pending_payment_${keySale}`,
                      JSON.stringify(pendingPaymentObj),
                    );
                  } catch (e) {
                    console.warn('Error saving pending_payment', e);
                  }
                }
              } catch (e) {
          ...
          <TouchableOpacity
            style={[
              styles.secondaryButton,
              {
                ...
                backgroundColor: '#fff',
                borderColor: '#ddd',
                paddingVertical: Math.max(12, hp(1.4)),
              },
            ]}
            ...
            <Text
              style={[
                styles.secondaryButtonText,
                {color: '#444', fontSize: clamp(rf(3.2), 13, 16)},
              ]}>
              Volver a escanear
            </Text>
          </TouchableOpacity>
          ...
          <LinearGradient
            ...
            <Text
              style={[
                ...
                {color: '#fff', fontSize: clamp(rf(3.6), 16, 20)},
              ]}>
              {styledAlertTitle}
            </Text>
            <Text
              style={[
                ...
                {color: '#fff', fontSize: clamp(rf(2.8), 13, 16)},
              ]}>
              {styledAlertMessage}
            </Text>
            ...
            <TouchableOpacity
              ...
              onPress={() => setStyledAlertVisible(false)}
              activeOpacity={0.9}
              hitSlop={{top: 8, bottom: 8, left: 8, right: 8}}>
...
const styles = StyleSheet.create({
  ...
  modalBox: {
    ...
    // prevents gradient corners from looking clipped
    overflow: 'hidden',
  },
  ...
  modalMessageScroll: {
    width: '100%',
    maxHeight: 180, // adjust if you want more/less
    marginBottom: 14,
  },
