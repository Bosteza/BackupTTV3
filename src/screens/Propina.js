...
export default function Propina() {
  ...
  const total_comensales =
    params.total_comensales ?? params.totalComensales ?? null;
  ...
  const normalizeItems = rawItems => {
    ...
    return rawItems.map((it, idx) => {
      ...
      const line =
        Number(it.lineTotal ?? it.line_total ?? it.total ?? unit * qty) ||
  ...
  const initialPercent = incomingTipApplied
    ? Number(incomingTipApplied.percent || 0)
    : null;
  const initialOther =
    incomingTipApplied && ![10, 15, 20, 25].includes(initialPercent)
  ...
  const perPersonSubtotal = round2(
    params.perPersonSubtotal ??
      (peopleCount > 0 ? groupSubtotal / peopleCount : groupSubtotal),
  );
  const perPersonIva = round2(
    params.perPersonIva ??
      (peopleCount > 0 ? groupIva / peopleCount : groupIva),
  );
  const perPersonTotal = round2(
    params.perPersonTotal ??
      (peopleCount > 0 ? groupTotal / peopleCount : groupTotal),
  );

  const comingFromEqualSplit =
    returnScreen === 'EqualSplit' || params.from === 'EqualSplit';
  const comingFromConsumo =
  ...
  const groupTotalWithTip = useMemo(
    ...
    [groupTotal, groupTipAmount],
  );
  const perPersonTotalWithTip = useMemo(
    () => Number((perPersonTotal + perPersonTipAmount).toFixed(2)),
    [perPersonTotal, perPersonTipAmount],
  );

  const effectiveTotal =
    comingFromEqualSplit && peopleCount > 1 ? perPersonTotal : groupTotal;
  const effectiveSubtotal =
    comingFromEqualSplit && peopleCount > 1 ? perPersonSubtotal : groupSubtotal;
  const effectiveIva =
    comingFromEqualSplit && peopleCount > 1 ? perPersonIva : groupIva;
  const effectiveTipAmount =
    comingFromEqualSplit && peopleCount > 1
      ? perPersonTipAmount
      : groupTipAmount;
  const effectiveTotalWithTip =
    comingFromEqualSplit && peopleCount > 1
  ...
  const attachMetaDup = base => ({
    ...base,
    mesa_id: extraMeta.mesa_id,
    mesaId: extraMeta.mesa_id,
    mesa_numero: extraMeta.mesa_numero,
    mesaNumero: extraMeta.mesa_numero,
    sucursal_id: extraMeta.sucursal_id,
    sucursalId: extraMeta.sucursal_id,
    restaurante_id: extraMeta.restaurante_id,
    restauranteId: extraMeta.restaurante_id,
    sale_id: extraMeta.sale_id,
    saleId: extraMeta.sale_id,
    total_comensales: extraMeta.total_comensales,
    totalComensales: extraMeta.total_comensales,
    fecha_apertura: extraMeta.fecha_apertura,
    fechaApertura: extraMeta.fecha_apertura,
    moneda: extraMeta.moneda,
    mesero: extraMeta.mesero,
  });
  ...
  const buttonLabel =
    hasAppliedBefore || percent > 0
      ? 'Añadir/editar propina'
      : 'Añadir propina';
  ...
  const topPadding =
    Platform.OS === 'android' ? StatusBar.currentHeight || 0 : insets.top || 8;
  ...
  const contentMaxWidth = Math.min(
    Math.round(width - 32),
    Math.max(420, Math.round(width * 0.9)),
  );
  ...
  const rfBtn = size => Math.round((size * width) / BASE_WIDTH);
  const clampBtn = (val, min, max) => Math.max(min, Math.min(max, val));

  const btnRadius = clampBtn(rfBtn(8), 6, 14);
  const btnPaddingVertical = clampBtn(rfBtn(12), 8, 18);
  ...
  return (
    <SafeAreaView
      style={[
        styles.safe,
        {paddingTop: topPadding, paddingBottom: insets.bottom || 12},
      ]}>
      <StatusBar
      ...
      <View
        ...
        <TouchableOpacity
          ...
          <Text
            style={[styles.backArrow, {fontSize: clampLocal(rf(32), 20, 36)}]}>
            {'‹'}
          </Text>
        </TouchableOpacity>
        <Text style={[styles.title, {fontSize: clampLocal(rf(15), 14, 20)}]}>
          Tu cuenta
        </Text>
        <Text style={[styles.topSmall, {fontSize: clampLocal(rf(10), 10, 14)}]}>
          {todayString}
        </Text>
      </View>
      ...
      <ScrollView
        contentContainerStyle={[
          styles.container,
          {
            paddingBottom: Math.max(24, rf(20)) + (insets.bottom || 0),
            flexGrow: 1,
          },
        ]}
          ...
          <View style={[styles.headerInner, {paddingHorizontal: 0}]}>
            <View style={[styles.leftCol, {flex: 1}]}>
              <Image
                ...
                style={[
                  styles.tabtrackLogo,
                  {width: logoWidth, height: Math.round(logoWidth * 0.32)},
                ]}
                resizeMode="contain"
              />
              <View
                style={[
                  styles.logoWrap,
                  {marginTop: rf(6), padding: Math.round(rf(6))},
                ]}>
                <Image
                  source={
                    restaurantImage
                  ...
                  style={[
                    ...
                    {
                      width: restImageSize,
                      height: restImageSize,
                      borderRadius: Math.round(restImageSize * 0.14),
                    },
                  ]}
                  resizeMode="cover"
                />
            ...
            <View
              ...
              <View style={styles.totalRow}>
                <Text
                  ...
                  numberOfLines={1}>
                  {formatMoney(effectiveTotalWithTip)}
                </Text>
                <Text
                  style={[
                    styles.totalCurrency,
                    {fontSize: clampLocal(rf(12), 11, 14)},
                  ]}>
                  {moneda ?? 'MXN'}
                </Text>
              </View>
              <View style={styles.rightThanks}>
                <Text
                  style={[
                    styles.thanksText,
                    {fontSize: clampLocal(rf(14), 12, 18)},
                  ]}>
                ...
                <Text
                  style={[
                    styles.thanksSub,
                    {fontSize: clampLocal(rf(11), 10, 14)},
                  ]}>
        ...
        <View
          style={[
            styles.content,
            {
              ...
              padding: clampLocal(Math.round(width * 0.035), 10, 22),
            },
          ]}>
          <View style={styles.sectionRow}>
            <Text style={[styles.sectionTitle, {fontSize: sectionTitleFont}]}>
              Selecciona el porcentaje que deseas añadir a tu cuenta.
            </Text>
          </View>
                  ...
                  <View
                    style={[
                      ...
                      checked && styles.checkboxChecked,
                      {
                        width: checkboxSize,
                        height: checkboxSize,
                        borderRadius: Math.round(checkboxSize * 0.14),
                      },
                    ]}>
                    {checked && (
                      <View
                        style={[
                          ...
                          {
                            width: Math.round(checkboxSize * 0.44),
                            height: Math.round(checkboxSize * 0.44),
                          },
                        ]}
                      />
                    )}
                  </View>
                  <Text style={[styles.optionText, {fontSize: optionFont}]}>
                    {p}%
                  </Text>
            ...
            <View
              ...
              <TouchableOpacity
                ...
                <View
                  style={[
                    styles.checkbox,
                    customActive && styles.checkboxChecked,
                    {
                      width: checkboxSize,
                      height: checkboxSize,
                      borderRadius: Math.round(checkboxSize * 0.14),
              ...
              <View
                style={[
                  ...
                  {
                    width: clampLocal(Math.round(width * 0.24), 80, 160),
                    height: inputHeight,
                  },
                ]}>
                <TextInput
                  ...
                  style={[
                    styles.otherInput,
                    {height: inputHeight, fontSize: clampLocal(rf(14), 13, 18)},
                  ]}
                />
                <Text
                  style={[
          ...
          <View style={styles.totalsRow}>
            ...
            <Text style={[styles.totValue, {fontSize: smallFont}]}>
              {formatMoney(effectiveTipAmount)} MXN
            </Text>
          </View>
          <View style={[styles.totalsRow, {marginTop: 8}]}>
            <Text
              style={[
                styles.totLabel,
                {fontWeight: '800', fontSize: clampLocal(rf(14), 13, 20)},
              ]}>
              Total con propina
            </Text>
            <Text
              style={[
                styles.totValue,
                {fontWeight: '900', fontSize: clampLocal(rf(20), 16, 28)},
              ]}>
              {formatMoney(effectiveTotalWithTip)} MXN
            </Text>
          </View>
          ...
          <View style={styles.buttonsWrap}>
            <TouchableOpacity
              onPress={payNow}
              activeOpacity={0.85}
              style={styles.payPrimaryWrapper}
              hitSlop={{top: 6, bottom: 6, left: 8, right: 8}}>
              <View
                style={[
                  styles.payPrimaryInner,
                  {
...
const styles = StyleSheet.create({
  ...
  tabtrackLogo: {
    width: Math.min(staticWidth * 0.32, 160),
    height: Math.min(staticWidth * 0.32 * 0.32, 60),
    marginBottom: 8,
  },
  logoWrap: {
    marginTop: 6,
  ...
  rightCol: {alignItems: 'flex-end', right: 20},
  ...
  content: {
    width: Math.min(staticWidth - 32, 420),
    backgroundColor: '#fff',
    padding: 16,
    marginTop: 8,
    borderRadius: 10,
  ...
  payButtonWrapper: {
    ...
    marginTop: 12,
    minHeight: 52,
    // Optional if you want it closer to Escanear "primary" feel:
    // shadowColor: '#085bff',
  ...
  payPrimaryInner: {
    ...
    alignItems: 'center',
    justifyContent: 'center',
    overflow: 'hidden', // same idea as WelcomeScreen
  },

  payPrimaryGradientBg: {
