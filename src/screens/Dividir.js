...
function groupConsumptionItems(flatItems = []) {
  ...
  return grouped.map(group => {
    ...
    const groupPrice = round2(
      members.reduce((s, m) => s + Number(m.price || 0), 0),
    );
    ...
    return {
      ...
      groupChecked:
        ...
        checkedMembers.length === selectableMembers.length,
    };
  });
}

// --- NUEVO: firma que identifica si dos "líneas" (item + sus subitems) son
// EXACTAMENTE iguales: mismo nombre, mismo precio total de línea (incluyendo
// modificadores) y mismos subitems (nombre+precio). Solo líneas con firma
// idéntica se agrupan bajo un mismo selector con +/-. Un "café con leche" y
// un "café solo" nunca comparten firma porque sus subitems difieren.
function getLineSignature(line) {
  const subs = Array.isArray(line.subitems) ? line.subitems : [];
  const subsSig = subs
    .map(
      s =>
        `${(s.name || '').trim().toLowerCase()}::${round2(s.price).toFixed(2)}`,
    )
    .sort()
    .join('|');
  return `${(line.name || '').trim().toLowerCase()}::${round2(
    line.price,
  ).toFixed(2)}::[${subsSig}]`;
}
...
export default function Dividir() {
  ...
  const [totalComensales, setTotalComensales] = useState(
    incomingComensales ?? null,
  );

  const [saleId, setSaleId] = useState(
    route?.params?.saleId ??
      route?.params?.sale_id ??
      route?.params?.venta_id ??
      null,
  );
  const [restauranteId, setRestauranteId] = useState(
    route?.params?.restauranteId ?? route?.params?.restaurante_id ?? null,
  );
  const [sucursalId, setSucursalId] = useState(
    route?.params?.sucursalId ?? route?.params?.sucursal_id ?? null,
  );
  const [mesaId, setMesaId] = useState(
    route?.params?.mesaId ?? route?.params?.mesa_id ?? null,
  );
  const [mesero, setMesero] = useState(
    route?.params?.mesero ?? route?.params?.waiter ?? null,
  );
  ...
  const normalizeItem = (raw, fallbackId) => {
    ...
    if (priceCandidate === null) {
      ...
      else if (raw?.line_total_price !== undefined)
        priceCandidate = raw.line_total_price;
    }

    if (priceCandidate === null) {
      if (raw?.unitPrice !== undefined)
        priceCandidate = Number(raw.unitPrice) * qty;
      else if (raw?.unit_price !== undefined)
        priceCandidate = Number(raw.unit_price) * qty;
      else if (raw?.unitprice !== undefined)
        priceCandidate = Number(raw.unitprice) * qty;
    }
    ...
    try {
      ...
      if (
        ...
        locked = true;
        paidInfo = {reason: 'flag_boolean', rawFlag: true};
      }
      ...
      if (
        !locked &&
        (Number(raw?.paid_amount || raw?.monto_pagado || raw?.monto_pago || 0) >
          0 ||
          Number(raw?.paid_qty || raw?.qty_paid || 0) > 0)
      ) {
        locked = true;
        paidInfo = {
          reason: 'paid_amount_or_qty',
          paid_amount:
            raw?.paid_amount ?? raw?.monto_pagado ?? raw?.monto_pago ?? 0,
        };
      }
  ...
  useEffect(() => {
    ...
    if (route?.params) {
      ...
      if (route.params.restauranteId)
        setRestauranteId(route.params.restauranteId);
      if (route.params.restaurante_id)
        setRestauranteId(route.params.restaurante_id);
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          console.warn(
            'Dividir: no se pudo obtener el consumo (HTTP ' +
              res.status +
              '). URL:',
            url,
          );
        ...
        if (
          json.total_comensales !== undefined &&
          json.total_comensales !== null
        ) {
        ...
        const rawItems = Array.isArray(json.items)
          ? json.items
          : json.data?.items ?? json.result?.items ?? [];
        const mapped = (
        ...
        if (mounted) {
          ...
          if (allowPaymentsCheck) {
            try {
              applyPaymentsLockIfPossible(mapped, json).catch(e =>
                console.warn('applyPaymentsLockIfPossible error', e),
              );
            } catch (err) {
    ...
    const applyPaymentsLockIfPossible = async (
      currentItems = [],
      consumoJson = null,
    ) => {
      try {
        ...
        const useSale =
          ...
          (route?.params?.saleId ??
            ...
            route?.params?.venta_id ??
            (consumoJson &&
              (consumoJson.sale_id ?? consumoJson.venta_id ?? consumoJson.id)));
        const useRest =
          restauranteId ||
          (route?.params?.restauranteId ??
            route?.params?.restaurante_id ??
            (consumoJson && consumoJson.restaurante_id));
        const useSuc =
          sucursalId ||
          (route?.params?.sucursalId ??
            route?.params?.sucursal_id ??
            (consumoJson && consumoJson.sucursal_id));

        if (!useSale || !useRest || !useSuc) {
          if (consumoJson) {
            const saleState = (
              consumoJson.sale_state ??
              consumoJson.saleState ??
              ''
            )
              .toString()
              .toUpperCase();
            const paymentsTotal =
              consumoJson.payments_total ??
              consumoJson.paymentsTotal ??
              consumoJson.payments_total_venta ??
              null;
            const totalConsumo =
              consumoJson.total_consumo ??
              consumoJson.total ??
              externalTotalConsumo ??
              null;
            if (
              saleState === 'CLOSED' &&
              paymentsTotal !== null &&
              Number(paymentsTotal) >= Number(totalConsumo || 0) &&
              Number(totalConsumo || 0) > 0
            ) {
              const allLocked = (currentItems || []).map(it => ({
                ...it,
                locked: true,
                paidInfo: {reason: 'sale_closed_and_fully_paid'},
                checked: false,
              }));
              setItems(allLocked);
            }
        ...
        const pagosUrl = `${hostBase}/api/restaurantes/${encodeURIComponent(
          useRest,
        )}/sucursales/${encodeURIComponent(useSuc)}/ventas/${encodeURIComponent(
          useSale,
        )}/pagos`;
        ...
        if (!res.ok) {
          console.warn(
            'Dividir: no se pudo consultar pagos (http ' + res.status + ')',
            pagosUrl,
          );
          return;
        }
        ...
        const paymentsArr = Array.isArray(payJson.payments)
          ? payJson.payments
          : Array.isArray(payJson.data?.payments)
          ? payJson.data.payments
          : [];
        const paymentsTotalRaw =
          payJson.payments_total ??
          payJson.paymentsTotal ??
          payJson.payments_total_venta ??
          null;
        const paymentsTotal =
          paymentsTotalRaw !== null
            ? Number(paymentsTotalRaw)
            : paymentsArr.length
            ? paymentsArr.reduce((s, p) => s + Number(p.amount || 0), 0)
            : null;
        const saleState = (payJson.sale_state ?? payJson.saleState ?? '')
          .toString()
          .toUpperCase();
        ...
        if (
          ...
          const allLocked = (currentItems || []).map(it => ({
            ...it,
            locked: true,
            paidInfo: {reason: 'sale_closed_and_fully_paid_from_payments'},
            checked: false,
          }));
          setItems(allLocked);
          return;
              ...
              cand.forEach(pi => {
                ...
                if (typeof pi === 'object') {
                  if (pi.id) paidIdsSet.add(String(pi.id));
                    ...
                    paidIdsSet.add(String(pi.name).toLowerCase());
                } else {
                  paidIdsSet.add(String(pi));
                }
              });
          ...
          const mapped = (currentItems || []).map(it => {
            ...
            if (matched) {
              return {
                ...
                locked: true,
                checked: false,
                paidInfo: {reason: 'matched_in_payments_ids'},
              };
            }
            return it;
          });
        ...
        if (Array.isArray(paymentsArr) && paymentsArr.length > 0) {
          ...
          for (const p of paymentsArr) {
            ...
            const matchIndex = currentCopy.findIndex(
              it => !it.locked && Math.abs(Number(it.price || 0) - amt) <= eps,
            );
            if (matchIndex >= 0) {
              currentCopy[matchIndex] = {
                ...currentCopy[matchIndex],
                locked: true,
                checked: false,
                paidInfo: {reason: 'matched_by_exact_amount'},
              };
              changed = true;
            } else {
  ...
  // --- NUEVO: arma la lista de renderizado agrupando líneas idénticas por
  // producto (firma), SIN separar en dos filas lo ya pagado de lo pendiente
  // del mismo producto. Si un producto tiene unidades disponibles (sin
  // pagar), se muestra UNA sola fila con selector +/- cuyo máximo es esa
  // cantidad disponible — las unidades ya pagadas de ese mismo producto no
  // aparecen aparte. Solo cuando ya no queda ninguna unidad disponible
  // (todas pagadas) esa fila cambia a mostrarse bloqueada con candado.
  // Líneas con bloqueo parcial dentro de sí mismas (por ejemplo el item
  // pagado pero un subitem no) se muestran individuales, exactamente como
  // antes, para no arriesgar la conciliación de pagos.
  const renderList = useMemo(() => {
    const map = new Map();
    const list = [];
    const seen = new Set();

    groupedItems.forEach((line, idx) => {
      ...
      if (map.has(sig)) {
        const g = map.get(sig);
        if (allLocked) g.lockedLines.push(line);
        else g.unlockedLines.push(line);
      } else {
        map.set(sig, {
          sig,
          name: line.name,
          unitPrice: round2(line.groupPrice ?? 0),
          subitemsTemplate: Array.isArray(line.subitems) ? line.subitems : [],
          lockedLines: allLocked ? [line] : [],
          unlockedLines: allLocked ? [] : [line],
        });
      }

      if (!seen.has(sig)) {
        seen.add(sig);
        list.push({type: 'group', key: sig, sig});
      }
    });

    return list.map(entry => {
      if (entry.type === 'single') return entry;
      return {...entry, group: map.get(entry.sig)};
    });
  }, [groupedItems]);

  // --- NUEVO: aplica la cantidad seleccionada (0..disponibles) para un
  // grupo de items idénticos, marcando `checked` en las primeras N líneas
  // libres del grupo y desmarcando el resto. No toca items fuera del grupo
  // ni las líneas ya bloqueadas (pagadas) de ese mismo producto.
  const setGroupSelectionCount = (group, newCount) => {
    if (!group) return;
    const max = group.unlockedLines.length;
    const clamped = Math.max(0, Math.min(newCount, max));

    const idsToCheck = new Set();
    group.unlockedLines.slice(0, clamped).forEach(line => {
      (line.members || []).forEach(m => {
        if (!m.locked) idsToCheck.add(String(m.id));
      });
    });

    const allGroupIds = new Set();
    group.unlockedLines.forEach(line =>
      (line.members || []).forEach(m => allGroupIds.add(String(m.id))),
    );

    setItems(prev =>
      prev.map(it => {
        if (!allGroupIds.has(String(it.id))) return it;
        if (it.locked) return it;
        return {...it, checked: idsToCheck.has(String(it.id))};
      }),
    );
  };
  ...
  const toggleGroup = index => {
    setItems(prev => {
      ...
      const memberIds = new Set(
        (target.members || []).filter(m => !m.locked).map(m => String(m.id)),
      );
  ...
  const usesExternalTotal =
    externalTotalConsumo !== null && externalTotalConsumo !== undefined;
  const total = useMemo(
    () =>
  ...
  const selectedTotal = useMemo(
    () =>
      round2(
        (selectedItems || []).reduce((s, it) => s + Number(it.price || 0), 0),
      ),
    [selectedItems],
  );
  ...
  const savePendingLocal = async (saleIdLocal, idsArray = [], amount = 0) => {
    try {
      if (!saleIdLocal || !Array.isArray(idsArray) || idsArray.length === 0)
        return;
  ...
  const handlePorConsumo = () => {
    if (equalsSplitPaid) {
      showStyledAlert(
        ...
        'Se está procesando un pago por partes iguales para esta venta — no puedes usar "Por consumo".',
      );
      return;
    }
    ...
    const selTotalRaw = selected.reduce(
    ...
    if (saleId && selectedIdsArray.length > 0) {
      savePendingLocal(saleId, selectedIdsArray, selTotal).catch(e =>
        console.warn('savePendingLocal error', e),
      );
    }
  ...
  const handlePartesIguales = () => {
    if (anySelected) {
      showStyledAlert(
        'No permitido',
        'Has seleccionado productos. Para usar "Partes iguales" debes no seleccionar items. Deselecciona los items o utiliza "Por consumo".',
      );
      return;
    }
    ...
    if (!payload || payload.length === 0) {
      console.warn(
        'Dividir.handlePartesIguales: no hay productos para dividir.',
      );
      return;
    }

    const pTotalRaw =
      usesExternalTotal && !anySelected
        ? Number(total || 0)
        : payload.reduce((s, it) => s + Number(it.price || 0), 0);
  ...
  const handleOneExhibicion = () => {
    if (equalsSplitPaid) {
      showStyledAlert(
        'Pago por partes iguales',
        'Se está procesando un pago por partes iguales para esta venta — no puedes usar este método.',
      );
      return;
    }

    const payloadItems = anySelected
      ? selectedItems
      : items.filter(it => !it.locked);
    ...
    const pTotalRaw = payloadItems.reduce(
      (s, it) => s + Number(it.price || 0),
      0,
    );
    const pTotal = round2(pTotalRaw);
    ...
    if (anySelected && saleId && selectedIdsArray.length > 0) {
      savePendingLocal(saleId, selectedIdsArray, pTotal).catch(e =>
        console.warn('savePendingLocal error', e),
      );
    }
  ...
  const handleShare = async () => {
    try {
      ...
      if (fields.total_consumo)
        shareParts.push(
          `Consumo: ${formatMoney(fields.total_consumo)} ${moneda}`,
        );
      const message = shareParts.join('\n');

      await Share.share({title: 'Compartir cuenta', message}, {});
    } catch (err) {
    ...
    <SafeAreaView style={[styles.safe, {paddingTop: topSafe}]}>
      ...
      <ScrollView
        contentContainerStyle={[
          styles.container,
          {paddingBottom: Math.round(hp(3) + bottomSafe), flexGrow: 1},
        ]}>
        ...
        <LinearGradient
          ...
          <View style={[styles.gradientRow, {alignItems: 'flex-start'}]}>
            <View style={[styles.leftCol]}>
              <Image
                ...
                style={[
                  styles.tabtrackLogo,
                  {
                    width: Math.round(Math.min(120, wp(28))),
                    height: Math.round(Math.min(48, wp(28) * 0.32)),
              ...
              <View
                ...
                <Image
                  ...
                  style={[
                    ...
                    {
                      width: Math.round(Math.min(96, wp(16))),
                      height: Math.round(Math.min(96, wp(16))),
                      borderRadius: Math.round(Math.min(96, wp(16)) * 0.16),
                    },
                  ]}
                />
              </View>
            ...
            <View
              style={[
                styles.rightCol,
                {maxWidth: rightColWidth, marginRight: Math.max(12, wp(3))},
              ]}>
        ...
        <View style={[styles.whiteContent, {width: whiteContentWidth}]}>
            ...
            {items.length === 0 ? (
              ...
              renderList.map(entry => {
                if (entry.type === 'single') {
                  ...
                  const displayPrice = round2(
                    members.reduce((s, m) => s + Number(m.price || 0), 0),
                  );
                  const firstItem =
                    members.find(m => !m.isSubitem) || members[0] || group;
                  const groupName =
                    firstItem?.name || group?.name || 'Producto';
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() => !isLocked && toggleGroup(entry.index)}
                      ...
                      <View style={styles.itemLeft}>
                        <View
                          style={[
                            styles.checkbox,
                            isChecked && !isLocked && styles.checkboxChecked,
                          ]}>
                          {isChecked && !isLocked && (
                      ...
                      <View style={{alignItems: 'flex-end'}}>
                        <Text
                          ...
                          {formatMoney(displayPrice)} MXN
                        </Text>
                        {isLocked ? (
                          <Text
                            style={{
                              fontSize: Math.round(clamp(rf(2.8), 10, 12)),
                              color: '#ef4444',
                              marginTop: Math.round(hp(0.3)),
                              fontWeight: '700',
                            }}>
                            Pagado
                          </Text>
                ...
                // Un mismo producto (misma firma) se muestra en UNA sola
                // fila: si todavía hay unidades disponibles (sin pagar), se
                // muestra con el selector +/- limitado a esa cantidad — las
                // unidades ya pagadas de ese mismo producto no aparecen
                // aparte. Solo si ya no queda ninguna disponible se muestra
                // bloqueada con candado.
                const g = entry.group;
                const available = g.unlockedLines.length;
                const lockedCount = g.lockedLines.length;

                if (available === 0) {
                  return (
                    <View
                      key={entry.key}
                      style={[styles.itemRow, {opacity: 0.5}]}>
                      <View style={styles.itemLeft}>
                        ...
                        <View style={{flex: 1}}>
                          <Text
                            style={[styles.itemText, {color: '#9ca3af'}]}
                            numberOfLines={1}>
                            {g.name}
                            {lockedCount > 1 ? `  x${lockedCount}` : ''}
                          </Text>
                      ...
                      <View style={{alignItems: 'flex-end'}}>
                        <Text style={[styles.itemPrice, {color: '#9ca3af'}]}>
                          {formatMoney(g.unitPrice * lockedCount)} MXN
                        </Text>
                        <Text
                          style={{
                            fontSize: Math.round(clamp(rf(2.8), 10, 12)),
                ...
                // Hay unidades disponibles: fila con selector +/-
                const selectedCount = g.unlockedLines.filter(
                  l => l.groupChecked,
                ).length;
                  ...
                  <View key={entry.key} style={styles.itemRow}>
                    ...
                    <View style={{alignItems: 'flex-end'}}>
                      <Text style={styles.itemPrice}>
                        {formatMoney(g.unitPrice)} MXN
                      </Text>
                      <View style={styles.stepperContainer}>
                        <TouchableOpacity
                          style={[
                            styles.stepperBtn,
                            selectedCount <= 0 && styles.stepperBtnDisabled,
                          ]}
                          onPress={() =>
                            setGroupSelectionCount(g, selectedCount - 1)
                          }
                        ...
                        <TouchableOpacity
                          ...
                          disabled={selectedCount >= available}
                          hitSlop={{top: 8, bottom: 8, left: 8, right: 8}}>
                          <Text style={styles.stepperBtnText}>+</Text>
                        </TouchableOpacity>
                      </View>
          ...
          <View style={styles.desgloseContainer}>
            ...
            <View style={[styles.desgloseRow, styles.ivaRow]}>
              ...
              <Text style={[styles.desgloseValue, styles.ivaText]}>
                {formatMoney(iva)} MXN
              </Text>
            </View>

            <View
              style={[styles.desgloseRow, {marginTop: Math.round(hp(0.5))}]}>
              <Text
                style={[
          ...
          {anySelected && (
            <Text style={styles.selectedTotalText}>
              Seleccionado: {formatMoney(selectedTotal)} MXN
            </Text>
          )}
          ...
          <LinearGradient
            ...
            style={[
              styles.shareButtonGradient,
              {width: shareBtnWidth, marginTop: Math.round(hp(1))},
            ]}>
            <TouchableOpacity
      ...
      {styledAlertVisible && (
        <View style={styles.modalBackdrop}>
          <LinearGradient
            colors={['#fff', '#fff', '#fff']}
            style={[styles.modalBox, {width: modalBoxWidth}]}>
            <Ionicons
              name="alert-circle"
              size={Math.round(rf(9))}
              color="#0046ff"
              style={{marginTop: 10, marginBottom: 10}}
            />
            <Text style={[styles.modalTitle, {color: '#0046ff'}]}>
              {styledAlertTitle}
            </Text>
            <Text
              style={[
...
function makeStyles({
  ...
  return StyleSheet.create({
    ...
    container: {
      alignItems: 'center',
      paddingTop: Math.round(hp(1)),
      paddingBottom: Math.round(hp(3) + bottomSafe),
    },
    ...
    ghostButton: {
      width: isNarrow ? '100%' : Math.round(wp(38)),
      borderWidth: 1.6,
      borderColor: 'rgba(255,255,255,0.6)',
      paddingVertical: Math.round(hp(1.4)),
      paddingHorizontal: Math.round(wp(4)),
      borderRadius: Math.round(wp(2)),
      marginBottom: Math.round(hp(0.8)),
    },
    ghostButtonText: {
      color: '#fff',
      fontWeight: '700',
      textAlign: 'center',
      fontSize: Math.round(clamp(rf(3.6), 13, 16)),
    },
    ...
    pinkButtonText: {
      ...
      fontWeight: '800',
      textAlign: 'center',
      fontSize: Math.round(clamp(rf(3.8), 14, 18)),
    },

    whiteContent: {
      width:
        whiteContentWidth ||
        Math.round(Math.min(width - Math.round(wp(2)), 960)),
      backgroundColor: '#fff',
    ...
    checkbox: {
      width: Math.round(clamp(rf(3.6), 16, 22)),
      height: Math.round(clamp(rf(3.6), 16, 22)),
      borderRadius: Math.round(wp(0.8)),
      borderWidth: 1.6,
      borderColor: '#cbd5e1',
    ...
    checkboxInner: {
      width: Math.round(clamp(rf(1.6), 6, 10)),
      height: Math.round(clamp(rf(1.6), 6, 10)),
      backgroundColor: '#fff',
      borderRadius: Math.round(wp(0.4)),
    },
    itemText: {fontSize: Math.round(clamp(rf(3.8), 13, 16)), color: '#374151'},
    itemPrice: {
      width: Math.round(Math.min(140, wp(36))),
      textAlign: 'right',
      color: '#111827',
    ...
    desgloseRow: {
      flexDirection: 'row',
      justifyContent: 'space-between',
      paddingVertical: Math.round(hp(0.8)),
    },
    desgloseLabel: {
      fontSize: Math.round(clamp(rf(3.8), 14, 18)),
      color: '#374151',
    ...
    desgloseValue: {
      fontSize: Math.round(clamp(rf(4.2), 16, 20)),
      color: '#111827',
      fontWeight: '900',
    },
    ivaRow: {paddingTop: 0},
    ...
    iconWrap: {
      marginHorizontal: Math.round(wp(2.4)),
      width: Math.round(clamp(rf(8.8), 36, 56)),
      height: Math.round(clamp(rf(8.8), 36, 56)),
      borderRadius: Math.round(clamp(rf(8.8), 36, 56) / 2),
      backgroundColor: '#fff',
      alignItems: 'center',
      justifyContent: 'center',
      elevation: 2,
    ...
    modalBackdrop: {
      ...
      backgroundColor: 'rgba(0,0,0,0.35)', // helps readability + "modal" feel
    },

    modalBox: {
      width: modalBoxWidth || Math.round(Math.min(width - 48, 420)),
      borderRadius: 18,
      padding: 5,
      alignItems: 'center',
      backgroundColor: '#fff', // optional safeguard even with LinearGradient
      paddingBottom: 15,
    },
    ...
    // --- NUEVO: estilos del selector +/- para items agrupados por cantidad
    stepperContainer: {
      flexDirection: 'row',
      alignItems: 'center',
      marginTop: 6,
    ...
    stepperBtn: {
      width: Math.round(clamp(rf(6.4), 26, 34)),
      height: Math.round(clamp(rf(6.4), 26, 34)),
      alignItems: 'center',
      justifyContent: 'center',
    ...
    stepperBtnText: {
      ...
      fontWeight: '900',
      fontSize: Math.round(clamp(rf(4.2), 16, 20)),
      lineHeight: Math.round(clamp(rf(4.6), 18, 22)),
    },
    stepperCount: {
      minWidth: Math.round(clamp(rf(6.4), 26, 34)),
      textAlign: 'center',
      color: '#111827',
      fontWeight: '800',
      fontSize: Math.round(clamp(rf(3.6), 13, 16)),
