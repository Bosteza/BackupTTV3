  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // assuming newest accounts are saved first
        const recentTwo = normalized.slice(0, 2);

        if (mounted) setAccounts(recentTwo);
      } catch (e) {
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.container}>
        <Image
          ...
          style={[
            styles.logo,
            {width: logoWidth, height: Math.round(logoWidth * 0.42)},
          ]}
          resizeMode="contain"
        />
        ...
        <View
          style={{
            width: '100%',
            paddingHorizontal: Math.round(Math.min(width * 0.06, 28)),
          }}>
          {accounts.length === 0 ? (
            <View style={styles.empty}>
              <Text style={styles.emptyText}>
                No hay cuentas recientes guardadas.
              </Text>
