...
export default function RestaurantDetailScreen() {
  ...
  const restauranteIdParam = (() => {
    ...
    if (branchParam?.raw?.restaurante_id)
      return String(branchParam.raw.restaurante_id);
    return null;
  })();
    ...
    (async () => {
      ...
      try {
        ...
        if (found) {
          ...
          setData({
            ...found,
            id: String(found.id ?? idParam),
            restaurante_id: restauranteIdParam,
          });
        } else {
          console.warn(
            '[RestaurantDetailScreen] No se encontró la sucursal del link:',
            {restauranteIdParam, idParam},
          );
          _showToast('No se encontró la sucursal');
        }
      } catch (e) {
        console.warn(
          '[RestaurantDetailScreen] Error resolviendo sucursal desde deep link:',
          e,
        );
  ...
  const toggleFavorite = async () => {
    try {
      ...
      if (!Array.isArray(objs)) {
        const globalObjsRaw = await AsyncStorage.getItem(
          GLOBAL_FAVORITES_OBJS_KEY,
        );
  ...
  const buildShareLink = () => {
    const restId =
      data?.restaurante_id ??
      data?.raw?.restaurante_id ??
      restauranteIdParam ??
      null;
    const sucId = idParam ?? data?.id ?? null;
    if (!restId || !sucId) return null;
    return `${APP_SCHEME}restaurante/${encodeURIComponent(
      restId,
    )}/sucursal/${encodeURIComponent(sucId)}`;
  };
  ...
  const onShare = async () => {
    ...
    const phone = data.telefono_sucursal
      ? `Tel: ${data.telefono_sucursal}`
      : '';
  ...
  const openMap = () => {
    const lat = parseFloat(
      data?.latitud ?? data?.latitude ?? data?.lat ?? null,
    );
    const lng = parseFloat(
      data?.longitud ?? data?.longitude ?? data?.lng ?? null,
    );
    ...
    Linking.openURL(url).catch(() =>
      _showToast('No se pudo abrir la app de mapas'),
    );
  };
  ...
  const openSocial = async platform => {
    ...
    if (platform === 'whatsapp') {
      ...
      if (!url) {
        const phone =
          data.telefono_sucursal || data.telefono || data.phone || null;
        if (phone) {
  ...
  const buildAddressFromParts = d => {
    ...
    let streetPart = [calle, numExt ? `#${numExt}` : null]
      .filter(Boolean)
      .join(' ');
    if (numInt)
      streetPart = streetPart
        ? `${streetPart}, Int. ${numInt}`
        : `Int. ${numInt}`;

    const localityPart =
      municipio && ciudad && String(municipio).trim() !== String(ciudad).trim()
        ? `${municipio}, ${ciudad}`
  ...
  const displayBranchName =
    branchNameParam || data?.nombre || data?.name || null;
    ...
    try {
      ...
      const imgsFromData =
        Array.isArray(data.imagenes) && data.imagenes.length > 0
          ...
          : null;
      const imgsFromRaw =
        Array.isArray(data?.raw?.imagenes) && data.raw.imagenes.length > 0
          ? data.raw.imagenes
          : null;
      const imgsFromDataImages =
        Array.isArray(data.images) && data.images.length > 0
          ? data.images
          : null;
      const imgsFromRawImages =
        Array.isArray(data?.raw?.images) && data.raw.images.length > 0
          ? data.raw.images
          : null;
      ...
      const pushImageCandidate = it => {
        ...
        if (typeof it === 'string') {
          ...
          const candidate =
            it.url ?? it.path ?? it.imagen ?? it.imagen_url ?? it.src ?? it;
  ...
  const hasFacebook = Boolean(
    data.url_facebook || data.facebook_url || data.facebook || data.fb,
  );
  const hasInstagram = Boolean(
    data.url_instagram || data.instagram_url || data.instagram,
  );
          ...
          {images.length > 0 ? (
            images.map((uri, i) => (
              <Image
                ...
                style={[
                  styles.sliderImage,
                  {width: width, height: SLIDER_HEIGHT},
                ]}
              />
            ))
          ) : (
            <Image
              source={placeholderBanner}
              style={[
                styles.sliderImage,
                {width: width, height: SLIDER_HEIGHT},
              ]}
      ...
      <ScrollView
        ...
        <View
            ...
            <View style={{flexShrink: 1}}>
              {displayRestaurantName ? (
                <Text
                  style={[
                    styles.title,
                    {fontSize: Math.max(12, Math.round(TITLE_FONT))},
                  ]}
                  numberOfLines={1}>
                  {displayRestaurantName}
                </Text>
          ...
          <Text
            style={[
              styles.sectionTitle,
              {fontSize: Math.max(12, Math.round(TITLE_FONT * 0.45))},
            ]}>
            Descripción breve
          </Text>
          <Text style={[styles.paragraph, {fontSize: PARAGRAPH_FONT}]}>
            {data.descripcion ?? data.short_description ?? '—'}
          </Text>
              ...
              {data.horarios.map((h, i) => {
                ...
                return (
                  <Text
                    ...
                    style={[
                      styles.paragraph,
                      {fontSize: PARAGRAPH_FONT},
                    ]}>{`${dia}: ${apertura} - ${cierre}`}</Text>
                );
              })}
            </>
          ) : null}
          ...
          {data.telefono_sucursal ? (
            <>
              ...
              <TouchableOpacity
                onPress={() =>
                  Linking.openURL(`tel:${data.telefono_sucursal}`).catch(() =>
                    _showToast('No se pudo marcar'),
                  )
                }>
                <Text
                  style={[
                    styles.paragraph,
                    {color: BLUE, fontSize: PARAGRAPH_FONT},
                  ]}>
                  {data.telefono_sucursal}
                </Text>
              </TouchableOpacity>
          ...
          <View style={styles.buttonRow}>
            <TouchableOpacity
              ...
              <Text
                style={[
                  styles.buttonText,
                  {fontSize: Math.max(14, Math.round(PARAGRAPH_FONT * 0.95))},
                ]}>
                Reservar
              </Text>
            </TouchableOpacity>
          ...
          <View style={styles.socialIconsRow}>
            <TouchableOpacity
              style={[
                ...
                {
                  width: SOCIAL_ICON_SIZE * 1.8,
                  height: SOCIAL_ICON_SIZE * 1.8,
                  borderRadius: Math.round((SOCIAL_ICON_SIZE * 1.8) / 2),
                },
              ]}
              onPress={() =>
                hasWhatsapp
                  ? openSocial('whatsapp')
                  : _showToast('No disponible en esta sucursal')
              }
              activeOpacity={hasWhatsapp ? 0.8 : 1}>
              <Ionicons
                name="logo-whatsapp"
                size={SOCIAL_ICON_SIZE}
                color={hasWhatsapp ? '#25D366' : '#bfc7cc'}
              />
            </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              <Ionicons
                name="logo-facebook"
                size={SOCIAL_ICON_SIZE}
                color={hasFacebook ? '#1877F2' : '#bfc7cc'}
              />
            </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              <Ionicons
                name="logo-instagram"
                size={SOCIAL_ICON_SIZE}
                color={hasInstagram ? '#C13584' : '#bfc7cc'}
              />
            </TouchableOpacity>
          </View>
        </View>
