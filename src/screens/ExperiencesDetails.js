...
function candidateMatchesCodigo(candidateRaw, codigoRaw) {
  ...
  if (candidate.includes(codigoBase) || codigoBase.includes(candidate))
    return true;
...
export default function DetailScreen({navigation, route}) {
  ...
  const tryFetchSplits = async theVisit => {
    ...
    try {
      ...
      const propinasArr = Array.isArray(json.propinas_por_tx)
        ...
        : Array.isArray(json.data?.propinas_por_tx)
        ? json.data.propinas_por_tx
        : [];
      ...
      for (const s of userPaidSplits) {
        const tx =
          ...
          s.transactionId ??
          null;
        if (tx !== undefined && tx !== null && String(tx).trim())
      ...
      if (
        ...
        for (const p of propinasArr) {
          ...
          if (txIdSet.has(String(ptx))) {
            userTipSum += safeNum(
              p.monto_propina ?? p.montoPropina ?? p.amount ?? 0,
            );
          }
        }
      }
      ...
      for (const s of userPaidSplits) {
        ...
        if (visitItems && visitItems.length > 0) {
          for (const it of visitItems) {
            ...
            const anyMatch = cands.some(c =>
              candidateMatchesCodigo(c, codigoRaw),
            );
            if (anyMatch) {
              matched = it;
              break;
        ...
        if (thisTx && Array.isArray(propinasArr)) {
          for (const p of propinasArr) {
            const ptx =
              p.payment_transaction_id ?? p.paymentTransactionId ?? null;
        ...
        if (matched) {
          const unitPrice =
            safeNum(
              matched.unitPrice ?? matched.price ?? matched.lineTotal ?? precio,
            ) || precio;
      ...
      for (const it of built) {
        ...
        if (foundIdx >= 0) {
          ...
          aggregated[foundIdx].lineTotal = +(
            aggregated[foundIdx].lineTotal + safeNum(it.lineTotal)
          ).toFixed(2);
          aggregated[foundIdx].splitTip = +(
            aggregated[foundIdx].splitTip + safeNum(it.splitTip)
          ).toFixed(2);
        } else {
          aggregated.push({
            key,
            ...
            splitTip: +safeNum(it.splitTip).toFixed(2),
          });
        }
      }
  ...
  const handleOpenWhatsApp = async () => {
    try {
      ...
      const visitUrl =
        (visit &&
          (visit.whatsapp_url ?? visit.whatsappUrl ?? visit.whatsapp)) ??
        null;
      ...
      if (!urlToOpen || String(urlToOpen).trim().length === 0) {
        Alert.alert(
          'URL no disponible',
          'No se encontró la URL de WhatsApp para abrir.',
        );
        return;
      }
      ...
      if (!can) {
        try {
          ...
          if (!can2) {
            Alert.alert(
              'No se puede abrir',
              'No pude abrir la URL de WhatsApp en este dispositivo.',
            );
            return;
          }
  ...
  const computeFilteredTotals = () => {
    const totalPaid = shouldShowFiltered
      ? filteredItems.reduce((s, it) => s + safeNum(it.lineTotal ?? 0), 0)
      : 0;
    ...
    const totalDisplayed = +((totalPaid || 0) + (propinaFiltered || 0)).toFixed(
      2,
    );

    return {
      subtotal: subtotalFiltered,
      iva: ivaFiltered,
      propina: propinaFiltered,
  ...
  return (
    <SafeAreaView
      ...
      <ScrollView
        ...
        <Text style={[styles.sectionHeading, {fontSize: sectionHeadingFont}]}>
          Detalle
        </Text>
          ...
          <View style={styles.totalTextWrapper}>
            ...
            <Text
              style={[
                styles.totalAmount,
                {
                  fontSize: totalAmountFont,
                  lineHeight: Math.round(totalAmountFont * 1.05),
                },
              ]}>
              ${total.toFixed(2)} {visit.moneda ?? 'MXN'}
            </Text>
            <Text
              style={[
        ...
        <Text style={[styles.tableInfo, {fontSize: clamp(rf(2.8), 12, 16)}]}>
          Mesa {visit.mesa ?? '—'} /{' '}
          {visit.fecha ? new Date(visit.fecha).toLocaleString() : '—'}
        </Text>
        ...
        {shouldShowFiltered ? (
          <>
            ...
            <View style={styles.itemRow}>
              ...
              <Text style={[styles.itemPrice, {fontSize: itemPriceFont}]}>
                ${filteredTotals ? filteredTotals.subtotal.toFixed(2) : '0.00'}{' '}
                {visit.moneda ?? 'MXN'}
              </Text>
            </View>
            ...
            <View style={styles.itemRow}>
              ...
              <Text style={[styles.itemPrice, {fontSize: itemPriceFont}]}>
                ${filteredTotals ? filteredTotals.iva.toFixed(2) : '0.00'}{' '}
                {visit.moneda ?? 'MXN'}
              </Text>
            </View>
            ...
            <View style={styles.itemRow}>
              ...
              <Text style={[styles.itemPrice, {fontSize: itemPriceFont}]}>
                ${filteredTotals ? filteredTotals.propina.toFixed(2) : '0.00'}{' '}
                {visit.moneda ?? 'MXN'}
              </Text>
            </View>
            ...
            <View style={[styles.itemRow, {marginTop: 6}]}>
              <Text
                style={[
                  styles.itemName,
                  {fontWeight: '800', fontSize: itemFont},
                ]}>
                Total tu parte
              </Text>
              <Text
                style={[
                  styles.itemPrice,
                  {fontWeight: '900', fontSize: itemPriceFont},
                ]}>
                ${filteredTotals ? filteredTotals.total.toFixed(2) : '0.00'}{' '}
                {visit.moneda ?? 'MXN'}
              </Text>
            </View>
          </>
          ...
          <View style={{padding: 12}}>
            <Text style={{color: '#666'}}>
              No se encontraron items pagados por este usuario en la
              transacción.
            </Text>
            <Text style={{color: '#666', marginTop: 6}}>
              Pulsa "Ver cuenta completa" para ver la cuenta completa.
            </Text>
          </View>
        ) : (
          <>
        ...
        <View style={[styles.pointsRow, {marginTop: Math.max(18, hp(2.5))}]}>
          <TouchableOpacity
            style={[
              styles.verCuentaBtn,
              {
                paddingVertical: btnPaddingVert,
