...
export default function Help({navigation}) {
  ...
  const openWhatsApp = async () => {
    try {
      ...
      const appScheme = `whatsapp://send?text=${encodeURIComponent(
        WHATSAPP_TEXT_FALLBACK,
      )}`;
      try {
  ...
  const openMailClient = async () => {
    try {
      ...
      Alert.alert(
        'Error',
        'Ocurrió un error al intentar abrir la app de correo.',
      );
    }
  };
  ...
  const handlePress = async key => {
    try {
      ...
      Alert.alert(
        'Error',
        'Ocurrió un error inesperado al intentar realizar la acción.',
      );
    }
  };
    ...
    <SafeAreaView style={[styles.container, {paddingTop: topSafe}]}>
      ...
      <View
        ...
        <Text
          style={[
            styles.headerTitle,
            {fontSize: clamp(Math.round(width * 0.055), 16, 24)},
          ]}>
          Perfil
        </Text>
          ...
          <View
            style={{
              ...
              borderRadius: Math.round(
                clamp(Math.round(width * 0.07), 28, 48) / 2,
              ),
              overflow: 'hidden',
            ...
            {profileUrl ? (
              ...
              <Text
                style={{
                  ...
                  fontWeight: '700',
                  fontSize: Math.round(
                    clamp(Math.round(width * 0.07), 28, 48) * 0.36,
                  ),
      ...
      <ScrollView
        ...
        <View
          style={[
            ...
            {paddingHorizontal: Math.max(6, Math.round(wp(1.5)))},
          ]}>
          {methods.map(m => (
            <TouchableOpacity
              key={m.key}
              style={[
                styles.methodRow,
                {paddingVertical: clamp(Math.round(hp(1.2)), 10, 18)},
              ]}
              activeOpacity={0.7}
              onPress={() => handlePress(m.key)}>
              <View style={styles.methodLeft}>
                <Ionicons
                  name={m.icon || 'chevron-forward-outline'}
                  size={Math.max(18, iconSize - 2)}
                  color={styles.methodText.color}
                />
                <Text
                  style={[
                    styles.methodText,
              ...
              {m.key === 'card2' ? (
                <Text
                  style={[
                    styles.editText,
                    {fontSize: clamp(Math.round(width * 0.036), 12, 16)},
                  ]}>
                  Activo
                </Text>
              ) : null}
