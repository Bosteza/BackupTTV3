...
export default function Consumo() {
  ...
  const passedSelected =
    route?.params?.selectedItems ?? route?.params?.items ?? null;
  ...
  const [items, setItems] = useState(
    Array.isArray(passedSelected) ? passedSelected : null,
  );
  ...
  const normalizeItem = (raw, fallbackId) => {
    const name =
      raw.nombre_item ?? raw.nombre ?? raw.name ?? raw.product_name ?? '';
    const price =
      Number(raw.precio_item ?? raw.precio ?? raw.price ?? raw.amount ?? 0) ||
      0;
  ...
  useEffect(() => {
    ...
    const fetchIfNeeded = async () => {
      ...
      if (!token) {
        openError(
          'Error',
          'No hay items ni token para obtener la información.',
        );
        return;
      }
      setLoading(true);
      try {
        ...
        if (!res.ok) {
          openError(
            'Error',
            `No se pudo obtener el consumo (HTTP ${res.status}).`,
          );
          return;
        }
        ...
        const mapped = (Array.isArray(rawItems) ? rawItems : []).map((it, i) =>
          normalizeItem(it, i),
        );
        ...
        openError(
          'Error',
          'No se pudo recuperar la cuenta. Revisa tu conexión.',
        );
      } finally {
  ...
  const {iva, subtotal, total} = useMemo(() => {
    if (
      incomingSubtotal !== null &&
      incomingIva !== null &&
      incomingTotal !== null
    ) {
      return {
        subtotal: round2(incomingSubtotal),
        iva: round2(incomingIva),
        total: round2(incomingTotal),
      };
    }
  ...
  if (loading || !items) {
    return (
      <View style={[stylesBase.loaderWrap, {backgroundColor: '#f5f7fb'}]}>
  ...
  useEffect(() => {
    ...
    const tAmt =
      route?.params?.tipAmount ?? route?.params?.monto_propina ?? null;
    const tTotalWithTip =
      route?.params?.totalWithTip ?? route?.params?.total_with_tip ?? null;
    const tPercent =
      route?.params?.tipPercent ?? route?.params?.tip_percent ?? null;
    ...
    route?.params?.tipApplied,
    route?.params?.tipAmount,
    route?.params?.totalWithTip,
    route?.params,
  ]);
      ...
      <ScrollView contentContainerStyle={styles.container}>
          ...
          <View style={styles.gradientRow}>
            ...
            <View style={styles.rightCol}>
              ...
              <View style={styles.rightThanks}>
                <Text style={styles.thanksText}>Items seleccionados</Text>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.desgloseContainer}>
            ...
            <View style={[styles.desgloseRow, {marginTop: hp(0.5)}]}>
              <Text
                style={[
                  styles.desgloseLabel,
                  {fontSize: Math.round(clamp(rf(3.8), 16, 20))},
                ]}>
                Total
              </Text>
              <Text
                style={[
                  ...
                  {fontSize: Math.round(clamp(rf(4.2), 18, 22))},
                ]}>
          ...
          {tipApplied && (
            <>
              ...
              <View style={styles.desgloseContainer}>
                <View style={styles.desgloseRow}>
                  <Text style={styles.desgloseLabel}>Propina</Text>
                  <Text style={styles.desgloseValue}>
                    {formatMoney(Number(tipApplied.tipAmount || 0))} MXN
                  </Text>
                </View>
                <View style={[styles.desgloseRow, {marginTop: hp(0.5)}]}>
                  <Text
          ...
          <LinearGradient
            ...
            <View style={styles.modalButtonsRow}>
              ...
              <TouchableOpacity
                ...
                <Text style={styles.modalBtnGhostText}>Volver</Text>
              </TouchableOpacity>
            </View>
          </LinearGradient>
        </View>
...
function makeStyles({wp, hp, rf, clamp, width, height, totalFont, insets}) {
  ...
  return StyleSheet.create({
    ...
    container: {
      alignItems: 'center',
      paddingBottom: Math.round(hp(3) + (insets?.bottom ?? 0)),
    ...
    tabtrackLogo: {
      width: Math.round(clamp(wp(28), 80, 160)),
      height: Math.round(clamp(rf(5.5), 28, 48)),
    ...
    totalNumber: {
      color: '#fff',
      fontWeight: '900' /* fontSize set dynamically */,
    },
    totalCurrency: {
      color: '#fff',
      fontSize: Math.round(clamp(rf(3.2), 12, 16)),
      marginLeft: Math.round(wp(1)),
      marginBottom: Math.round(hp(0.2)),
      opacity: 0.95,
    },
    ...
    sectionTitleLarge: {
      fontSize: Math.round(clamp(rf(6.6), 18, 32)),
      fontWeight: '700',
    ...
    separator: {
      ...
      marginVertical: Math.round(hp(1.6)),
    },
    ...
    primaryButton: {
      ...
      paddingVertical: Math.round(hp(1.8)),
      alignItems: 'center',
      marginTop: Math.round(hp(1.6)),
    },
    primaryButtonText: {
      color: '#fff',
      fontWeight: '800',
    ...
    modalBox: {
      width: Math.round(Math.min(width - Math.round(wp(12)), 520)),
