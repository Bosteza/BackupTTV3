...
export default function InfoPersonal({navigation}) {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const cumpleanos_raw =
          (await AsyncStorage.getItem('user_cumpleanos')) || '';
        ...
        const tipo_comida =
          (await AsyncStorage.getItem('user_tipo_comida')) || '';
        ...
        try {
          ...
          if (mailToQuery) {
            ...
            const endpoint = `${API_BASE_URL}?mail=${encodeURIComponent(
              mailToQuery,
            )}&presign_ttl=30`;
            ...
            try {
              ...
              console.warn(
                'InfoPersonal: error network fetching usuario',
                networkErr,
              );
            }
            ...
            if (resp && resp.ok) {
              try {
                ...
                const apiUser =
                  Array.isArray(body.usuarios) && body.usuarios.length > 0
                    ? body.usuarios[0]
                  ...
                  try {
                    ...
                    if (apiUser.foto_perfil_url) {
                      await AsyncStorage.setItem(
                        'user_profile_url',
                        apiUser.foto_perfil_url,
                      );
                      setProfileUrl(apiUser.foto_perfil_url);
                    }
                  } catch (e) {
                    console.warn(
                      'InfoPersonal: error guardando campos desde API en AsyncStorage',
                      e,
                    );
                  }
                } else {
                  console.warn(
                    'InfoPersonal: la respuesta API no contiene usuarios para el mail dado',
                  );
                }
              } catch (parseErr) {
                console.warn(
                  'InfoPersonal: error parsing user api response',
                  parseErr,
                );
              }
            } else if (resp) {
              ...
              console.warn(
                'InfoPersonal: fetch usuarios returned not-ok',
                resp.status,
                txt,
              );
            }
  ...
  useEffect(() => {
    ...
    return () => {
      try {
        keyboardListenerRef.current && keyboardListenerRef.current.remove();
      } catch (_) {}
    };
  }, [editingKey, user]);
  ...
  const showToast = (
    message,
    cb,
    customStyle = styles.successToast,
    duration = 2000,
  ) => {
    setToastMsg(message);
      ...
      if (cur) {
        ...
        if (!isNaN(tryIso.getTime())) initial = tryIso;
          ...
          if (
            ...
            const dd = parts[0],
              mm = parts[1],
              yy = parts[2].length === 2 ? 2000 + parts[2] : parts[2];
    ...
    requestAnimationFrame(() => {
      try {
        if (
          ...
          typeof currentInputRef.current.focus === 'function'
        ) {
      ...
      if (json) {
        ...
        for (const key in user) {
          try {
            ...
            console.warn(
              'Error guardando campo en AsyncStorage después de save:',
              key,
              e,
            );
          }
  ...
  const onDatePickerChange = async (event, selectedDate) => {
    if (Platform.OS === 'android') {
      ...
      try {
        await AsyncStorage.setItem('user_cumpleanos', display);
      } catch (e) {}
      setDatePickerValue(selectedDate);
      return;
    }

    // iOS: only update temp state, save on Confirm button
    if (selectedDate) {
      setDatePickerValue(selectedDate);
    }
  };
    ...
    <SafeAreaView style={[styles.container, {paddingTop: topSafe}]}>
        ...
        <View style={styles.headerRight}>
          ...
          <Text
            style={[
              styles.username,
              {
                fontSize: clamp(Math.round(width * 0.036), 13, 18),
      ...
      <ScrollView
        contentContainerStyle={[
          ...
          {paddingHorizontal: Math.min(36, Math.round(wp(6)))},
        ]}
        ...
        <View style={styles.sectionHeader}>
          <Ionicons
            ...
            color={BLUE}
          />
          <Text style={[styles.sectionTitle, {fontSize: labelFont}]}>
        ...
        {fields.map(([key, label], idx) => (
          <View key={key}>
            <TouchableOpacity
              ...
              style={[
                styles.fieldRow,
                {marginVertical: clamp(Math.round(hp(1.5)), 8, 16)},
              ]}>
              <Text style={[styles.fieldLabel, {fontSize: fieldFont}]}>
                {label}
              </Text>
              ...
              <View style={styles.fieldValueRow}>
                ...
                key !== 'cumpleanos' ? (
                  ...
                  <Text style={[styles.fieldValue, {fontSize: fieldFont}]}>
                    {key === 'cumpleanos'
                      ? user.cumpleanos || 'No especificado'
        ...
        <TouchableOpacity
          style={[
            styles.saveButton,
            {
              marginLeft: Math.min(35, Math.round(wp(6))),
              marginTop: clamp(Math.round(hp(4)), 18, 40),
            },
            saving && {opacity: 0.6},
          ]}
          onPress={handleSave}
        ...
        <View style={styles.modalBackground}>
          <View
            style={[
              ...
              {width: modalWidth, padding: Math.max(12, Math.round(wp(3)))},
            ]}>
            <Text
              style={[
                styles.modalTitle,
            ...
            <TextInput
              style={[
                styles.modalInput,
                {fontSize: fieldFont, padding: Math.max(8, Math.round(wp(2)))},
              ]}
              value={fieldValue}
            ...
            <View style={styles.modalButtons}>
              <Pressable
                ...
                <Text style={[styles.modalButtonText, {fontSize: fieldFont}]}>
                  Cancelar
                </Text>
              </Pressable>
              <Pressable
        ...
        <View style={styles.selectorOverlay}>
          <View
            style={[
              ...
              {width: Math.min(modalWidth, Math.round(width * 0.96))},
            ]}>
            <View style={styles.selectorHeader}>
              <Text
                style={[
                  styles.selectorTitle,
                  {fontSize: Math.max(16, labelFont)},
                ]}>
            ...
            {foodLoading ? (
              ...
              <View style={{padding: 16}}>
                <Text style={{color: '#a00'}}>
                  No se pudieron cargar las opciones ({foodFetchError}).
                </Text>
                <TouchableOpacity
                  onPress={fetchFoodTypes}
                  style={{marginTop: 12}}>
      ...
      {/* iOS styled picker from first version */}
          ...
          <View
            ...
            <View
              ...
              <View style={styles.dateActions}>
                <TouchableOpacity
                  onPress={async () => {
                    const display = formatDateDisplay(datePickerValue);
                    setUser(prev => ({...prev, cumpleanos: display}));
                    await AsyncStorage.setItem('user_cumpleanos', display);
                    setShowDatePicker(false);
                  }}
                  style={[
                    styles.dateBtnPrimary,
                    {backgroundColor: PlatformColor('systemPurple')},
                  ]}>
                  <Text
                    style={[
                      styles.dateBtnPrimaryText,
      ...
      <Animated.View
        ...
        style={[
          toastStyle,
          {
            ...
            bottom:
              Platform.OS === 'ios'
                ? Math.max(70, bottomSafe + 10)
                : Math.max(40, bottomSafe + 6),
          },
        ]}>
        <Text
          style={[
...
const styles = StyleSheet.create({
  ...
  selectorItemSelected: {
    backgroundColor: 'rgba(0,70,255,0.06)',
  },
  selectorItemText: {
    fontSize: 15,
    color: '#222',
