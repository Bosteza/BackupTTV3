...
export default function SaleDetail() {
  ...
  const fetchSplits = async (branchId, saleId) => {
    try {
      ...
      if (!res || !res.ok) {
        ...
        throw new Error(
          `Error fetching splits: ${res?.status ?? 'no-res'} ${txt || ''}`,
        );
      }
  ...
  const sumArray = (arr = [], key = 'subtotal') => {
    try {
      return arr.reduce(
        (acc, it) => acc + (Number(it?.[key] ?? it?.precio_unitario ?? 0) || 0),
        0,
      );
    } catch {
      return 0;
  ...
  const groupedTransactions = useMemo(() => {
    ...
    const tips = Array.isArray(data?.propinas_por_tx)
      ? data.propinas_por_tx
      : [];
    const map = new Map();
    ...
    for (const s of splits) {
      ...
      if (!map.has(tx))
        map.set(tx, {
          ...
          items: [],
          tip: 0,
          payer: s?.usuario_app_id ?? null,
        });
      map.get(tx).items.push(s);
      if (!map.get(tx).payer && s?.usuario_app_id)
        map.get(tx).payer = s.usuario_app_id;
    }
    for (const t of tips) {
    ...
    const arr = Array.from(map.values()).map(g => {
      const firstDate =
        g.items && g.items.length > 0
          ? new Date(
              g.items[0]?.fecha_pago ?? g.items[0]?.fecha_actualizacion ?? 0,
            ).getTime()
          : 0;
  ...
  const totalSubtotal = sumArray(
    Array.isArray(data?.splits) ? data.splits : [],
    'subtotal',
  );
  const totalTips = (
    Array.isArray(data?.propinas_por_tx) ? data.propinas_por_tx : []
  ).reduce((acc, p) => acc + (Number(p?.monto_propina ?? 0) || 0), 0);
    ...
    <View style={[styles.txCard, {padding: s(12), borderRadius: s(12)}]}>
      <View style={styles.txHeader}>
        <View style={styles.txHeaderLeft}>
          <Text style={[styles.txTitle, {fontSize: s(14)}]}>
            {item.payer ? `Pagó: ${item.payer}` : 'Pagador desconocido'}
          </Text>
        </View>

        <View style={styles.txHeaderRight}>
          <Text style={[styles.txAmount, {fontSize: s(16)}]}>
            {formatMoney(item.subtotal || 0)} MXN
          </Text>
        </View>
      </View>
      ...
      {Array.isArray(item.items) &&
        item.items.map((it, idx) => (
          <View key={idx} style={[styles.itemRow, {paddingVertical: s(6)}]}>
            <View style={{flex: 1}}>
              <Text style={[styles.itemName, {fontSize: s(14)}]}>
                {it?.nombre_item ?? `Item ${it?.id ?? ''}`}
              </Text>
              <Text style={[styles.itemSmallMeta, {fontSize: s(12)}]}>
                {it?.cantidad ?? '1'} ×{' '}
                {formatMoney(it?.precio_unitario ?? it?.subtotal ?? 0)}
              </Text>
            </View>

            <View style={{justifyContent: 'center'}}>
              <Text style={[styles.itemPrice, {fontSize: s(14)}]}>
                {formatMoney(it?.subtotal ?? it?.precio_unitario ?? 0)}
              </Text>
            </View>
          </View>
        ))}
      ...
      <View style={[styles.totalsRow, {marginTop: 0}]}>
        ...
        <Text style={[styles.totalsValue, {fontSize: s(13)}]}>
          {formatMoney(item.tip ?? 0)} MXN
        </Text>
      </View>
    </View>
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View
        style={[
          styles.header,
          {paddingTop: headerPaddingTop, paddingBottom: s(10)},
        ]}>
        <TouchableOpacity
      ...
      {loading ? (
        ...
        <View style={styles.center}>
          ...
          <TouchableOpacity
            onPress={() => fetchSplits(routeBranchId, routeSaleId)}
            style={[styles.retryBtn, {padding: s(10), borderRadius: s(8)}]}>
        ...
        <ScrollView
          ...
          <View style={[styles.topCard, {padding: s(16), borderRadius: s(12)}]}>
            <View style={{flex: 1}}>
              <Text
                style={[styles.branchNameBig, {fontSize: s(20)}]}
                numberOfLines={2}
                ellipsizeMode="tail">
                {displayBranchName}
              </Text>
              ...
              <View style={{marginTop: s(10)}}>
                <Text style={[styles.smallMeta, {fontSize: s(13)}]}>
                  Pago:{' '}
                  <Text style={[styles.boldText, {fontSize: s(14)}]}>
                    {formatMoney(totalSubtotal)} MXN
                  </Text>
                </Text>
                <Text
                  style={[
                    styles.smallMeta,
                    {marginTop: s(6), fontSize: s(13)},
                  ]}>
                  Propinas:{' '}
                  <Text style={[styles.boldText, {fontSize: s(14)}]}>
                    {formatMoney(totalTips)} MXN
                  </Text>
                </Text>
              </View>
            </View>
          ...
          <View style={{marginTop: s(14)}}>
            <Text style={[styles.sectionTitle, {fontSize: s(15)}]}>
              Transacciones
            </Text>
            ...
            {groupedTransactions.length > 0 ? (
              ...
              <Text style={{color: '#666', marginTop: s(6)}}>
                No hay detalles de transacciones disponibles.
              </Text>
            )}
          </View>
        </ScrollView>
